...
------------------------------------------------------------------------
-- ValidityStratified.agda  (NAT/ — Pi + U fragment)
--
...
module OpenRecords
  ...
  -- Nat value records (positive type: characterized by canonical form).
  -- M reduces to `Suc pred` at type A, with `pred` recursively valid as a
  -- Nat value at the predecessor code `v` (one rank lower => predecessor
  -- stage relation V2, exactly like PiCode's domain recursion).
  -- Annotated with the literal Nat type NatT (not the ambient A): the paired
  -- `vty G A NatCode` already witnesses A reduces to NatT, so the value record
  -- is type-INDEPENDENT and passes through type-forwarding/sup/symtrans
  -- unchanged.  (A kept as a parameter only so call sites need not change.)
  record RValNatSuc {n : Nat} (G : Ctx n) (M : Expr n) (v : FinEl) : Set where
    inductive
    field
      pred : Expr n
      red  : Red3 G M (Suc pred) NatT
      htP  : HasType G pred NatT
      valP : V2 G pred NatT v NatCode

  record REqValNatSuc {n : Nat} (G : Ctx n) (M N : Expr n) (v : FinEl) : Set where
    inductive
    field
      predM : Expr n
      predN : Expr n
      redM  : Red3 G M (Suc predM) NatT
      redN  : Red3 G N (Suc predN) NatT
      htM   : HasType G predM NatT
      htN   : HasType G predN NatT
      cvP   : ConvTm G predM predN NatT
      eqP   : EV2 G predM predN NatT v NatCode
...
buildStage B = record { val = vl ; eqval = evl ; valty = vty ; eqvalty = evty }
  where
    ...
    vty G M NatCode      = Red3 G M NatT U
    vty G M ZeroEl       = Top
    vty G M (SucEl x)    = Top
    ...
    evty G M N NatCode      = Pair (Red3 G M NatT U) (Red3 G N NatT U)
    evty G M N ZeroEl       = Top
    evty G M N (SucEl x)    = Top
    ...
    vl G M A NatCode UCode          = Pair (vty G A UCode) (vty G M NatCode)
    ...
    vl G M A ZeroEl    NatCode      = Pair (vty G A NatCode) (Red3 G M Zero NatT)
    vl G M A (SucEl v) NatCode      = Pair (vty G A NatCode) (RValNatSuc G M v)
    vl G M A u         NatCode      = Top
    vl G M A u         ZeroEl       = Top
    vl G M A u         (SucEl x)    = Top
    ...
    evl G M N A NatCode UCode          =
      Pair (vty G A UCode)
           (Pair (vty G M NatCode)
                 (Pair (vty G N NatCode) (evty G M N NatCode)))
    ...
    evl G M N A ZeroEl    NatCode      =
      Pair (vty G A NatCode) (Pair (Red3 G M Zero NatT) (Red3 G N Zero NatT))
    evl G M N A (SucEl v) NatCode      =
      Pair (vty G A NatCode)
           (Pair (RValNatSuc G M v)
                 (Pair (RValNatSuc G N v) (REqValNatSuc G M N v)))
    evl G M N A u         NatCode      = Top
    evl G M N A u         ZeroEl       = Top
    evl G M N A u         (SucEl x)    = Top
