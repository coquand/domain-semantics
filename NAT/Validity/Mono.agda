{-# OPTIONS --without-K #-}
------------------------------------------------------------------------
-- ValidityMono.agda  (NAT/ — Pi + U fragment)
--
...
module NAT.Validity.Mono where
...
open RS using (Expr ; Var ; U ; Pi ; Lam ; App ; NatT ;
  wkExpr ; subst1 ; Fin ; fzero ; fsuc)
open import NAT.Syntax.Typing using (Ctx ; empty ; extend ;
  HasType ; ConvTm ;
  WfCtx ;
...
open import NAT.Syntax.Reduction using (Red ; mkRed ; Red-hr ; HeadRed ; headred-refl ; HeadRed-trans ;
  HeadRed-App ;
  HeadRed-strip-Pi )
open import NAT.Domain.Kernel using (EvalFun ;
...
open import NAT.Validity.Core using (Red-unique-Pi ;
  bU-from-cf-fmFun ;
  FinMem-Coherent)
open import NAT.Domain.MemProps using (sucNat-to ; sucNat-from)
open import NAT.Syntax.Substitution using (typing-ConvTm ; ctx-conv-ConvTm ; ctx-conv-HasType ;
  subst1-cong-ConvTm)
...
------------------------------------------------------------------------
-- Nat value-component restrict combinators (the SucEl predecessor
-- recursion), analogous to restrictVal2-PiCode but over the atom NatCode:
-- restricting SucEl v -> SucEl v' restricts the predecessor validity from
-- code v to v' via the MonoPack's own restrictVal2 / restrictEqVal2.
------------------------------------------------------------------------

restrictVal2-NatSuc : (k : Nat) (mp : MonoPack k) {n : Nat} (G : Ctx n) (M : Expr n) (v v' : FinEl) ->
  LeCode v' v -> FinMem v' NatCode -> FinMem v NatCode ->
  SR.RValNatSuc k G M v -> SR.RValNatSuc k G M v'
restrictVal2-NatSuc k mp G M v v' le memv' memv vp = record
  { pred = RValNatSuc.pred vp
  ; red  = RValNatSuc.red vp
  ; htP  = RValNatSuc.htP vp
  ; valP = MonoPack.restrictVal2 mp G (RValNatSuc.pred vp) NatT v v' NatCode le memv' memv (RValNatSuc.valP vp)
  }
  where open SR k

restrictEqVal2-NatSuc : (k : Nat) (mp : MonoPack k) {n : Nat} (G : Ctx n) (M N : Expr n) (v v' : FinEl) ->
  LeCode v' v -> FinMem v' NatCode -> FinMem v NatCode ->
  SR.REqValNatSuc k G M N v -> SR.REqValNatSuc k G M N v'
restrictEqVal2-NatSuc k mp G M N v v' le memv' memv vp = record
  { predM = REqValNatSuc.predM vp
  ; predN = REqValNatSuc.predN vp
  ; redM  = REqValNatSuc.redM vp
  ; redN  = REqValNatSuc.redN vp
  ; htM   = REqValNatSuc.htM vp
  ; htN   = REqValNatSuc.htN vp
  ; cvP   = REqValNatSuc.cvP vp
  ; eqP   = MonoPack.restrictEqVal2 mp G (REqValNatSuc.predM vp) (REqValNatSuc.predN vp) NatT v v' NatCode le memv' memv (REqValNatSuc.eqP vp)
  }
  where open SR k
...
goodStage (suc n) = record
  ...
  where
    ...
    -- Nat type-codes (a0 -> a1).  a1 in {ZeroEl,SucEl} unreachable as a type
    -- (FinMem a1 UCode Empty) but le already rules out the non-Bot/Nat a0.
    dV G M T u UCode        NatCode      ()
    dV G M T u UCode        ZeroEl       ()
    dV G M T u UCode        (SucEl x)    ()
    dV G M T u (FunEl g)    NatCode      ()
    dV G M T u (FunEl g)    ZeroEl       ()
    dV G M T u (FunEl g)    (SucEl x)    ()
    dV G M T u (PiCode b0 f0) NatCode    ()
    dV G M T u (PiCode b0 f0) ZeroEl     ()
    dV G M T u (PiCode b0 f0) (SucEl x)  ()
    dV G M T u NatCode      Bot          ()
    dV G M T u NatCode      UCode        ()
    dV G M T u NatCode      (FunEl h)    ()
    dV G M T u NatCode      (PiCode b f) ()
    dV G M T u NatCode      NatCode      le mem ca0 ca1 src = src
    dV G M T u NatCode      ZeroEl       ()
    dV G M T u NatCode      (SucEl x)    ()
    dV G M T u ZeroEl       a1           le mem ca0 ca1 src = tt
    dV G M T u (SucEl x)    a1           le mem ca0 ca1 src = tt
    ...
    dEV G M N T u UCode        NatCode      ()
    dEV G M N T u UCode        ZeroEl       ()
    dEV G M N T u UCode        (SucEl x)    ()
    dEV G M N T u (FunEl g)    NatCode      ()
    dEV G M N T u (FunEl g)    ZeroEl       ()
    dEV G M N T u (FunEl g)    (SucEl x)    ()
    dEV G M N T u (PiCode b0 f0) NatCode    ()
    dEV G M N T u (PiCode b0 f0) ZeroEl     ()
    dEV G M N T u (PiCode b0 f0) (SucEl x)  ()
    dEV G M N T u NatCode      Bot          ()
    dEV G M N T u NatCode      UCode        ()
    dEV G M N T u NatCode      (FunEl h)    ()
    dEV G M N T u NatCode      (PiCode b f) ()
    dEV G M N T u NatCode      NatCode      le mem ca0 ca1 src = src
    dEV G M N T u NatCode      ZeroEl       ()
    dEV G M N T u NatCode      (SucEl x)    ()
    dEV G M N T u ZeroEl       a1           le mem ca0 ca1 src = tt
    dEV G M N T u (SucEl x)    a1           le mem ca0 ca1 src = tt
    ...
    dVT G M UCode        NatCode      ()
    dVT G M UCode        ZeroEl       ()
    dVT G M UCode        (SucEl x)    ()
    dVT G M (PiCode b0 f0) NatCode    ()
    dVT G M (PiCode b0 f0) ZeroEl     ()
    dVT G M (PiCode b0 f0) (SucEl x)  ()
    dVT G M NatCode      Bot          ()
    dVT G M NatCode      UCode        ()
    dVT G M NatCode      (FunEl h)    ()
    dVT G M NatCode      (PiCode b f) ()
    dVT G M NatCode      NatCode      le fmem cu1 src = src
    dVT G M NatCode      ZeroEl       ()
    dVT G M NatCode      (SucEl x)    ()
    dVT G M ZeroEl       u1           le ()
    dVT G M (SucEl x)    u1           le ()
    ...
    dEVT G M N UCode        NatCode      ()
    dEVT G M N UCode        ZeroEl       ()
    dEVT G M N UCode        (SucEl x)    ()
    dEVT G M N (PiCode b0 f0) NatCode    ()
    dEVT G M N (PiCode b0 f0) ZeroEl     ()
    dEVT G M N (PiCode b0 f0) (SucEl x)  ()
    dEVT G M N NatCode      Bot          ()
    dEVT G M N NatCode      UCode        ()
    dEVT G M N NatCode      (FunEl h)    ()
    dEVT G M N NatCode      (PiCode b f) ()
    dEVT G M N NatCode      NatCode      le fmem cu1 src = src
    dEVT G M N NatCode      ZeroEl       ()
    dEVT G M N NatCode      (SucEl x)    ()
    dEVT G M N ZeroEl       u1           le ()
    dEVT G M N (SucEl x)    u1           le ()
    ...
    uV G M T NatCode        UCode UCode le mem0 mem1 ca0 ca1 src vta1 = src
    uV G M T ZeroEl         UCode UCode le ()
    uV G M T (SucEl x)      UCode UCode le ()
    ...
    -- a0 = Bot, value u = Bot, target a1 a Nat type-code
    uV G M T Bot Bot NatCode      le mem0 mem1 ca0 ca1 src vta1 = src
    uV G M T Bot Bot ZeroEl       le mem0 ()
    uV G M T Bot Bot (SucEl x)    le mem0 ()
    -- a0 = Bot, value u a Nat code: FinMem u Bot Empty
    uV G M T NatCode   Bot a1 le ()
    uV G M T ZeroEl    Bot a1 le ()
    uV G M T (SucEl x) Bot a1 le ()
    -- a0 = UCode, a1 a Nat type-code: LeCode UCode _ Empty
    uV G M T u UCode NatCode      ()
    uV G M T u UCode ZeroEl       ()
    uV G M T u UCode (SucEl x)    ()
    -- a0 = FunEl
    uV G M T Bot (FunEl g) NatCode      ()
    uV G M T Bot (FunEl g) ZeroEl       ()
    uV G M T Bot (FunEl g) (SucEl x)    ()
    uV G M T NatCode   (FunEl g) a1 le ()
    uV G M T ZeroEl    (FunEl g) a1 le ()
    uV G M T (SucEl x) (FunEl g) a1 le ()
    -- a0 = PiCode, a1 a Nat type-code: LeCode PiCode _ Empty
    uV G M T u (PiCode b0 f0) NatCode      ()
    uV G M T u (PiCode b0 f0) ZeroEl       ()
    uV G M T u (PiCode b0 f0) (SucEl x)    ()
    -- a0 = NatCode: only a1 = NatCode (identity); others LeCode-absurd
    uV G M T u NatCode Bot          ()
    uV G M T u NatCode UCode        ()
    uV G M T u NatCode (FunEl h)    ()
    uV G M T u NatCode (PiCode b f) ()
    uV G M T u NatCode NatCode      le mem0 mem1 ca0 ca1 src vta1 = src
    uV G M T u NatCode ZeroEl       ()
    uV G M T u NatCode (SucEl x)    ()
    -- a0 in {ZeroEl,SucEl}: only equal Nat type-code reachable (target Top)
    uV G M T u ZeroEl Bot          ()
    uV G M T u ZeroEl UCode        ()
    uV G M T u ZeroEl (FunEl h)    ()
    uV G M T u ZeroEl (PiCode b f) ()
    uV G M T u ZeroEl NatCode      ()
    uV G M T u ZeroEl ZeroEl       le mem0 mem1 ca0 ca1 src vta1 = tt
    uV G M T u ZeroEl (SucEl x)    ()
    uV G M T u (SucEl x) Bot          ()
    uV G M T u (SucEl x) UCode        ()
    uV G M T u (SucEl x) (FunEl h)    ()
    uV G M T u (SucEl x) (PiCode b f) ()
    uV G M T u (SucEl x) NatCode      ()
    uV G M T u (SucEl x) ZeroEl       ()
    uV G M T u (SucEl x) (SucEl y)    le mem0 mem1 ca0 ca1 src vta1 = tt
    ...
    uEV G M N T NatCode        UCode UCode le mem0 mem1 ca0 ca1 src vta1 = src
    uEV G M N T ZeroEl         UCode UCode le ()
    uEV G M N T (SucEl x)      UCode UCode le ()
    ...
    uEV G M N T Bot Bot NatCode      le mem0 mem1 ca0 ca1 src vta1 = src
    uEV G M N T Bot Bot ZeroEl       le mem0 ()
    uEV G M N T Bot Bot (SucEl x)    le mem0 ()
    uEV G M N T NatCode   Bot a1 le ()
    uEV G M N T ZeroEl    Bot a1 le ()
    uEV G M N T (SucEl x) Bot a1 le ()
    uEV G M N T u UCode NatCode      ()
    uEV G M N T u UCode ZeroEl       ()
    uEV G M N T u UCode (SucEl x)    ()
    uEV G M N T Bot (FunEl g) NatCode      ()
    uEV G M N T Bot (FunEl g) ZeroEl       ()
    uEV G M N T Bot (FunEl g) (SucEl x)    ()
    uEV G M N T NatCode   (FunEl g) a1 le ()
    uEV G M N T ZeroEl    (FunEl g) a1 le ()
    uEV G M N T (SucEl x) (FunEl g) a1 le ()
    uEV G M N T u (PiCode b0 f0) NatCode      ()
    uEV G M N T u (PiCode b0 f0) ZeroEl       ()
    uEV G M N T u (PiCode b0 f0) (SucEl x)    ()
    uEV G M N T u NatCode Bot          ()
    uEV G M N T u NatCode UCode        ()
    uEV G M N T u NatCode (FunEl h)    ()
    uEV G M N T u NatCode (PiCode b f) ()
    uEV G M N T u NatCode NatCode      le mem0 mem1 ca0 ca1 src vta1 = src
    uEV G M N T u NatCode ZeroEl       ()
    uEV G M N T u NatCode (SucEl x)    ()
    uEV G M N T u ZeroEl Bot          ()
    uEV G M N T u ZeroEl UCode        ()
    uEV G M N T u ZeroEl (FunEl h)    ()
    uEV G M N T u ZeroEl (PiCode b f) ()
    uEV G M N T u ZeroEl NatCode      ()
    uEV G M N T u ZeroEl ZeroEl       le mem0 mem1 ca0 ca1 src vta1 = tt
    uEV G M N T u ZeroEl (SucEl x)    ()
    uEV G M N T u (SucEl x) Bot          ()
    uEV G M N T u (SucEl x) UCode        ()
    uEV G M N T u (SucEl x) (FunEl h)    ()
    uEV G M N T u (SucEl x) (PiCode b f) ()
    uEV G M N T u (SucEl x) NatCode      ()
    uEV G M N T u (SucEl x) ZeroEl       ()
    uEV G M N T u (SucEl x) (SucEl y)    le mem0 mem1 ca0 ca1 src vta1 = tt
    ...
    -- a = UCode, Nat value codes (NatCode : U informative; ZeroEl/SucEl not in U)
    rV G M T Bot NatCode      UCode ()
    rV G M T Bot ZeroEl       UCode ()
    rV G M T Bot (SucEl x)    UCode ()
    rV G M T UCode NatCode    UCode ()
    rV G M T UCode ZeroEl     UCode ()
    rV G M T UCode (SucEl x)  UCode ()
    rV G M T (FunEl g) NatCode    UCode ()
    rV G M T (FunEl g) ZeroEl     UCode ()
    rV G M T (FunEl g) (SucEl x)  UCode ()
    rV G M T (PiCode a' f') NatCode   UCode ()
    rV G M T (PiCode a' f') ZeroEl    UCode ()
    rV G M T (PiCode a' f') (SucEl x) UCode ()
    rV G M T NatCode Bot          UCode le mem fmu src = tt
    rV G M T NatCode UCode        UCode ()
    rV G M T NatCode (FunEl h)    UCode ()
    rV G M T NatCode (PiCode b f) UCode ()
    rV G M T NatCode NatCode      UCode le mem fmu src = src
    rV G M T NatCode ZeroEl       UCode ()
    rV G M T NatCode (SucEl x)    UCode ()
    rV G M T ZeroEl    u' UCode le mem ()
    rV G M T (SucEl x) u' UCode le mem ()
    ...
    -- a = NatCode (Nat values): restrict value code u -> u'
    rV G M T Bot Bot          NatCode le mem fmu src = src
    rV G M T Bot UCode        NatCode ()
    rV G M T Bot (FunEl h)    NatCode ()
    rV G M T Bot (PiCode b f) NatCode ()
    rV G M T Bot NatCode      NatCode ()
    rV G M T Bot ZeroEl       NatCode ()
    rV G M T Bot (SucEl x)    NatCode ()
    rV G M T UCode        u' NatCode le mem ()
    rV G M T (FunEl g)    u' NatCode le mem ()
    rV G M T (PiCode b f) u' NatCode le mem ()
    rV G M T NatCode      u' NatCode le mem ()
    rV G M T ZeroEl Bot          NatCode le mem fmu src = tt
    rV G M T ZeroEl UCode        NatCode ()
    rV G M T ZeroEl (FunEl h)    NatCode ()
    rV G M T ZeroEl (PiCode b f) NatCode ()
    rV G M T ZeroEl NatCode      NatCode ()
    rV G M T ZeroEl ZeroEl       NatCode le mem fmu src = src
    rV G M T ZeroEl (SucEl x)    NatCode ()
    rV G M T (SucEl v) Bot          NatCode le mem fmu src = tt
    rV G M T (SucEl v) UCode        NatCode ()
    rV G M T (SucEl v) (FunEl h)    NatCode ()
    rV G M T (SucEl v) (PiCode b f) NatCode ()
    rV G M T (SucEl v) NatCode      NatCode ()
    rV G M T (SucEl v) ZeroEl       NatCode ()
    rV G M T (SucEl v) (SucEl v') NatCode le mem fmu src =
      mkSigma (fst src)
        (restrictVal2-NatSuc n ih G M v v' le (sucNat-to v' mem) (sucNat-to v fmu) (snd src))
    -- a in {ZeroEl,SucEl}: target Vl u' a = Top
    rV G M T u u' ZeroEl    le mem fmu src = tt
    rV G M T u u' (SucEl x) le mem fmu src = tt
    ...
    rEV G M N T Bot NatCode      UCode ()
    rEV G M N T Bot ZeroEl       UCode ()
    rEV G M N T Bot (SucEl x)    UCode ()
    rEV G M N T UCode NatCode    UCode ()
    rEV G M N T UCode ZeroEl     UCode ()
    rEV G M N T UCode (SucEl x)  UCode ()
    rEV G M N T (FunEl g) NatCode    UCode ()
    rEV G M N T (FunEl g) ZeroEl     UCode ()
    rEV G M N T (FunEl g) (SucEl x)  UCode ()
    rEV G M N T (PiCode a' f') NatCode   UCode ()
    rEV G M N T (PiCode a' f') ZeroEl    UCode ()
    rEV G M N T (PiCode a' f') (SucEl x) UCode ()
    rEV G M N T NatCode Bot          UCode le mem fmu src = tt
    rEV G M N T NatCode UCode        UCode ()
    rEV G M N T NatCode (FunEl h)    UCode ()
    rEV G M N T NatCode (PiCode b f) UCode ()
    rEV G M N T NatCode NatCode      UCode le mem fmu src = src
    rEV G M N T NatCode ZeroEl       UCode ()
    rEV G M N T NatCode (SucEl x)    UCode ()
    rEV G M N T ZeroEl    u' UCode le mem ()
    rEV G M N T (SucEl x) u' UCode le mem ()
    ...
    -- a = NatCode (Nat eq-values): restrict value code u -> u'
    rEV G M N T Bot Bot          NatCode le mem fmu src = src
    rEV G M N T Bot UCode        NatCode ()
    rEV G M N T Bot (FunEl h)    NatCode ()
    rEV G M N T Bot (PiCode b f) NatCode ()
    rEV G M N T Bot NatCode      NatCode ()
    rEV G M N T Bot ZeroEl       NatCode ()
    rEV G M N T Bot (SucEl x)    NatCode ()
    rEV G M N T UCode        u' NatCode le mem ()
    rEV G M N T (FunEl g)    u' NatCode le mem ()
    rEV G M N T (PiCode b f) u' NatCode le mem ()
    rEV G M N T NatCode      u' NatCode le mem ()
    rEV G M N T ZeroEl Bot          NatCode le mem fmu src = tt
    rEV G M N T ZeroEl UCode        NatCode ()
    rEV G M N T ZeroEl (FunEl h)    NatCode ()
    rEV G M N T ZeroEl (PiCode b f) NatCode ()
    rEV G M N T ZeroEl NatCode      NatCode ()
    rEV G M N T ZeroEl ZeroEl       NatCode le mem fmu src = src
    rEV G M N T ZeroEl (SucEl x)    NatCode ()
    rEV G M N T (SucEl v) Bot          NatCode le mem fmu src = tt
    rEV G M N T (SucEl v) UCode        NatCode ()
    rEV G M N T (SucEl v) (FunEl h)    NatCode ()
    rEV G M N T (SucEl v) (PiCode b f) NatCode ()
    rEV G M N T (SucEl v) NatCode      NatCode ()
    rEV G M N T (SucEl v) ZeroEl       NatCode ()
    rEV G M N T (SucEl v) (SucEl v') NatCode le mem fmu src =
      mkSigma (fst src)
        (mkSigma (restrictVal2-NatSuc n ih G M v v' le (sucNat-to v' mem) (sucNat-to v fmu) (fst (snd src)))
          (mkSigma (restrictVal2-NatSuc n ih G N v v' le (sucNat-to v' mem) (sucNat-to v fmu) (fst (snd (snd src))))
            (restrictEqVal2-NatSuc n ih G M N v v' le (sucNat-to v' mem) (sucNat-to v fmu) (snd (snd (snd src))))))
    rEV G M N T u u' ZeroEl    le mem fmu src = tt
    rEV G M N T u u' (SucEl x) le mem fmu src = tt
    ...
    vfe1 Bot NatCode ev = tt
    vfe1 UCode NatCode ev = tt
    vfe1 (FunEl g) NatCode ev = tt
    vfe1 (PiCode a f) NatCode ev = tt
    vfe1 NatCode NatCode ev = tt
    vfe1 ZeroEl NatCode ev = mkSigma (fst ev) (fst (snd ev))
    vfe1 (SucEl v) NatCode ev = mkSigma (fst ev) (fst (snd ev))
    vfe1 NatCode UCode ev = mkSigma (fst ev) (fst (snd ev))
    vfe1 ZeroEl UCode ev = tt
    vfe1 (SucEl v) UCode ev = tt
    vfe1 NatCode (FunEl h) ev = tt
    vfe1 ZeroEl (FunEl h) ev = tt
    vfe1 (SucEl v) (FunEl h) ev = tt
    vfe1 NatCode (PiCode b f) ev = tt
    vfe1 ZeroEl (PiCode b f) ev = tt
    vfe1 (SucEl v) (PiCode b f) ev = tt
    vfe1 u ZeroEl ev = tt
    vfe1 u (SucEl x) ev = tt
    ...
    vfe2 Bot NatCode ev = tt
    vfe2 UCode NatCode ev = tt
    vfe2 (FunEl g) NatCode ev = tt
    vfe2 (PiCode a f) NatCode ev = tt
    vfe2 NatCode NatCode ev = tt
    vfe2 ZeroEl NatCode ev = mkSigma (fst ev) (snd (snd ev))
    vfe2 (SucEl v) NatCode ev = mkSigma (fst ev) (fst (snd (snd ev)))
    vfe2 NatCode UCode ev = mkSigma (fst ev) (fst (snd (snd ev)))
    vfe2 ZeroEl UCode ev = tt
    vfe2 (SucEl v) UCode ev = tt
    vfe2 NatCode (FunEl h) ev = tt
    vfe2 ZeroEl (FunEl h) ev = tt
    vfe2 (SucEl v) (FunEl h) ev = tt
    vfe2 NatCode (PiCode b f) ev = tt
    vfe2 ZeroEl (PiCode b f) ev = tt
    vfe2 (SucEl v) (PiCode b f) ev = tt
    vfe2 u ZeroEl ev = tt
    vfe2 u (SucEl x) ev = tt
