...
------------------------------------------------------------------------
-- ConvPiVariant.agda  (NAT/ — Pi + U fragment)
--
...
-- d1.  Both lemmas already live in NAT.Syntax.Substitution.
--
...
module NAT.Validity.ConvPiVariant where

open import NAT.Domain.Basic using (Nat ; suc ; fst ; snd)
open import NAT.Syntax.Raw using (Expr ; U ; Pi)
open import NAT.Syntax.Typing using (Ctx ; extend ; HasType ; ConvTm ;
  conv-Pi ; conv-sym)
open import NAT.Syntax.Substitution using (typing-ConvTm ; ctx-conv-HasType)
