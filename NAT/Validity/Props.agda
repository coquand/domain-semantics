{-# OPTIONS --without-K #-}
------------------------------------------------------------------------
-- ValidityProps.agda  (NAT/ — Pi + U fragment)
--
-- Stratified ports of ValidityFwd / ValiditySymTrans / ValiditySup, the
-- same combinator+induction way as NAT.Validity.Mono:
...
module NAT.Validity.Props where

open import NAT.Validity.Mono
open import NAT.Validity.Stratified using (Red3 ; mkRed3)
...
open RS using (Expr ; Var ; U ; Pi ; Lam ; App ; NatT ; Zero ; Suc ;
...
open import NAT.Syntax.Reduction using (Red ; mkRed ; Red-hr ; HeadRed ; headred-refl ; HeadRed-trans ;
  HeadRed-App ;
  HeadRed-strip-Pi ; HeadRed-unique-Suc )
...
open import NAT.Syntax.Substitution using (typing-ConvTm ; typing-type ; ctx-conv-ConvTm ; ctx-conv-HasType ;
  subst1-cong-ConvTm)
...
goodStageFwd (suc n) = record
  ...
  where
    ...
    Esym NatCode ca ev = mkSigma (snd ev) (fst ev)
    Esym ZeroEl ca ev = tt
    Esym (SucEl x) ca ev = tt
    ...
    Vfwd (FunEl g) (PiCode b0 f0) cb eqv val =
      ...
      in mkSigma vtyC' vpi'
    Vfwd NatCode UCode cb eqv val = mkSigma (snd eqv) (snd val)
    Vfwd ZeroEl UCode cb eqv val = tt
    Vfwd (SucEl v) UCode cb eqv val = tt
    Vfwd NatCode (PiCode b0 f0) cb eqv val = tt
    Vfwd ZeroEl (PiCode b0 f0) cb eqv val = tt
    Vfwd (SucEl v) (PiCode b0 f0) cb eqv val = tt
    Vfwd Bot NatCode cb eqv val = tt
    Vfwd UCode NatCode cb eqv val = tt
    Vfwd (FunEl g) NatCode cb eqv val = tt
    Vfwd (PiCode a' f') NatCode cb eqv val = tt
    Vfwd NatCode NatCode cb eqv val = tt
    Vfwd ZeroEl NatCode cb eqv val = mkSigma (snd eqv) (snd val)
    Vfwd (SucEl v) NatCode cb eqv val = mkSigma (snd eqv) (snd val)
    Vfwd u ZeroEl cb eqv val = tt
    Vfwd u (SucEl x) cb eqv val = tt
    ...
    EVfwd NatCode UCode cb eqv ev = mkSigma (snd eqv) (snd ev)
    EVfwd ZeroEl UCode cb eqv ev = tt
    EVfwd (SucEl v) UCode cb eqv ev = tt
    EVfwd NatCode (PiCode b0 f0) cb eqv ev = tt
    EVfwd ZeroEl (PiCode b0 f0) cb eqv ev = tt
    EVfwd (SucEl v) (PiCode b0 f0) cb eqv ev = tt
    EVfwd Bot NatCode cb eqv ev = tt
    EVfwd UCode NatCode cb eqv ev = tt
    EVfwd (FunEl g) NatCode cb eqv ev = tt
    EVfwd (PiCode a' f') NatCode cb eqv ev = tt
    EVfwd NatCode NatCode cb eqv ev = tt
    EVfwd ZeroEl NatCode cb eqv ev = mkSigma (snd eqv) (snd ev)
    EVfwd (SucEl v) NatCode cb eqv ev = mkSigma (snd eqv) (snd ev)
    EVfwd u ZeroEl cb eqv ev = tt
    EVfwd u (SucEl x) cb eqv ev = tt
...
goodStageSymTrans (suc n) = record
  ...
  where
    ...
    ETrans NatCode cu eqAB eqBC = mkSigma (fst eqAB) (snd eqBC)
    ETrans ZeroEl cu eqAB eqBC = tt
    ETrans (SucEl x) cu eqAB eqBC = tt
    ...
    EVsym NatCode UCode cu ca ev =
      mkSigma (fst ev) (mkSigma (fst (snd (snd ev))) (mkSigma (fst (snd ev)) (syms NatCode cu (snd (snd (snd ev))))))
    EVsym ZeroEl UCode cu ca ev = tt
    EVsym (SucEl v) UCode cu ca ev = tt
    EVsym NatCode (PiCode b f) cu ca ev = tt
    EVsym ZeroEl (PiCode b f) cu ca ev = tt
    EVsym (SucEl v) (PiCode b f) cu ca ev = tt
    EVsym Bot NatCode cu ca ev = tt
    EVsym UCode NatCode cu ca ev = tt
    EVsym (FunEl g) NatCode cu ca ev = tt
    EVsym (PiCode a' f') NatCode cu ca ev = tt
    EVsym NatCode NatCode cu ca ev = tt
    EVsym ZeroEl NatCode cu ca ev =
      mkSigma (fst ev) (mkSigma (snd (snd ev)) (fst (snd ev)))
    EVsym (SucEl v) NatCode cu ca ev =
      let core = snd (snd (snd ev))
      in mkSigma (fst ev) (mkSigma (fst (snd (snd ev))) (mkSigma (fst (snd ev))
           (record { predM = REqValNatSuc.predN core ; predN = REqValNatSuc.predM core
                   ; redM = REqValNatSuc.redN core ; redN = REqValNatSuc.redM core
                   ; htM = REqValNatSuc.htN core ; htN = REqValNatSuc.htM core
                   ; cvP = conv-sym (REqValNatSuc.cvP core)
                   ; eqP = evs v NatCode cu tt (REqValNatSuc.eqP core) })))
    EVsym u ZeroEl cu ca ev = tt
    EVsym u (SucEl x) cu ca ev = tt
    ...
    EVtrans NatCode UCode cu ca ev1 ev2 =
      mkSigma (fst ev1) (mkSigma (fst (snd ev1)) (mkSigma (fst (snd (snd ev2)))
        (ETrans NatCode cu (snd (snd (snd ev1))) (snd (snd (snd ev2))))))
    EVtrans ZeroEl UCode cu ca ev1 ev2 = tt
    EVtrans (SucEl v) UCode cu ca ev1 ev2 = tt
    EVtrans NatCode (PiCode b f) cu ca ev1 ev2 = tt
    EVtrans ZeroEl (PiCode b f) cu ca ev1 ev2 = tt
    EVtrans (SucEl v) (PiCode b f) cu ca ev1 ev2 = tt
    EVtrans Bot NatCode cu ca ev1 ev2 = tt
    EVtrans UCode NatCode cu ca ev1 ev2 = tt
    EVtrans (FunEl g) NatCode cu ca ev1 ev2 = tt
    EVtrans (PiCode a' f') NatCode cu ca ev1 ev2 = tt
    EVtrans NatCode NatCode cu ca ev1 ev2 = tt
    EVtrans ZeroEl NatCode cu ca ev1 ev2 =
      mkSigma (fst ev1) (mkSigma (fst (snd ev1)) (snd (snd ev2)))
    EVtrans (SucEl v) NatCode cu ca ev1 ev2 =
      let core1 = snd (snd (snd ev1))
          core2 = snd (snd (snd ev2))
          eqMid = HeadRed-unique-Suc (Red3.hr (REqValNatSuc.redN core1)) (Red3.hr (REqValNatSuc.redM core2))
          eqP2' = Eq-transport (\ X -> EVl n _ X (REqValNatSuc.predN core2) NatT v NatCode)
                    (Eq-sym eqMid) (REqValNatSuc.eqP core2)
      in mkSigma (fst ev1) (mkSigma (fst (snd ev1)) (mkSigma (fst (snd (snd ev2)))
           (record { predM = REqValNatSuc.predM core1 ; predN = REqValNatSuc.predN core2
                   ; redM = REqValNatSuc.redM core1 ; redN = REqValNatSuc.redN core2
                   ; htM = REqValNatSuc.htM core1 ; htN = REqValNatSuc.htN core2
                   ; cvP = conv-trans (REqValNatSuc.cvP core1)
                             (Eq-transport (\ X -> ConvTm _ X (REqValNatSuc.predN core2) NatT)
                               (Eq-sym eqMid) (REqValNatSuc.cvP core2))
                   ; eqP = evt v NatCode cu tt (REqValNatSuc.eqP core1) eqP2' })))
    EVtrans u ZeroEl cu ca ev1 ev2 = tt
    EVtrans u (SucEl x) cu ca ev1 ev2 = tt
...
goodStageSup (suc n) = record
  ...
  where
    ...
    VSup G T UCode NatCode ()
    VSup G T UCode ZeroEl ()
    VSup G T UCode (SucEl x) ()
    VSup G T (FunEl g) NatCode ()
    VSup G T (FunEl g) ZeroEl ()
    VSup G T (FunEl g) (SucEl x) ()
    VSup G T (PiCode b1 f1) NatCode ()
    VSup G T (PiCode b1 f1) ZeroEl ()
    VSup G T (PiCode b1 f1) (SucEl x) ()
    VSup G T NatCode Bot comp fm1 fm2 vt1 vt2 = vt1
    VSup G T NatCode UCode ()
    VSup G T NatCode (FunEl h) ()
    VSup G T NatCode (PiCode b h) ()
    VSup G T NatCode NatCode comp fm1 fm2 vt1 vt2 = vt1
    VSup G T NatCode ZeroEl ()
    VSup G T NatCode (SucEl x) ()
    VSup G T ZeroEl a2 comp ()
    VSup G T (SucEl x) a2 comp ()
    ...
    EVSup G M N UCode NatCode ()
    EVSup G M N UCode ZeroEl ()
    EVSup G M N UCode (SucEl x) ()
    EVSup G M N (FunEl g) NatCode ()
    EVSup G M N (FunEl g) ZeroEl ()
    EVSup G M N (FunEl g) (SucEl x) ()
    EVSup G M N (PiCode b1 f1) NatCode ()
    EVSup G M N (PiCode b1 f1) ZeroEl ()
    EVSup G M N (PiCode b1 f1) (SucEl x) ()
    EVSup G M N NatCode Bot comp fm1 fm2 eq1 eq2 = eq1
    EVSup G M N NatCode UCode ()
    EVSup G M N NatCode (FunEl h) ()
    EVSup G M N NatCode (PiCode b h) ()
    EVSup G M N NatCode NatCode comp fm1 fm2 eq1 eq2 = eq1
    EVSup G M N NatCode ZeroEl ()
    EVSup G M N NatCode (SucEl x) ()
    EVSup G M N ZeroEl u2 comp ()
    EVSup G M N (SucEl x) u2 comp ()
...
goodStageRefl (suc n) = record
  ...
  where
    ...
    V2E NatCode UCode v = mkSigma (fst v) (mkSigma (snd v) (mkSigma (snd v) (VT2E NatCode (snd v))))
    V2E ZeroEl UCode v = tt
    V2E (SucEl w) UCode v = tt
    V2E NatCode (FunEl h) v = tt
    V2E ZeroEl (FunEl h) v = tt
    V2E (SucEl w) (FunEl h) v = tt
    V2E NatCode (PiCode b f) v = tt
    V2E ZeroEl (PiCode b f) v = tt
    V2E (SucEl w) (PiCode b f) v = tt
    V2E Bot NatCode v = tt
    V2E UCode NatCode v = tt
    V2E (FunEl g) NatCode v = tt
    V2E (PiCode a f) NatCode v = tt
    V2E NatCode NatCode v = tt
    V2E ZeroEl NatCode v = mkSigma (fst v) (mkSigma (snd v) (snd v))
    V2E (SucEl w) NatCode (mkSigma vty rvns) =
      mkSigma vty (mkSigma rvns (mkSigma rvns
        (record { predM = RValNatSuc.pred rvns ; predN = RValNatSuc.pred rvns
                ; redM = RValNatSuc.red rvns ; redN = RValNatSuc.red rvns
                ; htM = RValNatSuc.htP rvns ; htN = RValNatSuc.htP rvns
                ; cvP = conv-refl (RValNatSuc.htP rvns)
                ; eqP = v2e w NatCode (RValNatSuc.valP rvns) })))
    V2E u ZeroEl v = tt
    V2E u (SucEl x) v = tt
...
goodStageTransport (suc n) = record { Val2-type-transport = VTT ; EqVal2-type-transport = EVTT }
  where
    ...
    VTT (FunEl g) (PiCode b f) eqvt val =
      ...
      in mkSigma vtyC' vpi'
    VTT NatCode UCode eqvt val = mkSigma (snd eqvt) (snd val)
    VTT ZeroEl UCode eqvt val = tt
    VTT (SucEl w) UCode eqvt val = tt
    VTT NatCode (PiCode b f) eqvt val = tt
    VTT ZeroEl (PiCode b f) eqvt val = tt
    VTT (SucEl w) (PiCode b f) eqvt val = tt
    VTT Bot NatCode eqvt val = tt
    VTT UCode NatCode eqvt val = tt
    VTT (FunEl g) NatCode eqvt val = tt
    VTT (PiCode a' f') NatCode eqvt val = tt
    VTT NatCode NatCode eqvt val = tt
    VTT ZeroEl NatCode eqvt val = mkSigma (snd eqvt) (snd val)
    VTT (SucEl w) NatCode eqvt val = mkSigma (snd eqvt) (snd val)
    VTT u ZeroEl eqvt val = tt
    VTT u (SucEl x) eqvt val = tt
    ...
    EVTT NatCode UCode eqvt ev = mkSigma (snd eqvt) (snd ev)
    EVTT ZeroEl UCode eqvt ev = tt
    EVTT (SucEl w) UCode eqvt ev = tt
    EVTT NatCode (PiCode b f) eqvt ev = tt
    EVTT ZeroEl (PiCode b f) eqvt ev = tt
    EVTT (SucEl w) (PiCode b f) eqvt ev = tt
    EVTT Bot NatCode eqvt ev = tt
    EVTT UCode NatCode eqvt ev = tt
    EVTT (FunEl g) NatCode eqvt ev = tt
    EVTT (PiCode a' f') NatCode eqvt ev = tt
    EVTT NatCode NatCode eqvt ev = tt
    EVTT ZeroEl NatCode eqvt ev = mkSigma (snd eqvt) (snd ev)
    EVTT (SucEl w) NatCode eqvt ev = mkSigma (snd eqvt) (snd ev)
    EVTT u ZeroEl eqvt ev = tt
    EVTT u (SucEl x) eqvt ev = tt
...
goodStageBeta (suc n) = record { Val2-beta-expand = Beta }
  where
    ...
    Beta NatCode UCode hr ct val =
      let vtA = fst val ; vtM' = snd val
          ctU = conv-conv ct (Red3.ct vtA) (typing-type (typing-type (fst (typing-ConvTm ct))))
          vtM = mkRed3 (HeadRed-trans hr (Red3.hr vtM')) (conv-trans ctU (Red3.ct vtM'))
      in mkSigma vtA (mkSigma vtM' (mkSigma vtM (mkSigma vtM' vtM)))
    Beta ZeroEl UCode hr ct val = tt
    Beta (SucEl w) UCode hr ct val = tt
    Beta NatCode (PiCode b f) hr ct val = tt
    Beta ZeroEl (PiCode b f) hr ct val = tt
    Beta (SucEl w) (PiCode b f) hr ct val = tt
    Beta Bot NatCode hr ct val = tt
    Beta UCode NatCode hr ct val = tt
    Beta (FunEl g) NatCode hr ct val = tt
    Beta (PiCode a' f') NatCode hr ct val = tt
    Beta NatCode NatCode hr ct val = tt
    Beta ZeroEl NatCode hr ct val =
      let vty = fst val
          red' = snd val
          ctNat = conv-conv ct (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          redM = mkRed3 (HeadRed-trans hr (Red3.hr red')) (conv-trans ctNat (Red3.ct red'))
      in mkSigma vty (mkSigma red' redM)
    Beta (SucEl w) NatCode hr ct val =
      let vty = fst val
          rvns = snd val
          ctNat = conv-conv ct (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          redM = mkRed3 (HeadRed-trans hr (Red3.hr (RValNatSuc.red rvns)))
                        (conv-trans ctNat (Red3.ct (RValNatSuc.red rvns)))
          rvnsM = record { pred = RValNatSuc.pred rvns ; red = redM
                         ; htP = RValNatSuc.htP rvns ; valP = RValNatSuc.valP rvns }
          coreEq = record { predM = RValNatSuc.pred rvns ; predN = RValNatSuc.pred rvns
                          ; redM = RValNatSuc.red rvns ; redN = redM
                          ; htM = RValNatSuc.htP rvns ; htN = RValNatSuc.htP rvns
                          ; cvP = conv-refl (RValNatSuc.htP rvns)
                          ; eqP = v2e w NatCode (RValNatSuc.valP rvns) }
      in mkSigma vty (mkSigma rvns (mkSigma rvnsM coreEq))
    Beta u ZeroEl hr ct val = tt
    Beta u (SucEl x) hr ct val = tt
