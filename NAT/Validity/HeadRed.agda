{-# OPTIONS --without-K #-}
------------------------------------------------------------------------
-- ValidityHeadRed.agda  (NAT/ — Pi + U fragment)
--
...
module NAT.Validity.HeadRed where

open import NAT.Validity.Mono
open import NAT.Validity.Props using (BetaPack ; goodStageBeta)
open import NAT.Validity.Stratified using (Red3 ; mkRed3 ; Val2 ; EqVal2)

import NAT.Domain.Basic as S
open S using (Nat ; zero ; suc ; Top ; tt ; Empty ; Pair ; mkSigma ; fst ; snd ;
              max ; FinEl ; Bot ; UCode ; FunEl ; PiCode ; NatCode ; ZeroEl ; SucEl ; FinFun)
open import NAT.Syntax.Raw using (Expr ; U ; Pi ; App ; NatT ; Zero ; Suc ; subst1)
open import NAT.Syntax.Typing using (Ctx ; extend ; HasType ; ConvTm ;
  conv-refl ; conv-sym ; conv-trans ; conv-conv ; conv-App-fun)
open import NAT.Syntax.Reduction using (HeadRed ; HeadRed1 ; HeadRed1-det ;
  headred-refl ; headred-step ; HeadRed-trans ; HeadRed-App ; HeadRed-strip-Pi ;
  HeadRed1-not-Zero ; HeadRed1-not-Suc)
open import NAT.Domain.Kernel using (EvalFun)
open import NAT.Syntax.Substitution using (typing-ConvTm ; typing-type ; subst1-cong-ConvTm)
...
HeadRed1-not-NatT : {n : Nat} {N : Expr n} -> HeadRed1 NatT N -> Empty
HeadRed1-not-NatT ()

HeadRed-strip-Nat : {n : Nat} {M M' : Expr n} ->
  HeadRed M M' -> HeadRed M NatT -> HeadRed M' NatT
HeadRed-strip-Nat headred-refl hr2 = hr2
HeadRed-strip-Nat (headred-step s1 hr1) headred-refl with HeadRed1-not-NatT s1
... | ()
HeadRed-strip-Nat (headred-step s1 hr1) (headred-step s2 hr2) =
  HeadRed-strip-Nat hr1
    (S.Eq-transport (\ x -> HeadRed x NatT) (S.Eq-sym (HeadRed1-det s1 s2)) hr2)

HeadRed-strip-Zero : {n : Nat} {M M' : Expr n} ->
  HeadRed M M' -> HeadRed M Zero -> HeadRed M' Zero
HeadRed-strip-Zero headred-refl hr2 = hr2
HeadRed-strip-Zero (headred-step s1 hr1) headred-refl with HeadRed1-not-Zero s1
... | ()
HeadRed-strip-Zero (headred-step s1 hr1) (headred-step s2 hr2) =
  HeadRed-strip-Zero hr1
    (S.Eq-transport (\ x -> HeadRed x Zero) (S.Eq-sym (HeadRed1-det s1 s2)) hr2)

HeadRed-strip-Suc : {n : Nat} {M M' p : Expr n} ->
  HeadRed M M' -> HeadRed M (Suc p) -> HeadRed M' (Suc p)
HeadRed-strip-Suc headred-refl hr2 = hr2
HeadRed-strip-Suc (headred-step s1 hr1) headred-refl with HeadRed1-not-Suc s1
... | ()
HeadRed-strip-Suc (headred-step s1 hr1) (headred-step s2 hr2) =
  HeadRed-strip-Suc hr1
    (S.Eq-transport (\ x -> HeadRed x (Suc _)) (S.Eq-sym (HeadRed1-det s1 s2)) hr2)
...
goodStageHeadRed (suc n) = record
  ...
  where
    ...
    ValTy2-headred-contract NatCode hr cv vt =
      mkRed3 (HeadRed-strip-Nat hr (Red3.hr vt)) (conv-trans (conv-sym cv) (Red3.ct vt))
    ValTy2-headred-contract ZeroEl hr cv vt = tt
    ValTy2-headred-contract (SucEl x) hr cv vt = tt
    ...
    ValTy2-headred-expand (PiCode b f) hr cv vt =
      record { domA = RValTyPi.domA vt ; codB = RValTyPi.codB vt
             ...
             ; valA = RValTyPi.valA vt
             ; edgeV = RValTyPi.edgeV vt ; edgeE = RValTyPi.edgeE vt }
    ValTy2-headred-expand NatCode hr cv vt =
      mkRed3 (HeadRed-trans hr (Red3.hr vt)) (conv-trans cv (Red3.ct vt))
    ValTy2-headred-expand ZeroEl hr cv vt = tt
    ValTy2-headred-expand (SucEl x) hr cv vt = tt
    ...
    EqValTy2-headred-contract NatCode hr1 hr2 cv1 cv2 eqvt =
      mkSigma (mkRed3 (HeadRed-strip-Nat hr1 (Red3.hr (fst eqvt))) (conv-trans (conv-sym cv1) (Red3.ct (fst eqvt))))
              (mkRed3 (HeadRed-strip-Nat hr2 (Red3.hr (snd eqvt))) (conv-trans (conv-sym cv2) (Red3.ct (snd eqvt))))
    EqValTy2-headred-contract ZeroEl hr1 hr2 cv1 cv2 eqvt = tt
    EqValTy2-headred-contract (SucEl x) hr1 hr2 cv1 cv2 eqvt = tt
      ...
      in mkSigma (ValTy2-headred-expand (PiCode b f) hr1 cv1 vt1)
           (mkSigma (ValTy2-headred-expand (PiCode b f) hr2 cv2 vt2)
             (record { domA = REqValTyPi.domA core ; codB = REqValTyPi.codB core
                     ...
                     ; convA = REqValTyPi.convA core ; convB = REqValTyPi.convB core
                     ; eqA = REqValTyPi.eqA core ; edgeET = REqValTyPi.edgeET core }))
    EqValTy2-headred-expand NatCode hr1 hr2 cv1 cv2 eqvt =
      mkSigma (mkRed3 (HeadRed-trans hr1 (Red3.hr (fst eqvt))) (conv-trans cv1 (Red3.ct (fst eqvt))))
              (mkRed3 (HeadRed-trans hr2 (Red3.hr (snd eqvt))) (conv-trans cv2 (Red3.ct (snd eqvt))))
    EqValTy2-headred-expand ZeroEl hr1 hr2 cv1 cv2 eqvt = tt
    EqValTy2-headred-expand (SucEl x) hr1 hr2 cv1 cv2 eqvt = tt
    ...
    Val2-headred-contract NatCode UCode hr cv val =
      let vtA = fst val
          ctU = conv-conv cv (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
      in mkSigma vtA (ValTy2-headred-contract NatCode hr ctU (snd val))
    Val2-headred-contract ZeroEl UCode hr cv tt = tt
    Val2-headred-contract (SucEl w) UCode hr cv tt = tt
    Val2-headred-contract NatCode (PiCode b f) hr cv tt = tt
    Val2-headred-contract ZeroEl (PiCode b f) hr cv tt = tt
    Val2-headred-contract (SucEl w) (PiCode b f) hr cv tt = tt
    Val2-headred-contract Bot NatCode hr cv tt = tt
    Val2-headred-contract UCode NatCode hr cv tt = tt
    Val2-headred-contract (FunEl g) NatCode hr cv tt = tt
    Val2-headred-contract (PiCode a' f') NatCode hr cv tt = tt
    Val2-headred-contract NatCode NatCode hr cv tt = tt
    Val2-headred-contract ZeroEl NatCode hr cv val =
      let vty = fst val
          ctNat = conv-conv cv (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          red' = mkRed3 (HeadRed-strip-Zero hr (Red3.hr (snd val))) (conv-trans (conv-sym ctNat) (Red3.ct (snd val)))
      in mkSigma vty red'
    Val2-headred-contract (SucEl w) NatCode hr cv val =
      let vty = fst val
          rvns = snd val
          ctNat = conv-conv cv (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          red' = mkRed3 (HeadRed-strip-Suc hr (Red3.hr (RValNatSuc.red rvns))) (conv-trans (conv-sym ctNat) (Red3.ct (RValNatSuc.red rvns)))
      in mkSigma vty (record { pred = RValNatSuc.pred rvns ; red = red'
                             ; htP = RValNatSuc.htP rvns ; valP = RValNatSuc.valP rvns })
    Val2-headred-contract u ZeroEl hr cv tt = tt
    Val2-headred-contract u (SucEl x) hr cv tt = tt
    ...
    EqVal2-headred-expand NatCode UCode hr1 hr2 cv1 cv2 ev =
      let vtA = fst ev
          ctU1 = conv-conv cv1 (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
          ctU2 = conv-conv cv2 (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
          expand1 = ValTy2-headred-expand NatCode hr1 ctU1 (fst (snd ev))
          expand2 = ValTy2-headred-expand NatCode hr2 ctU2 (fst (snd (snd ev)))
          eqexpand = EqValTy2-headred-expand NatCode hr1 hr2 ctU1 ctU2 (snd (snd (snd ev)))
      in mkSigma vtA (mkSigma expand1 (mkSigma expand2 eqexpand))
    EqVal2-headred-expand ZeroEl UCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (SucEl w) UCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand NatCode (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand ZeroEl (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (SucEl w) (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand Bot NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand UCode NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (FunEl g) NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (PiCode a' f') NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand NatCode NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand ZeroEl NatCode hr1 hr2 cv1 cv2 ev =
      let vty = fst ev
          ctNat1 = conv-conv cv1 (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          ctNat2 = conv-conv cv2 (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          redM' = mkRed3 (HeadRed-trans hr1 (Red3.hr (fst (snd ev)))) (conv-trans ctNat1 (Red3.ct (fst (snd ev))))
          redN' = mkRed3 (HeadRed-trans hr2 (Red3.hr (snd (snd ev)))) (conv-trans ctNat2 (Red3.ct (snd (snd ev))))
      in mkSigma vty (mkSigma redM' redN')
    EqVal2-headred-expand (SucEl w) NatCode hr1 hr2 cv1 cv2 ev =
      let vty = fst ev
          rvnsM = fst (snd ev) ; rvnsN = fst (snd (snd ev)) ; core = snd (snd (snd ev))
          ctNat1 = conv-conv cv1 (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          ctNat2 = conv-conv cv2 (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          redM'  = mkRed3 (HeadRed-trans hr1 (Red3.hr (RValNatSuc.red rvnsM))) (conv-trans ctNat1 (Red3.ct (RValNatSuc.red rvnsM)))
          redN'  = mkRed3 (HeadRed-trans hr2 (Red3.hr (RValNatSuc.red rvnsN))) (conv-trans ctNat2 (Red3.ct (RValNatSuc.red rvnsN)))
          rvnsM' = record { pred = RValNatSuc.pred rvnsM ; red = redM' ; htP = RValNatSuc.htP rvnsM ; valP = RValNatSuc.valP rvnsM }
          rvnsN' = record { pred = RValNatSuc.pred rvnsN ; red = redN' ; htP = RValNatSuc.htP rvnsN ; valP = RValNatSuc.valP rvnsN }
          redMc  = mkRed3 (HeadRed-trans hr1 (Red3.hr (REqValNatSuc.redM core))) (conv-trans ctNat1 (Red3.ct (REqValNatSuc.redM core)))
          redNc  = mkRed3 (HeadRed-trans hr2 (Red3.hr (REqValNatSuc.redN core))) (conv-trans ctNat2 (Red3.ct (REqValNatSuc.redN core)))
          core' = record { predM = REqValNatSuc.predM core ; predN = REqValNatSuc.predN core
                         ; redM = redMc ; redN = redNc
                         ; htM = REqValNatSuc.htM core ; htN = REqValNatSuc.htN core
                         ; cvP = REqValNatSuc.cvP core ; eqP = REqValNatSuc.eqP core }
      in mkSigma vty (mkSigma rvnsM' (mkSigma rvnsN' core'))
    EqVal2-headred-expand u ZeroEl hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand u (SucEl x) hr1 hr2 cv1 cv2 tt = tt
    ...
    EqVal2-headred-contract NatCode UCode hr1 hr2 cv1 cv2 ev =
      let vtA = fst ev
          ctU1 = conv-conv cv1 (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
          ctU2 = conv-conv cv2 (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
      in mkSigma vtA
           (mkSigma (ValTy2-headred-contract NatCode hr1 ctU1 (fst (snd ev)))
             (mkSigma (ValTy2-headred-contract NatCode hr2 ctU2 (fst (snd (snd ev))))
               (EqValTy2-headred-contract NatCode hr1 hr2 ctU1 ctU2 (snd (snd (snd ev))))))
    EqVal2-headred-contract ZeroEl UCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (SucEl w) UCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract NatCode (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract ZeroEl (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (SucEl w) (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract Bot NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract UCode NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (FunEl g) NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (PiCode a' f') NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract NatCode NatCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract ZeroEl NatCode hr1 hr2 cv1 cv2 ev =
      let vty = fst ev
          ctNat1 = conv-conv cv1 (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          ctNat2 = conv-conv cv2 (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          redM' = mkRed3 (HeadRed-strip-Zero hr1 (Red3.hr (fst (snd ev)))) (conv-trans (conv-sym ctNat1) (Red3.ct (fst (snd ev))))
          redN' = mkRed3 (HeadRed-strip-Zero hr2 (Red3.hr (snd (snd ev)))) (conv-trans (conv-sym ctNat2) (Red3.ct (snd (snd ev))))
      in mkSigma vty (mkSigma redM' redN')
    EqVal2-headred-contract (SucEl w) NatCode hr1 hr2 cv1 cv2 ev =
      let vty = fst ev
          rvnsM = fst (snd ev) ; rvnsN = fst (snd (snd ev)) ; core = snd (snd (snd ev))
          ctNat1 = conv-conv cv1 (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          ctNat2 = conv-conv cv2 (Red3.ct vty) (snd (typing-ConvTm (Red3.ct vty)))
          redM'  = mkRed3 (HeadRed-strip-Suc hr1 (Red3.hr (RValNatSuc.red rvnsM))) (conv-trans (conv-sym ctNat1) (Red3.ct (RValNatSuc.red rvnsM)))
          redN'  = mkRed3 (HeadRed-strip-Suc hr2 (Red3.hr (RValNatSuc.red rvnsN))) (conv-trans (conv-sym ctNat2) (Red3.ct (RValNatSuc.red rvnsN)))
          rvnsM' = record { pred = RValNatSuc.pred rvnsM ; red = redM' ; htP = RValNatSuc.htP rvnsM ; valP = RValNatSuc.valP rvnsM }
          rvnsN' = record { pred = RValNatSuc.pred rvnsN ; red = redN' ; htP = RValNatSuc.htP rvnsN ; valP = RValNatSuc.valP rvnsN }
          redMc  = mkRed3 (HeadRed-strip-Suc hr1 (Red3.hr (REqValNatSuc.redM core))) (conv-trans (conv-sym ctNat1) (Red3.ct (REqValNatSuc.redM core)))
          redNc  = mkRed3 (HeadRed-strip-Suc hr2 (Red3.hr (REqValNatSuc.redN core))) (conv-trans (conv-sym ctNat2) (Red3.ct (REqValNatSuc.redN core)))
          core' = record { predM = REqValNatSuc.predM core ; predN = REqValNatSuc.predN core
                         ; redM = redMc ; redN = redNc
                         ; htM = REqValNatSuc.htM core ; htN = REqValNatSuc.htN core
                         ; cvP = REqValNatSuc.cvP core ; eqP = REqValNatSuc.eqP core }
      in mkSigma vty (mkSigma rvnsM' (mkSigma rvnsN' core'))
    EqVal2-headred-contract u ZeroEl hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract u (SucEl x) hr1 hr2 cv1 cv2 tt = tt
