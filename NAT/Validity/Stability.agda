...
------------------------------------------------------------------------
-- ValidityStability.agda  (NAT/ — Pi + U fragment)
--
...
module NAT.Validity.Stability where
...
open import NAT.Syntax.Raw using (Expr ; U ; Pi ; App ; NatT ; subst1)
...
open import NAT.Domain.Rank using (RANK ; RANKFun ; RANK-cod ; RANK-EvalFun)
...
mutual
  ...
  -- RValNatSuc / REqValNatSuc transports (the SucEl value component at type
  -- NatCode); the predecessor validity is at code v (rank < RANK (SucEl v)),
  -- so they recurse into vlU/vlD/evlU/evlD at the SAME index j.
  rvnsU : (j : Nat) {n : Nat} (G : Ctx n) (M : Expr n) (v : FinEl) ->
    Le (suc (RANK (SucEl v))) (suc j) -> Le (suc (RANK NatCode)) (suc j) ->
    R.RValNatSuc j G M v -> R.RValNatSuc (suc j) G M v
  rvnsD : (j : Nat) {n : Nat} (G : Ctx n) (M : Expr n) (v : FinEl) ->
    Le (suc (RANK (SucEl v))) (suc j) -> Le (suc (RANK NatCode)) (suc j) ->
    R.RValNatSuc (suc j) G M v -> R.RValNatSuc j G M v
  reqvnsU : (j : Nat) {n : Nat} (G : Ctx n) (M N : Expr n) (v : FinEl) ->
    Le (suc (RANK (SucEl v))) (suc j) -> Le (suc (RANK NatCode)) (suc j) ->
    R.REqValNatSuc j G M N v -> R.REqValNatSuc (suc j) G M N v
  reqvnsD : (j : Nat) {n : Nat} (G : Ctx n) (M N : Expr n) (v : FinEl) ->
    Le (suc (RANK (SucEl v))) (suc j) -> Le (suc (RANK NatCode)) (suc j) ->
    R.REqValNatSuc (suc j) G M N v -> R.REqValNatSuc j G M N v
  ...
  vtyU (suc j) G M NatCode   bnd vt = vt
  vtyU (suc j) G M ZeroEl    bnd vt = vt
  vtyU (suc j) G M (SucEl x) bnd vt = vt
  ...
  vtyD (suc j) G M NatCode   bnd vt = vt
  vtyD (suc j) G M ZeroEl    bnd vt = vt
  vtyD (suc j) G M (SucEl x) bnd vt = vt
  ...
  evtyU (suc j) G M N NatCode   bnd vt = vt
  evtyU (suc j) G M N ZeroEl    bnd vt = vt
  evtyU (suc j) G M N (SucEl x) bnd vt = vt
  ...
  evtyD (suc j) G M N NatCode   bnd vt = vt
  evtyD (suc j) G M N ZeroEl    bnd vt = vt
  evtyD (suc j) G M N (SucEl x) bnd vt = vt
  ...
  vlU (suc j) G M A NatCode (PiCode b f)   bu ba vv = vv
  vlU (suc j) G M A ZeroEl (PiCode b f)    bu ba vv = vv
  vlU (suc j) G M A (SucEl w) (PiCode b f) bu ba vv = vv
  vlU (suc j) G M A NatCode UCode          bu ba vv = vv
  vlU (suc j) G M A ZeroEl UCode           bu ba vv = vv
  vlU (suc j) G M A (SucEl w) UCode        bu ba vv = vv
  -- NatCode type-code (Nat values)
  vlU (suc j) G M A Bot NatCode            bu ba vv = vv
  vlU (suc j) G M A UCode NatCode          bu ba vv = vv
  vlU (suc j) G M A (FunEl g) NatCode      bu ba vv = vv
  vlU (suc j) G M A (PiCode b f) NatCode   bu ba vv = vv
  vlU (suc j) G M A NatCode NatCode        bu ba vv = vv
  vlU (suc j) G M A ZeroEl NatCode         bu ba vv = vv
  vlU (suc j) G M A (SucEl v) NatCode      bu ba vv =
    mkSigma (fst vv) (rvnsU j G M v bu ba (snd vv))
  -- ZeroEl / SucEl type-codes (not types) -> Top
  vlU (suc j) G M A u ZeroEl               bu ba vv = vv
  vlU (suc j) G M A u (SucEl x)            bu ba vv = vv
  ...
  vlD (suc j) G M A NatCode (PiCode b f)   bu ba vv = vv
  vlD (suc j) G M A ZeroEl (PiCode b f)    bu ba vv = vv
  vlD (suc j) G M A (SucEl w) (PiCode b f) bu ba vv = vv
  vlD (suc j) G M A NatCode UCode          bu ba vv = vv
  vlD (suc j) G M A ZeroEl UCode           bu ba vv = vv
  vlD (suc j) G M A (SucEl w) UCode        bu ba vv = vv
  -- NatCode type-code (Nat values)
  vlD (suc j) G M A Bot NatCode            bu ba vv = vv
  vlD (suc j) G M A UCode NatCode          bu ba vv = vv
  vlD (suc j) G M A (FunEl g) NatCode      bu ba vv = vv
  vlD (suc j) G M A (PiCode b f) NatCode   bu ba vv = vv
  vlD (suc j) G M A NatCode NatCode        bu ba vv = vv
  vlD (suc j) G M A ZeroEl NatCode         bu ba vv = vv
  vlD (suc j) G M A (SucEl v) NatCode      bu ba vv =
    mkSigma (fst vv) (rvnsD j G M v bu ba (snd vv))
  -- ZeroEl / SucEl type-codes (not types) -> Top
  vlD (suc j) G M A u ZeroEl               bu ba vv = vv
  vlD (suc j) G M A u (SucEl x)            bu ba vv = vv
  ...
  evlU (suc j) G M N A NatCode (PiCode b f)   bu ba vv = vv
  evlU (suc j) G M N A ZeroEl (PiCode b f)    bu ba vv = vv
  evlU (suc j) G M N A (SucEl w) (PiCode b f) bu ba vv = vv
  evlU (suc j) G M N A NatCode UCode          bu ba vv = vv
  evlU (suc j) G M N A ZeroEl UCode           bu ba vv = vv
  evlU (suc j) G M N A (SucEl w) UCode        bu ba vv = vv
  -- NatCode type-code (Nat eq-values)
  evlU (suc j) G M N A Bot NatCode            bu ba vv = vv
  evlU (suc j) G M N A UCode NatCode          bu ba vv = vv
  evlU (suc j) G M N A (FunEl g) NatCode      bu ba vv = vv
  evlU (suc j) G M N A (PiCode b f) NatCode   bu ba vv = vv
  evlU (suc j) G M N A NatCode NatCode        bu ba vv = vv
  evlU (suc j) G M N A ZeroEl NatCode         bu ba vv = vv
  evlU (suc j) G M N A (SucEl v) NatCode      bu ba vv =
    mkSigma (fst vv)
      (mkSigma (rvnsU j G M v bu ba (fst (snd vv)))
        (mkSigma (rvnsU j G N v bu ba (fst (snd (snd vv))))
          (reqvnsU j G M N v bu ba (snd (snd (snd vv))))))
  -- ZeroEl / SucEl type-codes (not types) -> Top
  evlU (suc j) G M N A u ZeroEl               bu ba vv = vv
  evlU (suc j) G M N A u (SucEl x)            bu ba vv = vv
  ...
  evlD (suc j) G M N A NatCode (PiCode b f)   bu ba vv = vv
  evlD (suc j) G M N A ZeroEl (PiCode b f)    bu ba vv = vv
  evlD (suc j) G M N A (SucEl w) (PiCode b f) bu ba vv = vv
  evlD (suc j) G M N A NatCode UCode          bu ba vv = vv
  evlD (suc j) G M N A ZeroEl UCode           bu ba vv = vv
  evlD (suc j) G M N A (SucEl w) UCode        bu ba vv = vv
  -- NatCode type-code (Nat eq-values)
  evlD (suc j) G M N A Bot NatCode            bu ba vv = vv
  evlD (suc j) G M N A UCode NatCode          bu ba vv = vv
  evlD (suc j) G M N A (FunEl g) NatCode      bu ba vv = vv
  evlD (suc j) G M N A (PiCode b f) NatCode   bu ba vv = vv
  evlD (suc j) G M N A NatCode NatCode        bu ba vv = vv
  evlD (suc j) G M N A ZeroEl NatCode         bu ba vv = vv
  evlD (suc j) G M N A (SucEl v) NatCode      bu ba vv =
    mkSigma (fst vv)
      (mkSigma (rvnsD j G M v bu ba (fst (snd vv)))
        (mkSigma (rvnsD j G N v bu ba (fst (snd (snd vv))))
          (reqvnsD j G M N v bu ba (snd (snd (snd vv))))))
  -- ZeroEl / SucEl type-codes (not types) -> Top
  evlD (suc j) G M N A u ZeroEl               bu ba vv = vv
  evlD (suc j) G M N A u (SucEl x)            bu ba vv = vv
  ...
  ------------------------------------------------------------------
  -- RValNatSuc / REqValNatSuc transports.  bu : Le (suc (RANK (SucEl v)))
  -- (suc j) reduces definitionally to Le (suc (RANK v)) j (the predecessor
  -- value-code bound); the NatCode type bound Le 1 j follows by transitivity.
  ------------------------------------------------------------------
  rvnsU j G M v bu ba vp = record
    { pred = RValNatSuc.pred vp
    ; red  = RValNatSuc.red vp
    ; htP  = RValNatSuc.htP vp
    ; valP = vlU j G (RValNatSuc.pred vp) NatT v NatCode bu lnat (RValNatSuc.valP vp)
    }
    where
      open R j
      lnat : Le 1 j
      lnat = Le-trans 1 (suc (RANK v)) j tt bu

  rvnsD j G M v bu ba vp = record
    { pred = RValNatSuc.pred vp
    ; red  = RValNatSuc.red vp
    ; htP  = RValNatSuc.htP vp
    ; valP = vlD j G (RValNatSuc.pred vp) NatT v NatCode bu lnat (RValNatSuc.valP vp)
    }
    where
      open R (suc j)
      lnat : Le 1 j
  ...
  reqvnsU j G M N v bu ba vp = record
    ...
    ; eqP   = evlU j G (REqValNatSuc.predM vp) (REqValNatSuc.predN vp) NatT v NatCode bu lnat (REqValNatSuc.eqP vp)
    }
    where
      open R j
      lnat : Le 1 j
  ...
  reqvnsD j G M N v bu ba vp = record
    { predM = REqValNatSuc.predM vp
    ; predN = REqValNatSuc.predN vp
    ; redM  = REqValNatSuc.redM vp
    ; redN  = REqValNatSuc.redN vp
    ...
    ; htN   = REqValNatSuc.htN vp
    ; cvP   = REqValNatSuc.cvP vp
    ; eqP   = evlD j G (REqValNatSuc.predM vp) (REqValNatSuc.predN vp) NatT v NatCode bu lnat (REqValNatSuc.eqP vp)
    }
    where
      open R (suc j)
      lnat : Le 1 j
