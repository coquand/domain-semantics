...
------------------------------------------------------------------------
-- NAT/LemmaForTS.agda  (Pi + U fragment)
--
-- Provides:
...
module NAT.Model.SoundnessLemmas where
...
NotBot-from-Le u NatCode cu nbu le = tt
NotBot-from-Le u ZeroEl cu nbu le = tt
NotBot-from-Le u (SucEl v) cu nbu le = tt
...
NotBot-from-FinMem NatCode Bot nbu ()
NotBot-from-FinMem NatCode UCode nbu fm = tt
NotBot-from-FinMem NatCode (FunEl g) nbu ()
NotBot-from-FinMem NatCode (PiCode c h) nbu ()
NotBot-from-FinMem NatCode NatCode nbu ()
NotBot-from-FinMem NatCode ZeroEl nbu ()
NotBot-from-FinMem NatCode (SucEl v) nbu ()
NotBot-from-FinMem ZeroEl Bot nbu ()
NotBot-from-FinMem ZeroEl UCode nbu ()
NotBot-from-FinMem ZeroEl (FunEl g) nbu ()
NotBot-from-FinMem ZeroEl (PiCode c h) nbu ()
NotBot-from-FinMem ZeroEl NatCode nbu fm = tt
NotBot-from-FinMem ZeroEl ZeroEl nbu ()
NotBot-from-FinMem ZeroEl (SucEl v) nbu ()
NotBot-from-FinMem (SucEl u) Bot nbu ()
NotBot-from-FinMem (SucEl u) UCode nbu ()
NotBot-from-FinMem (SucEl u) (FunEl g) nbu ()
NotBot-from-FinMem (SucEl u) (PiCode c h) nbu ()
NotBot-from-FinMem (SucEl u) NatCode nbu fm = tt
NotBot-from-FinMem (SucEl u) ZeroEl nbu ()
NotBot-from-FinMem (SucEl u) (SucEl v) nbu ()
...
InvTyp-App A B M N rho fits invM = invTyp-App-aux
  where
    ...
    mkEvalApp NatCode nbv0 w evN evM-sing = mkSigma w (mkSigma evN evM-sing)
    mkEvalApp ZeroEl nbv0 w evN evM-sing = mkSigma w (mkSigma evN evM-sing)
    mkEvalApp (SucEl v0) nbv0 w evN evM-sing = mkSigma w (mkSigma evN evM-sing)
    ...
    invTyp-App-aux NatCode ev =
      let w = fst ev ; evN = fst (snd ev) ; evM-sing = snd (snd ev)
          cv = EvalRel-coh (App M N) rho NatCode ev
      in app-main w NatCode tt cv evN evM-sing
    invTyp-App-aux ZeroEl ev =
      let w = fst ev ; evN = fst (snd ev) ; evM-sing = snd (snd ev)
          cv = EvalRel-coh (App M N) rho ZeroEl ev
      in app-main w ZeroEl tt cv evN evM-sing
    invTyp-App-aux (SucEl v0) ev =
      let w = fst ev ; evN = fst (snd ev) ; evM-sing = snd (snd ev)
          cv = EvalRel-coh (App M N) rho (SucEl v0) ev
      in app-main w (SucEl v0) tt cv evN evM-sing
...
InvConv-beta {G = G} A B M N rho fits invN body-ih =
  ...
  where
    ...
    conv-app-to-sub NatCode ev = conv-app-to-sub-nonbot NatCode tt ev
    conv-app-to-sub ZeroEl ev = conv-app-to-sub-nonbot ZeroEl tt ev
    conv-app-to-sub (SucEl u) ev = conv-app-to-sub-nonbot (SucEl u) tt ev
    ...
    conv-sub-to-app (PiCode a0 f0) ev =
      ...
      in mkSigma (fst r) (mkSigma (fst (snd r)) (snd (snd r)))
    conv-sub-to-app NatCode ev =
      let r = mkLamEvidence NatCode tt ev
      in mkSigma (fst r) (mkSigma (fst (snd r)) (snd (snd r)))
    conv-sub-to-app ZeroEl ev =
      let r = mkLamEvidence ZeroEl tt ev
      in mkSigma (fst r) (mkSigma (fst (snd r)) (snd (snd r)))
    conv-sub-to-app (SucEl u) ev =
      let r = mkLamEvidence (SucEl u) tt ev
      in mkSigma (fst r) (mkSigma (fst (snd r)) (snd (snd r)))
    ...
    invTyp-app = InvTyp-App A B (Lam A M) N rho fits invLam
...
InvConv-funext {G = G} A B M N rho fits invM invN ih =
  ...
  where
    ...
    mkAppEvidence F x NatCode cx nbvi evF =
      mkSigma x (mkSigma (mkSigma cx (LeCode-refl x cx))
        (EvalRel-wk F rho x (FunEl (cons (mkSigma x NatCode) nil)) evF))
    mkAppEvidence F x ZeroEl cx nbvi evF =
      mkSigma x (mkSigma (mkSigma cx (LeCode-refl x cx))
        (EvalRel-wk F rho x (FunEl (cons (mkSigma x ZeroEl) nil)) evF))
    mkAppEvidence F x (SucEl vi) cx nbvi evF =
      mkSigma x (mkSigma (mkSigma cx (LeCode-refl x cx))
        (EvalRel-wk F rho x (FunEl (cons (mkSigma x (SucEl vi)) nil)) evF))
    ...
    decompAppEvidence F x NatCode nbvi ev =
      let w = fst ev ; evV0 = fst (snd ev) ; evWkF = snd (snd ev)
      in mkSigma w (mkSigma (snd evV0) (mkSigma (fst evV0)
           (EvalRel-unwk F rho x (FunEl (cons (mkSigma w NatCode) nil)) evWkF)))
    decompAppEvidence F x ZeroEl nbvi ev =
      let w = fst ev ; evV0 = fst (snd ev) ; evWkF = snd (snd ev)
      in mkSigma w (mkSigma (snd evV0) (mkSigma (fst evV0)
           (EvalRel-unwk F rho x (FunEl (cons (mkSigma w ZeroEl) nil)) evWkF)))
    decompAppEvidence F x (SucEl vi) nbvi ev =
      let w = fst ev ; evV0 = fst (snd ev) ; evWkF = snd (snd ev)
      in mkSigma w (mkSigma (snd evV0) (mkSigma (fst evV0)
           (EvalRel-unwk F rho x (FunEl (cons (mkSigma w (SucEl vi)) nil)) evWkF)))
    ...
    -- u' = NatCode/ZeroEl/SucEl are impossible function codes: absurd via
    -- the membership (FinMem u' piaf) or via EvalRel (Pi A B) at piaf.
    conv-main F F' convDir u NatCode Bot          cu le-u evF () evPi
    conv-main F F' convDir u NatCode UCode        cu le-u evF fm ()
    conv-main F F' convDir u NatCode (FunEl _)    cu le-u evF () evPi
    conv-main F F' convDir u NatCode (PiCode _ _) cu le-u evF () evPi
    conv-main F F' convDir u NatCode NatCode      cu le-u evF () evPi
    conv-main F F' convDir u NatCode ZeroEl       cu le-u evF () evPi
    conv-main F F' convDir u NatCode (SucEl _)    cu le-u evF () evPi
    conv-main F F' convDir u ZeroEl Bot          cu le-u evF () evPi
    conv-main F F' convDir u ZeroEl UCode        cu le-u evF () evPi
    conv-main F F' convDir u ZeroEl (FunEl _)    cu le-u evF () evPi
    conv-main F F' convDir u ZeroEl (PiCode _ _) cu le-u evF () evPi
    conv-main F F' convDir u ZeroEl NatCode      cu le-u evF fm ()
    conv-main F F' convDir u ZeroEl ZeroEl       cu le-u evF () evPi
    conv-main F F' convDir u ZeroEl (SucEl _)    cu le-u evF () evPi
    conv-main F F' convDir u (SucEl u') Bot          cu le-u evF () evPi
    conv-main F F' convDir u (SucEl u') UCode        cu le-u evF () evPi
    conv-main F F' convDir u (SucEl u') (FunEl _)    cu le-u evF () evPi
    conv-main F F' convDir u (SucEl u') (PiCode _ _) cu le-u evF () evPi
    conv-main F F' convDir u (SucEl u') NatCode      cu le-u evF fm ()
    conv-main F F' convDir u (SucEl u') ZeroEl       cu le-u evF () evPi
    conv-main F F' convDir u (SucEl u') (SucEl _)    cu le-u evF () evPi
    ...
    conv-one-dir F F' invF convDir NatCode ev =
      conv-nonbot F F' invF convDir NatCode tt ev
    conv-one-dir F F' invF convDir ZeroEl ev =
      conv-nonbot F F' invF convDir ZeroEl tt ev
    conv-one-dir F F' invF convDir (SucEl u) ev =
      conv-nonbot F F' invF convDir (SucEl u) tt ev
...
InvConv-App-fun A B f f' a rho fits invFF' invA =
  let invF  = fst invFF'
      ...
      invLHS = InvTyp-App A B f  a rho fits invF
      invRHS = InvTyp-App A B f' a rho fits invF'
  ...
  where
    ...
    conv-fwd eqF NatCode (mkSigma v (mkSigma eva evf)) =
      mkSigma v (mkSigma eva (eqF (FunEl (cons (mkSigma v NatCode) nil)) evf))
    conv-fwd eqF ZeroEl (mkSigma v (mkSigma eva evf)) =
      mkSigma v (mkSigma eva (eqF (FunEl (cons (mkSigma v ZeroEl) nil)) evf))
    conv-fwd eqF (SucEl u) (mkSigma v (mkSigma eva evf)) =
      mkSigma v (mkSigma eva (eqF (FunEl (cons (mkSigma v (SucEl u)) nil)) evf))
...
InvConv-App-arg {G = G} A B f a a' rho fits invF invAA' =
  ...
  where
    ...
    conv-fwd (PiCode a0 f0) (mkSigma v (mkSigma eva evf)) =
      mkSigma v (mkSigma (fwdA v eva) evf)
    conv-fwd NatCode (mkSigma v (mkSigma eva evf)) =
      mkSigma v (mkSigma (fwdA v eva) evf)
    conv-fwd ZeroEl (mkSigma v (mkSigma eva evf)) =
      mkSigma v (mkSigma (fwdA v eva) evf)
    conv-fwd (SucEl u) (mkSigma v (mkSigma eva evf)) =
      mkSigma v (mkSigma (fwdA v eva) evf)
    ...
    conv-bwd (PiCode a0 f0) (mkSigma v (mkSigma eva' evf)) =
      mkSigma v (mkSigma (bwdA v eva') evf)
    conv-bwd NatCode (mkSigma v (mkSigma eva' evf)) =
      mkSigma v (mkSigma (bwdA v eva') evf)
    conv-bwd ZeroEl (mkSigma v (mkSigma eva' evf)) =
      mkSigma v (mkSigma (bwdA v eva') evf)
    conv-bwd (SucEl u) (mkSigma v (mkSigma eva' evf)) =
      mkSigma v (mkSigma (bwdA v eva') evf)
...
------------------------------------------------------------------------
-- Nat eliminator soundness lemmas
------------------------------------------------------------------------

-- InvTyp for the Nat type and its constructors
InvTyp-NatT : {n : Nat} {G : Ctx n} (rho : EnvApprox n) -> InvTyp G NatT U rho
InvTyp-NatT rho u ev =
  mkSigma NatCode (mkSigma UCode
    (mkSigma (snd ev) (mkSigma (mkSigma tt tt) (mkSigma tt (mkSigma tt tt)))))

InvTyp-Zero : {n : Nat} {G : Ctx n} (rho : EnvApprox n) -> InvTyp G Zero NatT rho
InvTyp-Zero rho u ev =
  mkSigma ZeroEl (mkSigma NatCode
    (mkSigma (snd ev) (mkSigma (mkSigma tt tt) (mkSigma tt (mkSigma tt tt)))))

InvTyp-Suc : {n : Nat} {G : Ctx n} (m : Expr n) (rho : EnvApprox n) ->
  InvTyp G m NatT rho -> InvTyp G (Suc m) NatT rho
InvTyp-Suc m rho invm u ev =
  let cu  = fst ev
      v   = fst (snd ev)
      evm = fst (snd (snd ev))
      le  = snd (snd (snd ev))                       -- LeCode u (SucEl v)
      cv  = EvalRel-coh m rho v evm
      mkSigma v' (mkSigma a'' (mkSigma le-vv' (mkSigma evm' (mkSigma fm' evNatA)))) =
        invm v evm
      cv'        = FinMem-coh-u v' a'' fm'
      ca''       = fst evNatA
      le-a''-Nat = snd evNatA
      le-u-Sv'   = LeCode-trans u (SucEl v) (SucEl v') cu cv cv' le le-vv'
      fmv'-Nat   = finMem-upward v' a'' NatCode le-a''-Nat ca'' tt fm' tt
      fm-Sv'-Nat = sucNat-from v' fmv'-Nat
  in mkSigma (SucEl v') (mkSigma NatCode
       (mkSigma le-u-Sv'
         (mkSigma (mkSigma cv' (mkSigma v' (mkSigma evm' (LeCode-refl v' cv'))))
           (mkSigma fm-Sv'-Nat (mkSigma tt tt)))))

-- Suc congruence on evaluation (used by convSound' conv-Suc)
EvalRel-Suc-map : {n : Nat} (m m' : Expr n) (rho : EnvApprox n) ->
  ((x : FinEl) -> EvalRel m rho x -> EvalRel m' rho x) ->
  (x : FinEl) -> EvalRel (Suc m) rho x -> EvalRel (Suc m') rho x
EvalRel-Suc-map m m' rho f x ev =
  mkSigma (fst ev)
    (mkSigma (fst (snd ev))
      (mkSigma (f (fst (snd ev)) (fst (snd (snd ev)))) (snd (snd (snd ev)))))

-- CaseBranch congruence (used by InvConv-Case)
CaseBranch-map : {n : Nat} (a b a' b' : Expr n) (rho : EnvApprox n) (u w : FinEl) ->
  ((x : FinEl) -> EvalRel a rho x -> EvalRel a' rho x) ->
  ((x : FinEl) -> EvalRel b rho x -> EvalRel b' rho x) ->
  CaseBranch a b rho u w -> CaseBranch a' b' rho u w
CaseBranch-map a b a' b' rho u Bot          fa fb cb = cb
CaseBranch-map a b a' b' rho u UCode        fa fb ()
CaseBranch-map a b a' b' rho u (FunEl g)    fa fb ()
CaseBranch-map a b a' b' rho u (PiCode d f) fa fb ()
CaseBranch-map a b a' b' rho u NatCode      fa fb ()
CaseBranch-map a b a' b' rho u ZeroEl       fa fb cb = fa u cb
CaseBranch-map a b a' b' rho u (SucEl v)    fa fb cb =
  fb (FunEl (cons (mkSigma v u) nil)) cb

-- Build EvalRel (App M N) at a non-Bot result code from the singleton graph.
-- (Top-level version of the helper local to InvTyp-App.)
mkEvalApp : {n : Nat} (M N : Expr n) (rho : EnvApprox n) ->
  (v0 : FinEl) -> NotBot v0 ->
  (w : FinEl) -> EvalRel N rho w ->
  EvalRel M rho (FunEl (cons (mkSigma w v0) nil)) ->
  EvalRel (App M N) rho v0
mkEvalApp M N rho Bot          () w evN evM-sing
mkEvalApp M N rho UCode        nbv0 w evN evM-sing = mkSigma w (mkSigma evN evM-sing)
mkEvalApp M N rho (FunEl g)    nbv0 w evN evM-sing = mkSigma w (mkSigma evN evM-sing)
mkEvalApp M N rho (PiCode a0 f0) nbv0 w evN evM-sing = mkSigma w (mkSigma evN evM-sing)
mkEvalApp M N rho NatCode      nbv0 w evN evM-sing = mkSigma w (mkSigma evN evM-sing)
mkEvalApp M N rho ZeroEl       nbv0 w evN evM-sing = mkSigma w (mkSigma evN evM-sing)
mkEvalApp M N rho (SucEl v0)   nbv0 w evN evM-sing = mkSigma w (mkSigma evN evM-sing)

------------------------------------------------------------------------
-- InvTyp-Case : model soundness of caseNat (non-dependent motive)
------------------------------------------------------------------------

InvTyp-Case : {n : Nat} {G : Ctx n} (C M a b : Expr n) (rho : EnvApprox n) ->
  Fits G rho ->
  InvTyp G C U rho ->
  InvTyp G M NatT rho ->
  InvTyp G a C rho ->
  InvTyp G b (Pi NatT (wkExpr C)) rho ->
  InvTyp G (Case M a b) C rho
InvTyp-Case {G = G} C M a b rho fits invC invM inva invb = case-aux
  where
    crho = Fits-CoherentEnv rho fits

    -- the SucEl branch: b (a function value) applied to the predecessor p,
    -- mirroring InvTyp-App's app-h-case but with non-dependent codomain wkExpr C
    -- (so EvalRel-unwk replaces EvalRel-subst1-backward) and a Case re-embedding.
    suc-h-case :
      ...
      EvalRel b rho h ->
      FinMem h piaf ->
      EvalRel (Pi NatT (wkExpr C)) rho piaf ->
      Typed (Case M a b) C rho u
    ...
    suc-h-case p u nbu cu cp evM evb-sing (FunEl g') (PiCode a0 f0) le-g' evb-g' fm-g' evPi =
      let fmFun-g' = finMem-funel-fun g' a0 f0 fm-g'
          ...
          le-v-ef  = fst le-g'                       -- LeCode u (EvalFun g' p)
          ctg'     = cft-from-cf g' cf-g'
          sb       = selectionBelow g' p ctg' cp
          u0       = fst sb
          v0       = fst (snd sb)
          sel-g'   = fst (snd (snd sb))
          le-u0-w  = fst (snd (snd (snd sb)))        -- LeCode u0 p
          eq-v0    = snd (snd (snd (snd sb)))        -- Eq v0 (EvalFun g' p)
          le-v-v0  : LeCode u v0
          ...
          fm-z-a'  = fst (snd (snd sb-result))
          evB-z-wf = snd (snd (snd sb-result))
          evB-z-fu0 : EvalRel (wkExpr C) (extendEnv rho z) fu0
          evB-z-fu0 = Eq-transport (\ x -> EvalRel (wkExpr C) (extendEnv rho z) x)
                        (Eq-sym eq-wf) evB-z-wf
          evC-fu0  = EvalRel-unwk C rho z fu0 evB-z-fu0
    ...
    case-aux : (u : FinEl) -> EvalRel (Case M a b) rho u -> Typed (Case M a b) C rho u
    case-aux u ev = branch u (fst ev) (fst (snd ev)) (snd (snd ev))

------------------------------------------------------------------------
-- InvConv-case-zero / InvConv-case-suc / InvConv-Case
------------------------------------------------------------------------

InvConv-case-zero : {n : Nat} {G : Ctx n} (C a b : Expr n) (rho : EnvApprox n) ->
  Fits G rho ->
  InvTyp G C U rho ->
  InvTyp G a C rho ->
  InvTyp G b (Pi NatT (wkExpr C)) rho ->
  InvConv G (Case Zero a b) a C rho
InvConv-case-zero {G = G} C a b rho fits invC inva invb =
  mkSigma (InvTyp-Case C Zero a b rho fits invC (InvTyp-Zero {G = G} rho) inva invb)
...
InvConv-case-suc : {n : Nat} {G : Ctx n} (C m a b : Expr n) (rho : EnvApprox n) ->
  Fits G rho ->
  InvTyp G C U rho ->
  InvTyp G m NatT rho ->
  InvTyp G a C rho ->
  InvTyp G b (Pi NatT (wkExpr C)) rho ->
  InvConv G (Case (Suc m) a b) (App b m) C rho
InvConv-case-suc {G = G} C m a b rho fits invC invm inva invb =
  mkSigma (InvTyp-Case C (Suc m) a b rho fits invC (InvTyp-Suc {G = G} m rho invm) inva invb)
    (mkSigma invApp (mkSigma fwd bwd))
  where
    crho = Fits-CoherentEnv rho fits
    invApp : InvTyp G (App b m) C rho
    invApp = Eq-transport (\ T -> InvTyp G (App b m) T rho) (subst1-wk C m)
               (InvTyp-App NatT (wkExpr C) b m rho fits invb)
...
InvConv-Case : {n : Nat} {G : Ctx n} (C M M' a a' b b' : Expr n) (rho : EnvApprox n) ->
  Fits G rho ->
  InvTyp G C U rho ->
  InvConv G M M' NatT rho ->
  InvConv G a a' C rho ->
  InvConv G b b' (Pi NatT (wkExpr C)) rho ->
  InvConv G (Case M a b) (Case M' a' b') C rho
InvConv-Case {G = G} C M M' a a' b b' rho fits invC icM icA icB =
