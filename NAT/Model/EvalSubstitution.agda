...
module NAT.Model.EvalSubstitution where

import NAT.Domain.Basic as S
open S using (Nat ; zero ; suc ; Top ; tt ; Empty ; Pair ; mkSigma ; fst ; snd ;
              Sigma ; Eq ; refl ; Eq-transport ; Eq-sym ;
              FinEl ; Bot ; UCode ; FunEl ; PiCode ; NatCode ; ZeroEl ; SucEl ; FinFun ; isPos)
open import NAT.Domain.Kernel using (LeCode ; LeCode-refl ; LeCode-trans ;
...
open import NAT.Model.Selection using (Edge ; EdgeIn ; here ; there ; Selection ;
...
open import NAT.Model.Eval using (CaseBranch ; CaseBranch-mon-env)

CaseBranch-ren : {n m : Nat} (r : Ren n m)
  (a b : Expr n) (rho : EnvApprox n) (rho' : EnvApprox m) ->
  RenAgree r rho rho' -> (c w : FinEl) ->
  CaseBranch a b rho c w -> CaseBranch (renExpr r a) (renExpr r b) rho' c w
CaseBranch-ren r a b rho rho' eq c Bot          cb = cb
CaseBranch-ren r a b rho rho' eq c UCode        ()
CaseBranch-ren r a b rho rho' eq c (FunEl g)    ()
CaseBranch-ren r a b rho rho' eq c (PiCode d f) ()
CaseBranch-ren r a b rho rho' eq c NatCode      ()
CaseBranch-ren r a b rho rho' eq c ZeroEl       cb =
  EvalRel-ren r a rho rho' eq c cb
CaseBranch-ren r a b rho rho' eq c (SucEl v)    cb =
  EvalRel-ren r b rho rho' eq (FunEl (cons (mkSigma v c) nil)) cb
...
-- Y
EvalRel-ren r (Y g) rho rho' eq u ev =
  mkSigma (fst ev)
    (Approx-mon (\ p w -> EvalRel g rho (FunEl (cons (mkSigma p w) nil))) (\ p w -> EvalRel (renExpr r g) rho' (FunEl (cons (mkSigma p w) nil)))
       (\ p w sw -> EvalRel-ren r g rho rho' eq (FunEl (cons (mkSigma p w) nil)) sw)
       (fst ev) u (snd ev))

-- App at Nat-value result codes
EvalRel-ren r (App M N) rho rho' eq NatCode
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren r N rho rho' eq v evN)
                      (EvalRel-ren r M rho rho' eq _ evM))
EvalRel-ren r (App M N) rho rho' eq ZeroEl
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren r N rho rho' eq v evN)
                      (EvalRel-ren r M rho rho' eq _ evM))
EvalRel-ren r (App M N) rho rho' eq (SucEl w)
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren r N rho rho' eq v evN)
                      (EvalRel-ren r M rho rho' eq _ evM))

-- Nat / Zero / Suc / Case
EvalRel-ren r NatT rho rho' eq u ev = ev
EvalRel-ren r Zero rho rho' eq u ev = ev
EvalRel-ren r (Suc m) rho rho' eq u (mkSigma cu (mkSigma v (mkSigma evm le))) =
  mkSigma cu (mkSigma v (mkSigma (EvalRel-ren r m rho rho' eq v evm) le))
EvalRel-ren r (Case M a b) rho rho' eq c (mkSigma w (mkSigma evM cb)) =
  mkSigma w (mkSigma (EvalRel-ren r M rho rho' eq w evM)
                     (CaseBranch-ren r a b rho rho' eq c w cb))
...
CaseBranch-ren-inv : {n m : Nat} (r : Ren n m)
  (a b : Expr n) (rho : EnvApprox n) (rho' : EnvApprox m) ->
  RenAgree r rho rho' -> (c w : FinEl) ->
  CaseBranch (renExpr r a) (renExpr r b) rho' c w -> CaseBranch a b rho c w
CaseBranch-ren-inv r a b rho rho' eq c Bot          cb = cb
CaseBranch-ren-inv r a b rho rho' eq c UCode        ()
CaseBranch-ren-inv r a b rho rho' eq c (FunEl g)    ()
CaseBranch-ren-inv r a b rho rho' eq c (PiCode d f) ()
CaseBranch-ren-inv r a b rho rho' eq c NatCode      ()
CaseBranch-ren-inv r a b rho rho' eq c ZeroEl       cb =
  EvalRel-ren-inv r a rho rho' eq c cb
CaseBranch-ren-inv r a b rho rho' eq c (SucEl v)    cb =
  EvalRel-ren-inv r b rho rho' eq (FunEl (cons (mkSigma v c) nil)) cb
...
EvalRel-ren-inv r (Y g) rho rho' eq u ev =
  mkSigma (fst ev)
    (Approx-mon (\ p w -> EvalRel (renExpr r g) rho' (FunEl (cons (mkSigma p w) nil))) (\ p w -> EvalRel g rho (FunEl (cons (mkSigma p w) nil)))
       (\ p w sw -> EvalRel-ren-inv r g rho rho' eq (FunEl (cons (mkSigma p w) nil)) sw)
       (fst ev) u (snd ev))

-- App at Nat-value result codes
EvalRel-ren-inv r (App M N) rho rho' eq NatCode
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren-inv r N rho rho' eq v evN)
                      (EvalRel-ren-inv r M rho rho' eq _ evM))
EvalRel-ren-inv r (App M N) rho rho' eq ZeroEl
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren-inv r N rho rho' eq v evN)
                      (EvalRel-ren-inv r M rho rho' eq _ evM))
EvalRel-ren-inv r (App M N) rho rho' eq (SucEl w)
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren-inv r N rho rho' eq v evN)
                      (EvalRel-ren-inv r M rho rho' eq _ evM))

-- Nat / Zero / Suc / Case
EvalRel-ren-inv r NatT rho rho' eq u ev = ev
EvalRel-ren-inv r Zero rho rho' eq u ev = ev
EvalRel-ren-inv r (Suc m) rho rho' eq u (mkSigma cu (mkSigma v (mkSigma evm le))) =
  mkSigma cu (mkSigma v (mkSigma (EvalRel-ren-inv r m rho rho' eq v evm) le))
EvalRel-ren-inv r (Case M a b) rho rho' eq c (mkSigma w (mkSigma evM cb)) =
  mkSigma w (mkSigma (EvalRel-ren-inv r M rho rho' eq w evM)
                     (CaseBranch-ren-inv r a b rho rho' eq c w cb))
...
CaseBranch-subst : {h g : Nat} (sigma : Sub h g)
  (a b : Expr g) (rho : EnvApprox h) (rho' : EnvApprox g) ->
  CoherentEnv rho -> SubRel sigma rho rho' -> (c w : FinEl) ->
  CaseBranch a b rho' c w ->
  CaseBranch (substExpr sigma a) (substExpr sigma b) rho c w
CaseBranch-subst sigma a b rho rho' crho sr c Bot          cb = cb
CaseBranch-subst sigma a b rho rho' crho sr c UCode        ()
CaseBranch-subst sigma a b rho rho' crho sr c (FunEl g)    ()
CaseBranch-subst sigma a b rho rho' crho sr c (PiCode d f) ()
CaseBranch-subst sigma a b rho rho' crho sr c NatCode      ()
CaseBranch-subst sigma a b rho rho' crho sr c ZeroEl       cb =
  EvalRel-subst sigma a rho rho' crho sr c cb
CaseBranch-subst sigma a b rho rho' crho sr c (SucEl v)    cb =
  EvalRel-subst sigma b rho rho' crho sr (FunEl (cons (mkSigma v c) nil)) cb
...
-- Y
EvalRel-subst sigma (Y g) rho rho' crho sr u ev =
  mkSigma (fst ev)
    (Approx-mon (\ p w -> EvalRel g rho' (FunEl (cons (mkSigma p w) nil))) (\ p w -> EvalRel (substExpr sigma g) rho (FunEl (cons (mkSigma p w) nil)))
       (\ p w sw -> EvalRel-subst sigma g rho rho' crho sr (FunEl (cons (mkSigma p w) nil)) sw)
       (fst ev) u (snd ev))

-- App at Nat-value result codes
EvalRel-subst sigma (App M N) rho rho' crho sr NatCode
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst sigma N rho rho' crho sr v evN)
                      (EvalRel-subst sigma M rho rho' crho sr _ evM))
EvalRel-subst sigma (App M N) rho rho' crho sr ZeroEl
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst sigma N rho rho' crho sr v evN)
                      (EvalRel-subst sigma M rho rho' crho sr _ evM))
EvalRel-subst sigma (App M N) rho rho' crho sr (SucEl w)
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst sigma N rho rho' crho sr v evN)
                      (EvalRel-subst sigma M rho rho' crho sr _ evM))

-- Nat / Zero / Suc / Case
EvalRel-subst sigma NatT rho rho' crho sr u ev = ev
EvalRel-subst sigma Zero rho rho' crho sr u ev = ev
EvalRel-subst sigma (Suc m) rho rho' crho sr u
  (mkSigma cu (mkSigma v (mkSigma evm le))) =
  mkSigma cu (mkSigma v (mkSigma (EvalRel-subst sigma m rho rho' crho sr v evm) le))
EvalRel-subst sigma (Case M a b) rho rho' crho sr c
  (mkSigma w (mkSigma evM cb)) =
  mkSigma w (mkSigma (EvalRel-subst sigma M rho rho' crho sr w evM)
                     (CaseBranch-subst sigma a b rho rho' crho sr c w cb))
...
CaseBranch-subst-forward-max : {h g : Nat} (sigma : Sub h g)
  (a b : Expr g) (rho : EnvApprox h) (rho' : EnvApprox g) ->
  CoherentEnv rho -> CoherentEnv rho' -> MaxSubRel sigma rho rho' ->
  (c w : FinEl) ->
  CaseBranch (substExpr sigma a) (substExpr sigma b) rho c w ->
  CaseBranch a b rho' c w
CaseBranch-subst-forward-max sigma a b rho rho' crho crho' msr c Bot          cb = cb
CaseBranch-subst-forward-max sigma a b rho rho' crho crho' msr c UCode        ()
CaseBranch-subst-forward-max sigma a b rho rho' crho crho' msr c (FunEl g)    ()
CaseBranch-subst-forward-max sigma a b rho rho' crho crho' msr c (PiCode d f) ()
CaseBranch-subst-forward-max sigma a b rho rho' crho crho' msr c NatCode      ()
CaseBranch-subst-forward-max sigma a b rho rho' crho crho' msr c ZeroEl       cb =
  EvalRel-subst-forward-max sigma a rho rho' crho crho' msr c cb
CaseBranch-subst-forward-max sigma a b rho rho' crho crho' msr c (SucEl v)    cb =
  EvalRel-subst-forward-max sigma b rho rho' crho crho' msr (FunEl (cons (mkSigma v c) nil)) cb
...
-- Y
EvalRel-subst-forward-max sigma (Y g) rho rho' crho crho' msr u ev =
  mkSigma (fst ev)
    (Approx-mon (\ p w -> EvalRel (substExpr sigma g) rho (FunEl (cons (mkSigma p w) nil))) (\ p w -> EvalRel g rho' (FunEl (cons (mkSigma p w) nil)))
       (\ p w sw -> EvalRel-subst-forward-max sigma g rho rho' crho crho' msr (FunEl (cons (mkSigma p w) nil)) sw)
       (fst ev) u (snd ev))

-- App at Nat-value result codes
EvalRel-subst-forward-max sigma (App M N) rho rho' crho crho' msr NatCode
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst-forward-max sigma N rho rho' crho crho' msr v evN)
                      (EvalRel-subst-forward-max sigma M rho rho' crho crho' msr _ evM))
EvalRel-subst-forward-max sigma (App M N) rho rho' crho crho' msr ZeroEl
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst-forward-max sigma N rho rho' crho crho' msr v evN)
                      (EvalRel-subst-forward-max sigma M rho rho' crho crho' msr _ evM))
EvalRel-subst-forward-max sigma (App M N) rho rho' crho crho' msr (SucEl w)
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst-forward-max sigma N rho rho' crho crho' msr v evN)
                      (EvalRel-subst-forward-max sigma M rho rho' crho crho' msr _ evM))

-- Nat / Zero / Suc / Case
EvalRel-subst-forward-max sigma NatT rho rho' crho crho' msr u ev = ev
EvalRel-subst-forward-max sigma Zero rho rho' crho crho' msr u ev = ev
EvalRel-subst-forward-max sigma (Suc m) rho rho' crho crho' msr u
  (mkSigma cu (mkSigma v (mkSigma evm le))) =
  mkSigma cu (mkSigma v
    (mkSigma (EvalRel-subst-forward-max sigma m rho rho' crho crho' msr v evm) le))
EvalRel-subst-forward-max sigma (Case M a b) rho rho' crho crho' msr c
  (mkSigma w (mkSigma evM cb)) =
  mkSigma w (mkSigma (EvalRel-subst-forward-max sigma M rho rho' crho crho' msr w evM)
                     (CaseBranch-subst-forward-max sigma a b rho rho' crho crho' msr c w cb))
...
Comp-Bot-right NatCode         = tt
Comp-Bot-right ZeroEl          = tt
Comp-Bot-right (SucEl u)       = tt
...
-- Forward witness for a Y-approximant: combine the (finitely many, one per
-- Kleene level) witness envs along the chain.  Structural on the index k;
-- calls forward-wit on the function subterm g at each FunEl edge.
foldApproxFwd : {h n : Nat} (sigma : Sub h n) (g : Expr n)
  (rho : EnvApprox h) (k : Nat) (u : FinEl) -> CoherentEnv rho ->
  Approx (\ p w -> EvalRel (substExpr sigma g) rho (FunEl (cons (mkSigma p w) nil))) k u ->
  Sigma (EnvApprox n) (\ rho' ->
    Pair (CoherentEnv rho')
    (Pair (SubRel sigma rho rho')
          (Approx (\ p w -> EvalRel g rho' (FunEl (cons (mkSigma p w) nil))) k u)))

-- Forward witness for a case-branch (mutual with forward-wit).  The
-- ZeroEl/SucEl results are definitionally FwdResult on a / b.
CaseBranch-fwd : {h g : Nat} (sigma : Sub h g)
  (a b : Expr g) (rho : EnvApprox h) (c w : FinEl) ->
  CoherentEnv rho ->
  CaseBranch (substExpr sigma a) (substExpr sigma b) rho c w ->
  Sigma (EnvApprox g) (\ rho' ->
    Pair (CoherentEnv rho')
    (Pair (SubRel sigma rho rho')
          (CaseBranch a b rho' c w)))
CaseBranch-fwd sigma a b rho c Bot crho cb =
  mkSigma botEnv (mkSigma botEnv-Coherent (mkSigma (SubRel-botEnv sigma rho) cb))
CaseBranch-fwd sigma a b rho c UCode        crho ()
CaseBranch-fwd sigma a b rho c (FunEl g)    crho ()
CaseBranch-fwd sigma a b rho c (PiCode d f) crho ()
CaseBranch-fwd sigma a b rho c NatCode      crho ()
CaseBranch-fwd sigma a b rho c ZeroEl       crho cb =
  EvalRel-subst-forward-wit sigma a rho c crho cb
CaseBranch-fwd sigma a b rho c (SucEl v)    crho cb =
  EvalRel-subst-forward-wit sigma b rho (FunEl (cons (mkSigma v c) nil)) crho cb
...
foldApproxFwd sigma g rho zero u crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent (mkSigma (SubRel-botEnv sigma rho) ev))
foldApproxFwd sigma g rho (suc j) u crho ev =
  let p   = fst ev ; Aj = fst (snd ev) ; e = snd (snd ev)
      rec = foldApproxFwd sigma g rho j p crho Aj
      rhoJ = fst rec ; crhoJ = fst (snd rec) ; srJ = fst (snd (snd rec)) ; AjOut = snd (snd (snd rec))
      rE  = EvalRel-subst-forward-wit sigma g rho (FunEl (cons (mkSigma p u) nil)) crho e
      rhoE = fst rE ; crhoE = fst (snd rE) ; srE = fst (snd (snd rE)) ; evg = snd (snd (snd rE))
      comb = combineFwd sigma rho crho rhoJ rhoE crhoJ crhoE srJ srE
      rho' = fst comb ; crho' = fst (snd comb) ; sr' = fst (snd (snd comb))
      leJ  = fst (snd (snd (snd comb))) ; leE = snd (snd (snd (snd comb)))
      AjOut' = Approx-mon (\ p w -> EvalRel g rhoJ (FunEl (cons (mkSigma p w) nil))) (\ p w -> EvalRel g rho' (FunEl (cons (mkSigma p w) nil)))
                 (\ q w sw -> EvalRel-mon-env g rhoJ rho' (FunEl (cons (mkSigma q w) nil)) sw leJ)
                 j p AjOut
      evg' = EvalRel-mon-env g rhoE rho' (FunEl (cons (mkSigma p u) nil)) evg leE
  in mkSigma rho' (mkSigma crho' (mkSigma sr' (mkSigma p (mkSigma AjOut' evg'))))
...
-- Y
EvalRel-subst-forward-wit sigma (Y g) rho u crho ev =
  let r = foldApproxFwd sigma g rho (fst ev) u crho (snd ev)
  in mkSigma (fst r) (mkSigma (fst (snd r)) (mkSigma (fst (snd (snd r)))
       (mkSigma (fst ev) (snd (snd (snd r))))))

-- App at Nat-value result codes: combine witness envs (as UCode/FunEl/PiCode)
EvalRel-subst-forward-wit sigma (App M N) rho NatCode crho
  (mkSigma v (mkSigma evN evM)) =
  let rN = EvalRel-subst-forward-wit sigma N rho v crho evN
      rM = EvalRel-subst-forward-wit sigma M rho _ crho evM
      rhoN = fst rN ; crhoN = fst (snd rN) ; srN = fst (snd (snd rN))
  ...
  in mkSigma rho' (mkSigma crho' (mkSigma sr'
       (mkSigma v (mkSigma (EvalRel-mon-env N rhoN rho' v evN' leN)
                           (EvalRel-mon-env M rhoM rho' _ evM' leM)))))
EvalRel-subst-forward-wit sigma (App M N) rho ZeroEl crho
  (mkSigma v (mkSigma evN evM)) =
  let rN = EvalRel-subst-forward-wit sigma N rho v crho evN
      rM = EvalRel-subst-forward-wit sigma M rho _ crho evM
      rhoN = fst rN ; crhoN = fst (snd rN) ; srN = fst (snd (snd rN))
  ...
  in mkSigma rho' (mkSigma crho' (mkSigma sr'
       (mkSigma v (mkSigma (EvalRel-mon-env N rhoN rho' v evN' leN)
                           (EvalRel-mon-env M rhoM rho' _ evM' leM)))))
EvalRel-subst-forward-wit sigma (App M N) rho (SucEl w) crho
  (mkSigma v (mkSigma evN evM)) =
  let rN = EvalRel-subst-forward-wit sigma N rho v crho evN
      rM = EvalRel-subst-forward-wit sigma M rho _ crho evM
      rhoN = fst rN ; crhoN = fst (snd rN) ; srN = fst (snd (snd rN))
...
-- Nat / Zero : env-independent (botEnv witness, like U)
EvalRel-subst-forward-wit sigma NatT rho u crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent (mkSigma (SubRel-botEnv sigma rho) ev))
EvalRel-subst-forward-wit sigma Zero rho u crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent (mkSigma (SubRel-botEnv sigma rho) ev))
-- Suc : recurse on the predecessor witness
EvalRel-subst-forward-wit sigma (Suc m) rho u crho
  (mkSigma cu (mkSigma v (mkSigma evm le))) =
  let rm = EvalRel-subst-forward-wit sigma m rho v crho evm
      rho' = fst rm ; crho' = fst (snd rm)
      sr'  = fst (snd (snd rm)) ; evm' = snd (snd (snd rm))
  in mkSigma rho' (mkSigma crho' (mkSigma sr'
       (mkSigma cu (mkSigma v (mkSigma evm' le)))))
-- Case : combine the scrutinee witness with the branch witness
EvalRel-subst-forward-wit sigma (Case M a b) rho c crho
  (mkSigma w (mkSigma evM cb)) =
  let rM = EvalRel-subst-forward-wit sigma M rho w crho evM
      rBr = CaseBranch-fwd sigma a b rho c w crho cb
      rhoM = fst rM ; crhoM = fst (snd rM) ; srM = fst (snd (snd rM))
      evM' = snd (snd (snd rM))
      rhoBr = fst rBr ; crhoBr = fst (snd rBr) ; srBr = fst (snd (snd rBr))
      cb'   = snd (snd (snd rBr))
      comb = combineFwd sigma rho crho rhoM rhoBr crhoM crhoBr srM srBr
      rho' = fst comb ; crho' = fst (snd comb)
      sr'  = fst (snd (snd comb))
      leM  = fst (snd (snd (snd comb)))
      leBr = snd (snd (snd (snd comb)))
      evM'' = EvalRel-mon-env M rhoM rho' w evM' leM
      cb''  = CaseBranch-mon-env a b rhoBr rho' c w cb' leBr
  in mkSigma rho' (mkSigma crho' (mkSigma sr'
       (mkSigma w (mkSigma evM'' cb''))))
