...
------------------------------------------------------------------------
-- FinMemStageStable.agda  (NAT/ — Pi + U fragment)
--
...
module NAT.Domain.MemStable where
...
goodMemStab zero =
  ...
  where
    ...
    fmF (FunEl g)    a            ()
    fmF (PiCode a f) a'           ()
    fmF Bot          NatCode      bu ba mem = mem
    fmF Bot          ZeroEl       bu ba ()
    fmF Bot          (SucEl g)    bu ()
    fmF UCode        NatCode      bu ba ()
    fmF UCode        ZeroEl       bu ba ()
    fmF UCode        (SucEl g)    bu ()
    fmF NatCode      Bot          bu ba ()
    fmF NatCode      UCode        bu ba mem = mem
    fmF NatCode      (FunEl g)    bu ()
    fmF NatCode      (PiCode a f) bu ()
    fmF NatCode      NatCode      bu ba ()
    fmF NatCode      ZeroEl       bu ba ()
    fmF NatCode      (SucEl g)    bu ()
    fmF ZeroEl       Bot          bu ba ()
    fmF ZeroEl       UCode        bu ba ()
    fmF ZeroEl       (FunEl g)    bu ()
    fmF ZeroEl       (PiCode a f) bu ()
    fmF ZeroEl       NatCode      bu ba mem = mem
    fmF ZeroEl       ZeroEl       bu ba ()
    fmF ZeroEl       (SucEl g)    bu ()
    fmF (SucEl v)    a            ()
    ...
    fmB (FunEl g)    a            ()
    fmB (PiCode a f) a'           ()
    fmB Bot          NatCode      bu ba mem = mem
    fmB Bot          ZeroEl       bu ba ()
    fmB Bot          (SucEl g)    bu ()
    fmB UCode        NatCode      bu ba ()
    fmB UCode        ZeroEl       bu ba ()
    fmB UCode        (SucEl g)    bu ()
    fmB NatCode      Bot          bu ba ()
    fmB NatCode      UCode        bu ba mem = mem
    fmB NatCode      (FunEl g)    bu ()
    fmB NatCode      (PiCode a f) bu ()
    fmB NatCode      NatCode      bu ba ()
    fmB NatCode      ZeroEl       bu ba ()
    fmB NatCode      (SucEl g)    bu ()
    fmB ZeroEl       Bot          bu ba ()
    fmB ZeroEl       UCode        bu ba ()
    fmB ZeroEl       (FunEl g)    bu ()
    fmB ZeroEl       (PiCode a f) bu ()
    fmB ZeroEl       NatCode      bu ba mem = mem
    fmB ZeroEl       ZeroEl       bu ba ()
    fmB ZeroEl       (SucEl g)    bu ()
    fmB (SucEl v)    a            ()
...
goodMemStab (suc m) =
  ...
  where
    ...
    fmF Bot          NatCode      bu ba mem = mem
    fmF Bot          ZeroEl       bu ba ()
    fmF Bot          (SucEl g)    bu ba ()
    fmF UCode        NatCode      bu ba ()
    fmF UCode        ZeroEl       bu ba ()
    fmF UCode        (SucEl g)    bu ba ()
    fmF (FunEl g)    NatCode      bu ba ()
    fmF (FunEl g)    ZeroEl       bu ba ()
    fmF (FunEl g)    (SucEl h)    bu ba ()
    fmF (PiCode a f) NatCode      bu ba ()
    fmF (PiCode a f) ZeroEl       bu ba ()
    fmF (PiCode a f) (SucEl g)    bu ba ()
    fmF NatCode      Bot          bu ba ()
    fmF NatCode      UCode        bu ba mem = mem
    fmF NatCode      (FunEl g)    bu ba ()
    fmF NatCode      (PiCode a f) bu ba ()
    fmF NatCode      NatCode      bu ba ()
    fmF NatCode      ZeroEl       bu ba ()
    fmF NatCode      (SucEl g)    bu ba ()
    fmF ZeroEl       Bot          bu ba ()
    fmF ZeroEl       UCode        bu ba ()
    fmF ZeroEl       (FunEl g)    bu ba ()
    fmF ZeroEl       (PiCode a f) bu ba ()
    fmF ZeroEl       NatCode      bu ba mem = mem
    fmF ZeroEl       ZeroEl       bu ba ()
    fmF ZeroEl       (SucEl g)    bu ba ()
    fmF (SucEl v)    Bot          bu ba ()
    fmF (SucEl v)    UCode        bu ba ()
    fmF (SucEl v)    (FunEl g)    bu ba ()
    fmF (SucEl v)    (PiCode a f) bu ba ()
    fmF (SucEl v)    NatCode      bu ba mem = ihmF v NatCode bu (Le0 m) mem
    fmF (SucEl v)    ZeroEl       bu ba ()
    fmF (SucEl v)    (SucEl g)    bu ba ()
    ...
    fmB Bot          NatCode      bu ba mem = mem
    fmB Bot          ZeroEl       bu ba ()
    fmB Bot          (SucEl g)    bu ba ()
    fmB UCode        NatCode      bu ba ()
    fmB UCode        ZeroEl       bu ba ()
    fmB UCode        (SucEl g)    bu ba ()
    fmB (FunEl g)    NatCode      bu ba ()
    fmB (FunEl g)    ZeroEl       bu ba ()
    fmB (FunEl g)    (SucEl h)    bu ba ()
    fmB (PiCode a f) NatCode      bu ba ()
    fmB (PiCode a f) ZeroEl       bu ba ()
    fmB (PiCode a f) (SucEl g)    bu ba ()
    fmB NatCode      Bot          bu ba ()
    fmB NatCode      UCode        bu ba mem = mem
    fmB NatCode      (FunEl g)    bu ba ()
    fmB NatCode      (PiCode a f) bu ba ()
    fmB NatCode      NatCode      bu ba ()
    fmB NatCode      ZeroEl       bu ba ()
    fmB NatCode      (SucEl g)    bu ba ()
    fmB ZeroEl       Bot          bu ba ()
    fmB ZeroEl       UCode        bu ba ()
    fmB ZeroEl       (FunEl g)    bu ba ()
    fmB ZeroEl       (PiCode a f) bu ba ()
    fmB ZeroEl       NatCode      bu ba mem = mem
    fmB ZeroEl       ZeroEl       bu ba ()
    fmB ZeroEl       (SucEl g)    bu ba ()
    fmB (SucEl v)    Bot          bu ba ()
    fmB (SucEl v)    UCode        bu ba ()
    fmB (SucEl v)    (FunEl g)    bu ba ()
    fmB (SucEl v)    (PiCode a f) bu ba ()
    fmB (SucEl v)    NatCode      bu ba mem = ihmB v NatCode bu (Le0 m) mem
    fmB (SucEl v)    ZeroEl       bu ba ()
    fmB (SucEl v)    (SucEl g)    bu ba ()
