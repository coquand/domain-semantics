...
--
-- This replaces the monolithic NAT.Adequacy as the source of the
-- value-only recursors used by the bundled driver and by PiInjectivity.
------------------------------------------------------------------------

module NAT.Adequacy.Value where

import NAT.Domain.Basic as S
open S using (Nat ; tt ; mkSigma ; fst ; snd ; FinEl ; Bot ; UCode ; FunEl ; PiCode ; NatCode ; ZeroEl ; SucEl)
open import NAT.Syntax.Raw using (Expr ; U ; Pi ; Lam ; App ; Fin ; Sub ; substExpr ; subst1)
open import NAT.Domain.Kernel using (LeCode ; LeCode-refl ; FinMem ; FinMem-a-in-U)
open import NAT.Model.Eval using (EnvApprox ; EvalRel ; EvalRel-coh ; CoherentEnv)
open import NAT.Syntax.Typing using (Ctx ; extend ; lookup ; HasType ; ConvTm ; WfCtx ;
  ty-var ; ty-U ; ty-Pi ; ty-Lam ; ty-App ; ty-conv ; ty-Y ;
  conv-refl ; conv-sym ; conv-trans ; conv-conv ; conv-Pi ; conv-beta ;
  conv-funext ; conv-App-fun ; conv-App-arg ; conv-Y ; conv-Y-cong)
open import NAT.Model.Soundness using (convSound ; convSound-inv)
open import NAT.Model.SoundnessLemmas using (Fits)
open import NAT.Validity.Core using (FinMem-Coherent)
open import NAT.Syntax.Substitution using (WtSub ; WtConvSub)
open import NAT.Adequacy.HeadRed
open import NAT.Adequacy.Bundle
...
open import NAT.Adequacy.Lam using (adequacyV-ty-Lam)
open import NAT.Adequacy.Beta using (adequacyEqSub2-beta)
open import NAT.Adequacy.Funext using (adequacyEqSub2-funext)
open import NAT.Adequacy.FunCore using (adequacyEqSub2-App-fun)
...
mutual
  ...
  adequacySub2 (ty-conv {A = A} d1 d2 dB) sigma rho crho vs fits wtsub wfH u hu NatCode evA fm =
    let evA'  = convSound-inv d2 rho fits NatCode evA
        val   = adequacySub2 d1 sigma rho crho vs fits wtsub wfH u hu NatCode evA' fm
        aU    = FinMem-a-in-U u NatCode fm
        evU   = mkSigma tt (LeCode-refl UCode tt)
        eqAB  = adequacyEqSub2 d2 sigma rho crho vs fits wtsub wfH NatCode evA' UCode evU aU
        eqvty = EqVal2-U-to-EqValTy2 NatCode aU eqAB
    in Val2-EqValTy2-fwd u NatCode (EvalRel-coh A rho NatCode evA') eqvty val
  adequacySub2 (ty-conv d1 d2 dB) sigma rho crho vs fits wtsub wfH u hu ZeroEl evA fm = tt
  adequacySub2 (ty-conv d1 d2 dB) sigma rho crho vs fits wtsub wfH u hu (SucEl x) evA fm = tt
  ...
  adequacySub2 (ty-Y aU dg) sigma rho crho vs fits wtsub wfH u hu a evA fm =
    adequacyV-Y-approx aU dg sigma rho crho vs fits wtsub wfH (adequacySub2 dg) (adequacySub2 aU)
      (fst hu) u (snd hu) a evA fm
  adequacySub2 (ty-NatT wf) sigma rho crho vs fits wtsub wfH u hu ac evA fm =
    valNatT-U wfH u ac (snd hu) (snd evA) fm
  adequacySub2 (ty-Zero wf) sigma rho crho vs fits wtsub wfH u hu ac evA fm =
    valZero-Nat wfH u ac (snd hu) (snd evA) fm
  adequacySub2 (ty-Suc {m = m} dm) sigma rho crho vs fits wtsub wfH u hu ac evA fm =
  ...
  adequacyConvSub2 (ty-conv {A = A} d1 d2 dB) sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH u hu NatCode evA fm =
    let evA'  = convSound-inv d2 rho fits NatCode evA
        ih    = adequacyConvSub2 d1 sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH u hu NatCode evA' fm
        aU    = FinMem-a-in-U u NatCode fm
        evU   = mkSigma tt (LeCode-refl UCode tt)
        eqAB  = adequacyEqSub2 d2 sigma rho crho vs fits wtsub wfH NatCode evA' UCode evU aU
        eqvty = EqVal2-U-to-EqValTy2 NatCode aU eqAB
    in EqVal2-EqValTy2-fwd u NatCode (EvalRel-coh A rho NatCode evA') eqvty ih
  adequacyConvSub2 (ty-conv d1 d2 dB) sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH u hu ZeroEl evA fm = tt
  adequacyConvSub2 (ty-conv d1 d2 dB) sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH u hu (SucEl x) evA fm = tt
  ...
  adequacyConvSub2 (ty-Y aU dg) sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH u hu a evA fm =
    adequacyConvV-Y aU dg (adequacySub2 dg) (adequacySub2 aU) (adequacyConvSub2 dg)
      sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH u hu a evA fm
  adequacyConvSub2 (ty-NatT wf) sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH u hu ac evA fm =
    Val2-to-EqVal2 u ac (valNatT-U wfH u ac (snd hu) (snd evA) fm)
  adequacyConvSub2 (ty-Zero wf) sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH u hu ac evA fm =
    Val2-to-EqVal2 u ac (valZero-Nat wfH u ac (snd hu) (snd evA) fm)
  adequacyConvSub2 (ty-Suc {m = m} dm) sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH u hu ac evA fm =
  ...
  adequacyEqSub2 (conv-conv {A = A} d1 d2 dB) sigma rho crho vs fits wtsub wfH u hu NatCode evA fm =
    let evA'  = convSound-inv d2 rho fits NatCode evA
        eq    = adequacyEqSub2 d1 sigma rho crho vs fits wtsub wfH u hu NatCode evA' fm
        evU   = mkSigma tt (LeCode-refl UCode tt)
        aU    = FinMem-a-in-U u NatCode fm
        eqAB  = adequacyEqSub2 d2 sigma rho crho vs fits wtsub wfH NatCode evA' UCode evU aU
        eqvty = EqVal2-U-to-EqValTy2 NatCode aU eqAB
    in EqVal2-EqValTy2-fwd u NatCode (EvalRel-coh A rho NatCode evA') eqvty eq
  adequacyEqSub2 (conv-conv d1 d2 dB) sigma rho crho vs fits wtsub wfH u hu ZeroEl evA fm = tt
  adequacyEqSub2 (conv-conv d1 d2 dB) sigma rho crho vs fits wtsub wfH u hu (SucEl x) evA fm = tt
  ...
  adequacyEqSub2 (conv-Y aU dg) sigma rho crho vs fits wtsub wfH u hu a evA fm =
    adequacyEqV-Y-unfold aU dg (adequacySub2 dg) (adequacySub2 aU)
      sigma rho crho vs fits wtsub wfH u hu a evA fm
  adequacyEqSub2 (conv-Y-cong aU dcvg) sigma rho crho vs fits wtsub wfH u hu a evA fm =
    adequacyEqV-Y-cong aU dcvg (adequacySub2 aU) (adequacyEqSub2 dcvg)
      sigma rho crho vs fits wtsub wfH u hu a evA fm
  adequacyEqSub2 (conv-case-zero {C = C} {a = a} {b = b} dC da db) sigma rho crho vs fits wtsub wfH u hu ac evA fm =
    adequacyEqSub2-case-zero dC da db (adequacySub2 da) sigma rho crho vs fits wtsub wfH u hu ac evA fm
  adequacyEqSub2 (conv-case-suc {C = C} {m = m} {a = a} {b = b} dC dm da db) sigma rho crho vs fits wtsub wfH u hu ac evA fm =
    adequacyEqSub2-case-suc dC dm da db (adequacySub2 dC) (adequacySub2 dm) (adequacySub2 db)
      sigma rho crho vs fits wtsub wfH u hu ac evA fm
  adequacyEqSub2 (conv-Suc d) sigma rho crho vs fits wtsub wfH u hu ac evA fm =
    adequacyEqSub2-Suc d (adequacyEqSub2 d) sigma rho crho vs fits wtsub wfH u hu ac evA fm
