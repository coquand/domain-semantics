...
module NAT.Adequacy.VE where

open import NAT.Adequacy.HeadRed
open import NAT.Adequacy.Pi using (Adq ; AdqConv ; adequacy-ty-Pi ;
...
open import NAT.Domain.Kernel using (LeCode ; LeCode-refl ; Coherent ; coh-from-aU ;
...
open import NAT.Model.Eval using (EnvApprox ; extendEnv ; EvalRel ; EvalRel-coh ;
  CoherentEnv ; EvalRel-Comp)
open import NAT.Model.Soundness using (convSound ; convSound-inv)
open import NAT.Syntax.Raw using (Expr ; U ; Pi ; subst1 ; Sub ; liftSub ; substExpr)
...
open import NAT.Syntax.Substitution using (WtSub ; WtConvSub ; subst-HasType ; typing-ConvTm ;
...
open import NAT.Model.Eval using (EvalRel-mon-env ; EnvLe-refl)
...
open import NAT.Syntax.Typing using (ty-conv)
...
EqVal2-U-to-ValTy2-fst NatCode _ ev = fst (snd ev)
EqVal2-U-to-ValTy2-fst (FunEl _) () _
EqVal2-U-to-ValTy2-fst ZeroEl () _
EqVal2-U-to-ValTy2-fst (SucEl _) () _
...
EqVal2-U-to-ValTy2-snd NatCode _ ev = fst (snd (snd ev))
EqVal2-U-to-ValTy2-snd (FunEl _) () _
EqVal2-U-to-ValTy2-snd ZeroEl () _
EqVal2-U-to-ValTy2-snd (SucEl _) () _
    ...
    buildEdgeCrossBB' u0 v0 sel P htP valP =
      let fm_u0_b  = FinMemAllU-Selection b sel allU cf cb bU
          ...
          hyp0     = \ u' cu' le_u' a_arg evA_arg fm_u'_a ->
                       transportVal2' IH-A-diag sigma rho crho vs fits wtsub wfH b bU evAb u0 fm_u0_b P valP u' cu' le_u' a_arg evA_arg fm_u'_a
  ...
  where
    ...
    buildEdgeEqB2 u0 v0 sel N1 N2 htN1 htN2 cvN eqvalN =
      let valN1    = Val2-from-EqVal2-first u0 b eqvalN
          ...
          vcs_ext  = ValidConvSub2-extend sigma sigma N1 N2 rho u0
                       ...
                       (transportEqVal2' IHA-Val sigma rho crho vs fits wtsub wfH b bU evAb u0 fm_u0_b eqvalN)
    ...
    buildEdgeEqB'2 u0 v0 sel N1 N2 htN1_A' htN2_A' cvN_A' eqvalN_A' =
      let fm_u0_b  = FinMemAllU-Selection b sel allU cf cb bU
          ...
          vcs_ext  = ValidConvSub2-extend sigma sigma N1 N2 rho u0
                       (ValidConvSub2-refl {G = G} vs)
                       (transportEqVal2' IHA-Val sigma rho crho vs fits wtsub wfH b bU evAb u0 fm_u0_b eqvalN_A)
