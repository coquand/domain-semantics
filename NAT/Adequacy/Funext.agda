...
------------------------------------------------------------------------
-- AdequacyFunext.agda  (NAT/ -- PROTOTYPE)
--
...
module NAT.Adequacy.Funext where
...
open import NAT.Adequacy.Pi using (Adq ; transportVal2')
open import NAT.Adequacy.VE using (AdqE1)

import NAT.Domain.Basic as S
open S using (Nat ; suc ; tt ; mkSigma ; fst ; snd ; FinEl ; Bot ; UCode ; FunEl ; PiCode ; NatCode ; ZeroEl ; SucEl ; FinFun ; nil ; cons)
open import NAT.Domain.Kernel using (LeCode ; LeCode-refl ; Coherent ; EvalFun ; FinMem ;
...
open import NAT.Syntax.Raw using (Expr ; Var ; U ; Pi ; Lam ; App ; fzero ; wkExpr ; subst1 ;
...
open import NAT.Syntax.Substitution using (WtSub ; subst-HasType ; liftSub-WtSub)
open import NAT.Model.Soundness using (convSound)
open import NAT.Model.EvalSubstitution using (EvalRel-Pi-body ; EvalRel-wk)
open import NAT.Validity.Core using (Selection ; Coherent-Selection ; Coherent-Selection-val ;
  Selection-le-EvalFun)
open import NAT.Model.Selection using (FinMem-Selection ; FinMem-Selection-codomain)
...
adequacyEqSub2-funext {H = H} {G = G} {A = A} {B = B} {f = f} {g' = g'} dA d df dg sigma rho crho vs fits wtsub wfH IHfv IHgv IHbe IH-A (FunEl g0) hu (PiCode b f0) evA fm =
  ...
  where
    ...
    buildEqBodyCore u0 v0 sel cu0 ev_app P htP valP =
      let fm_u0_b   = FinMem-Selection b f0 sel fmg' ctg0' cb' bU'
          ...
          hyp0      = \ u'' cu'' le_u'' a_arg evA_arg fm_u''_a ->
                        transportVal2' IH-A sigma rho crho vs fits wtsub wfH b bU' evAb' u0 fm_u0_b P valP u'' cu'' le_u'' a_arg evA_arg fm_u''_a
    ...
    buildEqBody vps u0 NatCode sel P htP valP =
      let cu0 = Coherent-Selection sel ctg0'
          cv0 = Coherent-Selection-val sel ctg0'
          le  = Selection-le-EvalFun g0 sel (LeFunCode-refl g0 ctg0') ctg0' ctg0' cu0
          ev_f_sing = mkSingEvApp u0 NatCode cu0 cv0 tt le
          ev_wkf    = EvalRel-wk f rho u0 (FunEl (cons (mkSigma u0 NatCode) nil)) ev_f_sing
          ev_var    = mkSigma cu0 (LeCode-refl u0 cu0)
          ev_app    = mkSigma u0 (mkSigma ev_var ev_wkf)
      in buildEqBodyCore u0 NatCode sel cu0 ev_app P htP valP
    buildEqBody vps u0 ZeroEl sel P htP valP =
      let cu0 = Coherent-Selection sel ctg0'
          cv0 = Coherent-Selection-val sel ctg0'
          le  = Selection-le-EvalFun g0 sel (LeFunCode-refl g0 ctg0') ctg0' ctg0' cu0
          ev_f_sing = mkSingEvApp u0 ZeroEl cu0 cv0 tt le
          ev_wkf    = EvalRel-wk f rho u0 (FunEl (cons (mkSigma u0 ZeroEl) nil)) ev_f_sing
          ev_var    = mkSigma cu0 (LeCode-refl u0 cu0)
          ev_app    = mkSigma u0 (mkSigma ev_var ev_wkf)
      in buildEqBodyCore u0 ZeroEl sel cu0 ev_app P htP valP
    buildEqBody vps u0 (SucEl x) sel P htP valP =
      let cu0 = Coherent-Selection sel ctg0'
          cv0 = Coherent-Selection-val sel ctg0'
          le  = Selection-le-EvalFun g0 sel (LeFunCode-refl g0 ctg0') ctg0' ctg0' cu0
          ev_f_sing = mkSingEvApp u0 (SucEl x) cu0 cv0 tt le
          ev_wkf    = EvalRel-wk f rho u0 (FunEl (cons (mkSigma u0 (SucEl x)) nil)) ev_f_sing
          ev_var    = mkSigma cu0 (LeCode-refl u0 cu0)
          ev_app    = mkSigma u0 (mkSigma ev_var ev_wkf)
      in buildEqBodyCore u0 (SucEl x) sel cu0 ev_app P htP valP
