...
module NAT.Adequacy.Helpers where

import NAT.Domain.Basic as S
open S using (Nat ; zero ; suc ; Top ; tt ; Empty ; Sigma ; mkSigma ; fst ; snd ; Pair ; Eq ; FinEl ; Bot ; UCode ; FunEl ; PiCode ; NatCode ; ZeroEl ; SucEl ; FinFun ; List ; nil ; cons)
open import NAT.Domain.Kernel using (LeCode ; LeCode-Bot ; LeCode-refl ; LeCode-trans ; Coherent ; CoherentFun ; Comp ; Comp-down ; Sup ; LeCode-Sup-left ; LeCode-Sup-right ; Coherent-Sup ; EvalFun ; EvalFun-in-UCode ; Coherent-EvalFun ; EvalFun-mon-arg ; LeFunCode ; LeFunCode-refl ; FinMem ; FinMemFun ; FinMemAllU ; FinMem-a-in-U ; finMemUCode-Sup ; finMem-upward ; finMem-Sup-left ; finMem-Sup-right ; coh-from-aU ; FinMem-coh-u ; cft-from-cf ; CoherentFunTail ; CoherentFunTail-append ; mkCFT ; NotBot ; absurdEl)
open import NAT.Syntax.Reduction using (Red ; mkRed ; Red-refl ; Red-hr ; HeadRed ; HeadRed-trans ; HeadRed-App ; HeadRed-strip-Pi ; headred-step ; headred-beta ; headred-refl ; subst-subst1-comm ; idSub ; substExpr-id)
open import NAT.Model.Eval using (EnvApprox ; emptyEnv ; extendEnv ; lookupEnv ; EvalRel ; Pi-edgewise ; EvalRel-coh ; CoherentEnv ; lookupEnv-coh ; EvalRel-Comp ; EvalRel-Sup ; EvalRel-down ; EvalRel-mon-env ; EnvLe ; EnvLe-refl)
open import NAT.Syntax.Raw using (Expr ; Var ; U ; Pi ; Lam ; App ; Y ; NatT ; Zero ; Suc ; Case ; Eq-cong3 ; Fin ; fzero ; fsuc ; wkExpr ; subst1 ; Sub ; liftSub ; substExpr ; subst1Sub)
  renaming ()
open import NAT.Syntax.Typing using (Ctx ; empty ; extend ; lookup ; HasType ; ConvTm ; WfCtx ; wf-empty ; wf-extend ; ty-var ; ty-conv ; ty-U ; ty-Pi ; ty-Lam ; ty-App ; conv-refl ; conv-sym ; conv-trans ; conv-conv ; conv-beta ; conv-Pi ; conv-funext ; conv-App-fun ; conv-App-arg)
open import NAT.Validity.Public public
open import NAT.Validity.Core using (Edge ; EdgeIn ; here ; there ; Red-unique-Pi ; FinMem-Coherent ; Selection ; sel-nil ; sel-skip ; sel-take ; Selection-le-EvalFun ; Coherent-Selection ; Coherent-Selection-val ; bU-from-cf-fmU)
...
open import NAT.Model.Selection using (FinMemAllU-Selection ; FinMem-Selection-UCode ; FinMem-Selection ; FinMem-Selection-codomain ; selectionBelow)
open import NAT.Model.Soundness using (convSound ; convSound-inv ; convSound' ; theorem1 ; LeCode-Bot-eq)
open import NAT.Model.SoundnessLemmas using (Fits ; Typed ; Fits-CoherentEnv)
open import NAT.Model.EvalSubstitution using (EvalRel-subst1-backward ; EvalRel-wk ; EvalRel-unwk ; EvalRel-Pi-app-type ; EvalRel-Pi-body ; EvalRel-subst1-forward)

open import NAT.Syntax.Raw using (Ren ; liftRen ; renExpr ; wkRen)
open import NAT.Syntax.Substitution using (typing-ConvTm ; WtSub ; subst-HasType ; subst-ConvTm ; liftSub-WtSub ; subst1-WtSub ; typing-WfCtx ; typing-type ; ctx-conv-HasType ; ctx-conv-ConvTm ; subst1-cong-ConvTm ; wk-HasType ; wk-ConvTm ; WtConvSub ; subst-ConvTm-cross ; liftSub-WtConvSub)
...
substExpr-ext sigma sigma' hyp (Y gg) =
  S.Eq-cong Y (substExpr-ext sigma sigma' hyp gg)
substExpr-ext sigma sigma' hyp NatT       = S.refl
substExpr-ext sigma sigma' hyp Zero       = S.refl
substExpr-ext sigma sigma' hyp (Suc m)    = S.Eq-cong Suc (substExpr-ext sigma sigma' hyp m)
substExpr-ext sigma sigma' hyp (Case M a b) =
  Eq-cong3 Case (substExpr-ext sigma sigma' hyp M)
                (substExpr-ext sigma sigma' hyp a)
...
substExpr-ren sigma rho (Y gg) =
  S.Eq-cong Y (substExpr-ren sigma rho gg)
substExpr-ren sigma rho NatT       = S.refl
substExpr-ren sigma rho Zero       = S.refl
substExpr-ren sigma rho (Suc m)    = S.Eq-cong Suc (substExpr-ren sigma rho m)
substExpr-ren sigma rho (Case M a b) =
  Eq-cong3 Case (substExpr-ren sigma rho M) (substExpr-ren sigma rho a) (substExpr-ren sigma rho b)
...
renExpr-ext r1 r2 hyp (Y gg) =
  S.Eq-cong Y (renExpr-ext r1 r2 hyp gg)
renExpr-ext r1 r2 hyp NatT       = S.refl
renExpr-ext r1 r2 hyp Zero       = S.refl
renExpr-ext r1 r2 hyp (Suc m)    = S.Eq-cong Suc (renExpr-ext r1 r2 hyp m)
renExpr-ext r1 r2 hyp (Case M a b) =
  Eq-cong3 Case (renExpr-ext r1 r2 hyp M) (renExpr-ext r1 r2 hyp a) (renExpr-ext r1 r2 hyp b)
...
renExpr-comp r2 r1 (Y gg) =
  S.Eq-cong Y (renExpr-comp r2 r1 gg)
renExpr-comp r2 r1 NatT       = S.refl
renExpr-comp r2 r1 Zero       = S.refl
renExpr-comp r2 r1 (Suc m)    = S.Eq-cong Suc (renExpr-comp r2 r1 m)
renExpr-comp r2 r1 (Case M a b) =
  Eq-cong3 Case (renExpr-comp r2 r1 M) (renExpr-comp r2 r1 a) (renExpr-comp r2 r1 b)
...
renExpr-substExpr rho sigma (Y gg) =
  S.Eq-cong Y (renExpr-substExpr rho sigma gg)
renExpr-substExpr rho sigma NatT       = S.refl
renExpr-substExpr rho sigma Zero       = S.refl
renExpr-substExpr rho sigma (Suc m)    = S.Eq-cong Suc (renExpr-substExpr rho sigma m)
renExpr-substExpr rho sigma (Case M a b) =
  Eq-cong3 Case (renExpr-substExpr rho sigma M) (renExpr-substExpr rho sigma a) (renExpr-substExpr rho sigma b)
...
substExpr-sub tau sigma (Y gg) =
  S.Eq-cong Y (substExpr-sub tau sigma gg)
substExpr-sub tau sigma NatT       = S.refl
substExpr-sub tau sigma Zero       = S.refl
substExpr-sub tau sigma (Suc m)    = S.Eq-cong Suc (substExpr-sub tau sigma m)
substExpr-sub tau sigma (Case M a b) =
  Eq-cong3 Case (substExpr-sub tau sigma M) (substExpr-sub tau sigma a) (substExpr-sub tau sigma b)
...
EvalRel-bot (Var i)     rho = mkSigma tt (NAT.Domain.Kernel.LeCode-Bot (lookupEnv i rho))
EvalRel-bot U           rho = mkSigma tt (NAT.Domain.Kernel.LeCode-Bot UCode)
EvalRel-bot (Pi A B)    rho = tt
EvalRel-bot (Lam A M)   rho = tt
EvalRel-bot (App f a)   rho = tt
EvalRel-bot (Y gg)      rho = mkSigma zero (mkSigma tt (NAT.Domain.Kernel.LeCode-Bot Bot))
...
EvalRel-bot (Case M a b) rho = mkSigma Bot (mkSigma (EvalRel-bot M rho) (mkSigma tt tt))
...
FinMem-from-LeCode-UCode NatCode          ()
FinMem-from-LeCode-UCode ZeroEl           ()
FinMem-from-LeCode-UCode (SucEl x)        ()
------------------------------------------------------------------------
...
-- Val2-U-to-ValTy2, sup-transport-*, app-transport-* live in NAT.Adequacy.agda
