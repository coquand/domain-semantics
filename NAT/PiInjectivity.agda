...
------------------------------------------------------------------------
-- NAT.PiInjectivity.agda
--
-- Pi and Sigma injectivity for the Validity5/NAT.Adequacy system.
--
...
module NAT.PiInjectivity where
...
open import NAT.Domain.Kernel using (LeCode ; LeCode-Bot ; LeCode-refl ; LeCode-trans ; Coherent ; CoherentFun ; EvalFun ; FinMem ; FinMemFun ; FinMemAllU ; FinMem-coh-u ; coh-from-aU)
open import NAT.Model.Eval using (EnvApprox ; emptyEnv ; extendEnv ; lookupEnv ; EvalRel ; EvalRel-coh ; CoherentEnv ; lookupEnv-coh ; EvalRel-Bot ; EvalRel-down ; EvalRel-mon-env ; EnvLe)
import NAT.Syntax.Raw as RS
open RS using (Expr ; Var ; U ; Pi ; Lam ; App ; Fin ; fzero ; fsuc ; wkExpr ; subst1 ; Sub ; liftSub ; substExpr)
open import NAT.Syntax.Typing using (Ctx ; empty ; extend ; lookup ; HasType ; ConvTm ; WfCtx ; ty-var ; ty-conv ; ty-U ; ty-Pi ; ty-Lam ; ty-App ; conv-refl ; conv-sym ; conv-trans ; conv-conv ; conv-beta ; conv-Pi ; conv-funext ; conv-App-fun ; conv-App-arg)
open import NAT.Syntax.Reduction using (Red ; mkRed ; Red-hr ; HeadRed ; headred-refl ; headred-beta ; headred-step ; idSub ; substExpr-id)
open import NAT.Validity.Core using (Red-unique-Pi)
open import NAT.Validity.Public using (Val2 ; EqVal2 ; ValTy2 ; EqValTy2 ; Val2-Bot ; Red3 ; mkRed3 ; Red3-unique-Pi ; REqValTyPi ; un-REqValTyPi)
open import NAT.Adequacy.Helpers using (EqVal2-transport-A ; ValidSub2 ; ValidSub2-empty ; idSub-WtSub)
open import NAT.Model.SoundnessLemmas using (Fits)
open import NAT.Adequacy.Value using (adequacySub2 ; adequacyEqSub2)
open import NAT.Model.Soundness using (convSound')
open import NAT.Syntax.Substitution using (typing-ConvTm ; typing-WfCtx)
import NAT.Model.Selection
...
evalRel-Pi-trivial A B rho =
  ...
  where
    sel-body : (u v : FinEl) ->
      NAT.Model.Selection.Selection nil u v ->
      Sigma FinEl (\ x -> Pair (LeCode x u)
        (Pair (FinMem x Bot) (EvalRel B (extendEnv rho x) v)))
    sel-body .Bot .Bot NAT.Model.Selection.sel-nil =
