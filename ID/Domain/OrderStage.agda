...
module ID.Domain.OrderStage where
...
Sup (PiCode a f)    (IdCode _ _ _) = Bot
Sup (PiCode a f)    (RefEl _)     = Bot
Sup (IdCode t u v)  Bot           = IdCode t u v
Sup (IdCode t u v)  UCode         = Bot
Sup (IdCode t u v)  (FunEl h)     = Bot
Sup (IdCode t u v)  (PiCode b g)  = Bot
Sup (IdCode t u v)  (IdCode t' u' v') = IdCode (Sup t t') (Sup u u') (Sup v v')
Sup (IdCode t u v)  (RefEl _)     = Bot
Sup (RefEl w)       Bot           = RefEl w
Sup (RefEl w)       UCode         = Bot
Sup (RefEl w)       (FunEl h)     = Bot
Sup (RefEl w)       (PiCode b g)  = Bot
Sup (RefEl w)       (IdCode _ _ _) = Bot
Sup (RefEl w)       (RefEl w')    = RefEl (Sup w w')
...
mutual
  ...
  RANK (IdCode t u v) = suc (max (RANK t) (max (RANK u) (RANK v)))
  RANK (RefEl w)    = suc (RANK w)
...
mutual
  ...
  Comp UCode           (IdCode _ _ _) = Empty
  Comp UCode           (RefEl _)     = Empty
  Comp (FunEl g)       Bot           = Top
  Comp (FunEl g)       UCode         = Empty
  Comp (FunEl g)       (FunEl h)     = CompFun g h
  Comp (FunEl g)       (PiCode b h)  = Empty
  Comp (FunEl g)       (IdCode _ _ _) = Empty
  Comp (FunEl g)       (RefEl _)     = Empty
  ...
  Comp (PiCode a f)    (IdCode _ _ _) = Empty
  Comp (PiCode a f)    (RefEl _)     = Empty
  Comp (IdCode t u v)  Bot           = Top
  Comp (IdCode t u v)  UCode         = Empty
  Comp (IdCode t u v)  (FunEl h)     = Empty
  Comp (IdCode t u v)  (PiCode b g)  = Empty
  Comp (IdCode t u v)  (IdCode t' u' v') = Pair (Comp t t') (Pair (Comp u u') (Comp v v'))
  Comp (IdCode t u v)  (RefEl _)     = Empty
  Comp (RefEl w)       Bot           = Top
  Comp (RefEl w)       UCode         = Empty
  Comp (RefEl w)       (FunEl h)     = Empty
  Comp (RefEl w)       (PiCode b g)  = Empty
  Comp (RefEl w)       (IdCode _ _ _) = Empty
  Comp (RefEl w)       (RefEl w')    = Comp w w'
...
mutual
  ...
  Coherent (IdCode t u v)  = Pair (Coherent t) (Pair (Coherent u) (Coherent v))
  Coherent (RefEl w)       = Coherent w
...
trivBundle = record { leq = leq0 ; leqf = leqf0 ; lei = lei0 ; lef = lef0 ; ev = ev0 }
  where
    ...
    leq0 UCode        (IdCode _ _ _) = Empty
    leq0 UCode        (RefEl _)     = Empty
    leq0 (FunEl _)    _             = Empty
    leq0 (PiCode _ _) _             = Empty
    leq0 (IdCode _ _ _) _           = Empty
    leq0 (RefEl _)    _             = Empty
    ...
    lei0 UCode        (IdCode _ _ _) = zero
    lei0 UCode        (RefEl _)     = zero
    lei0 (FunEl _)    _             = zero
    lei0 (PiCode _ _) _             = zero
    lei0 (IdCode _ _ _) _           = zero
    lei0 (RefEl _)    _             = zero
...
buildOrderStage B =
  ...
  where
    ...
    leq' UCode        (IdCode _ _ _) = Empty
    leq' UCode        (RefEl _)     = Empty
    leq' (FunEl _)    Bot           = Empty
    leq' (FunEl _)    UCode         = Empty
    leq' (FunEl g)    (FunEl h)     = leqf' g h
    leq' (FunEl _)    (PiCode _ _)  = Empty
    leq' (FunEl _)    (IdCode _ _ _) = Empty
    leq' (FunEl _)    (RefEl _)     = Empty
    ...
    leq' (PiCode _ _) (IdCode _ _ _) = Empty
    leq' (PiCode _ _) (RefEl _)     = Empty
    leq' (IdCode _ _ _) Bot         = Empty
    leq' (IdCode _ _ _) UCode       = Empty
    leq' (IdCode _ _ _) (FunEl _)   = Empty
    leq' (IdCode _ _ _) (PiCode _ _) = Empty
    leq' (IdCode t u v) (IdCode t' u' v') = Pair (leqP t t') (Pair (leqP u u') (leqP v v'))
    leq' (IdCode _ _ _) (RefEl _)   = Empty
    leq' (RefEl _)    Bot           = Empty
    leq' (RefEl _)    UCode         = Empty
    leq' (RefEl _)    (FunEl _)     = Empty
    leq' (RefEl _)    (PiCode _ _)  = Empty
    leq' (RefEl _)    (IdCode _ _ _) = Empty
    leq' (RefEl w)    (RefEl w')    = leqP w w'
    ...
    lei' UCode        (IdCode _ _ _) = zero
    lei' UCode        (RefEl _)     = zero
    lei' (FunEl _)    Bot           = zero
    lei' (FunEl _)    UCode         = zero
    lei' (FunEl g)    (FunEl h)     = lef' g h
    lei' (FunEl _)    (PiCode _ _)  = zero
    lei' (FunEl _)    (IdCode _ _ _) = zero
    lei' (FunEl _)    (RefEl _)     = zero
    ...
    lei' (PiCode _ _) (IdCode _ _ _) = zero
    lei' (PiCode _ _) (RefEl _)     = zero
    lei' (IdCode _ _ _) Bot         = zero
    lei' (IdCode _ _ _) UCode       = zero
    lei' (IdCode _ _ _) (FunEl _)   = zero
    lei' (IdCode _ _ _) (PiCode _ _) = zero
    lei' (IdCode t u v) (IdCode t' u' v') = min (leiP t t') (min (leiP u u') (leiP v v'))
    lei' (IdCode _ _ _) (RefEl _)   = zero
    lei' (RefEl _)    Bot           = zero
    lei' (RefEl _)    UCode         = zero
    lei' (RefEl _)    (FunEl _)     = zero
    lei' (RefEl _)    (PiCode _ _)  = zero
    lei' (RefEl _)    (IdCode _ _ _) = zero
    lei' (RefEl w)    (RefEl w')    = leiP w w'
...
RANK-Sup UCode        (IdCode _ _ _) = tt
RANK-Sup UCode        (RefEl _)     = tt
RANK-Sup (FunEl g)    (IdCode _ _ _) = tt
RANK-Sup (FunEl g)    (RefEl _)     = tt
RANK-Sup (PiCode a f) (IdCode _ _ _) = tt
RANK-Sup (PiCode a f) (RefEl _)     = tt
RANK-Sup (IdCode t u v) Bot          = Le-refl (RANK (IdCode t u v))
RANK-Sup (IdCode t u v) UCode        = tt
RANK-Sup (IdCode t u v) (FunEl h)    = tt
RANK-Sup (IdCode t u v) (PiCode b g) = tt
RANK-Sup (IdCode t u v) (RefEl _)    = tt
RANK-Sup (IdCode t u v) (IdCode t' u' v') =
  let rt = RANK t ; ru = RANK u ; rv = RANK v
      rt' = RANK t' ; ru' = RANK u' ; rv' = RANK v'
      xu = max ru rv ; X = max rt xu
      yu = max ru' rv' ; Y = max rt' yu
      mXY = max X Y
      st = RANK-Sup t t'
      su = RANK-Sup u u'
      sv = RANK-Sup v v'
      rt≤X  = Le-max-l rt xu
      rt'≤Y = Le-max-l rt' yu
      ru≤X  = Le-trans ru xu X (Le-max-l ru rv) (Le-max-r rt xu)
      ru'≤Y = Le-trans ru' yu Y (Le-max-l ru' rv') (Le-max-r rt' yu)
      rv≤X  = Le-trans rv xu X (Le-max-r ru rv) (Le-max-r rt xu)
      rv'≤Y = Le-trans rv' yu Y (Le-max-r ru' rv') (Le-max-r rt' yu)
      bt = Le-trans (RANK (Sup t t')) (max rt rt') mXY st
             (max-mono rt rt' X Y rt≤X rt'≤Y)
      bu = Le-trans (RANK (Sup u u')) (max ru ru') mXY su
             (max-mono ru ru' X Y ru≤X ru'≤Y)
      bv = Le-trans (RANK (Sup v v')) (max rv rv') mXY sv
             (max-mono rv rv' X Y rv≤X rv'≤Y)
  in Le-max-lub (RANK (Sup t t')) (max (RANK (Sup u u')) (RANK (Sup v v'))) mXY
       bt (Le-max-lub (RANK (Sup u u')) (RANK (Sup v v')) mXY bu bv)
RANK-Sup (RefEl w)    Bot           = Le-max-l (RANK (RefEl w)) (RANK Bot)
RANK-Sup (RefEl w)    UCode         = tt
RANK-Sup (RefEl w)    (FunEl h)     = tt
RANK-Sup (RefEl w)    (PiCode b g)  = tt
RANK-Sup (RefEl w)    (IdCode _ _ _) = tt
RANK-Sup (RefEl w)    (RefEl w')    = RANK-Sup w w'
