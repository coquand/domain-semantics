...
module ID.Domain.OrderBridge where
...
open import ID.Domain.OrderProps using ( leq-Bot-any )
open import ID.Domain.OrderStable using ( leqC-from ; leqC-to ; lei-shift )
...
applyEl (IdCode t u v) w = Bot
applyEl (RefEl w)    v = Bot
...
mutual
  ...
  LeCode UCode           (IdCode _ _ _) = Empty
  LeCode UCode           (RefEl _)     = Empty
  LeCode (FunEl g)       (IdCode _ _ _) = Empty
  LeCode (FunEl g)       (RefEl _)     = Empty
  LeCode (PiCode a f)    (IdCode _ _ _) = Empty
  LeCode (PiCode a f)    (RefEl _)     = Empty
  LeCode (IdCode t u v)  Bot           = Empty
  LeCode (IdCode t u v)  UCode         = Empty
  LeCode (IdCode t u v)  (FunEl h)     = Empty
  LeCode (IdCode t u v)  (PiCode b g)  = Empty
  LeCode (IdCode t u v)  (IdCode t' u' v') = Pair (LeCode t t') (Pair (LeCode u u') (LeCode v v'))
  LeCode (IdCode t u v)  (RefEl _)     = Empty
  LeCode (RefEl w)       Bot           = Empty
  LeCode (RefEl w)       UCode         = Empty
  LeCode (RefEl w)       (FunEl h)     = Empty
  LeCode (RefEl w)       (PiCode b g)  = Empty
  LeCode (RefEl w)       (IdCode _ _ _) = Empty
  LeCode (RefEl w)       (RefEl w')    = LeCode w w'
...
mutual
  ...
  -- Id fragment: cross cells absurd on structural LeCode
  toLeq n UCode        (IdCode _ _ _) bnd ()
  toLeq n UCode        (RefEl _)     bnd ()
  toLeq n (FunEl g)    (IdCode _ _ _) bnd ()
  toLeq n (FunEl g)    (RefEl _)     bnd ()
  toLeq n (PiCode a f) (IdCode _ _ _) bnd ()
  toLeq n (PiCode a f) (RefEl _)     bnd ()
  toLeq n (IdCode _ _ _) Bot          bnd ()
  toLeq n (IdCode _ _ _) UCode        bnd ()
  toLeq n (IdCode _ _ _) (FunEl _)    bnd ()
  toLeq n (IdCode _ _ _) (PiCode _ _) bnd ()
  toLeq n (IdCode _ _ _) (RefEl _)    bnd ()
  toLeq zero    (IdCode t u v) (IdCode t' u' v') bnd lc = bnd
  toLeq (suc m) (IdCode t u v) (IdCode t' u' v') bnd lc =
    let Xt≤m  = max-Le-l (max (RANK t) (max (RANK u) (RANK v)))
                        (max (RANK t') (max (RANK u') (RANK v'))) m bnd
        Xt'≤m = max-Le-r (max (RANK t) (max (RANK u) (RANK v)))
                        (max (RANK t') (max (RANK u') (RANK v'))) m bnd
        ruv≤m  = max-Le-r (RANK t) (max (RANK u) (RANK v)) m Xt≤m
        ruv'≤m = max-Le-r (RANK t') (max (RANK u') (RANK v')) m Xt'≤m
        bt = Le-max-lub (RANK t) (RANK t') m
               (max-Le-l (RANK t) (max (RANK u) (RANK v)) m Xt≤m)
               (max-Le-l (RANK t') (max (RANK u') (RANK v')) m Xt'≤m)
        bu = Le-max-lub (RANK u) (RANK u') m
               (max-Le-l (RANK u) (RANK v) m ruv≤m)
               (max-Le-l (RANK u') (RANK v') m ruv'≤m)
        bv = Le-max-lub (RANK v) (RANK v') m
               (max-Le-r (RANK u) (RANK v) m ruv≤m)
               (max-Le-r (RANK u') (RANK v') m ruv'≤m)
    in mkSigma (toLeq m t t' bt (fst lc))
               (mkSigma (toLeq m u u' bu (fst (snd lc)))
                        (toLeq m v v' bv (snd (snd lc))))
  toLeq n (RefEl _)    Bot          bnd ()
  toLeq n (RefEl _)    UCode        bnd ()
  toLeq n (RefEl _)    (FunEl _)    bnd ()
  toLeq n (RefEl _)    (PiCode _ _) bnd ()
  toLeq n (RefEl _)    (IdCode _ _ _) bnd ()
  toLeq zero    (RefEl w) (RefEl w') bnd lc = bnd
  toLeq (suc m) (RefEl w) (RefEl w') bnd lc = toLeq m w w' bnd lc
  ...
  -- Id fragment: cross cells absurd on OB.leq (stage-indexed -> split n)
  fromLeq zero    UCode        (IdCode _ _ _) bnd ()
  fromLeq (suc m) UCode        (IdCode _ _ _) bnd ()
  fromLeq zero    UCode        (RefEl _)     bnd ()
  fromLeq (suc m) UCode        (RefEl _)     bnd ()
  fromLeq zero    (FunEl g)    (IdCode _ _ _) bnd ()
  fromLeq (suc m) (FunEl g)    (IdCode _ _ _) bnd ()
  fromLeq zero    (FunEl g)    (RefEl _)     bnd ()
  fromLeq (suc m) (FunEl g)    (RefEl _)     bnd ()
  fromLeq zero    (PiCode a f) (IdCode _ _ _) bnd ()
  fromLeq (suc m) (PiCode a f) (IdCode _ _ _) bnd ()
  fromLeq zero    (PiCode a f) (RefEl _)     bnd ()
  fromLeq (suc m) (PiCode a f) (RefEl _)     bnd ()
  fromLeq zero    (IdCode _ _ _) Bot          bnd ()
  fromLeq (suc m) (IdCode _ _ _) Bot          bnd ()
  fromLeq zero    (IdCode _ _ _) UCode        bnd ()
  fromLeq (suc m) (IdCode _ _ _) UCode        bnd ()
  fromLeq zero    (IdCode _ _ _) (FunEl _)    bnd ()
  fromLeq (suc m) (IdCode _ _ _) (FunEl _)    bnd ()
  fromLeq zero    (IdCode _ _ _) (PiCode _ _) bnd ()
  fromLeq (suc m) (IdCode _ _ _) (PiCode _ _) bnd ()
  fromLeq zero    (IdCode _ _ _) (RefEl _)    bnd ()
  fromLeq (suc m) (IdCode _ _ _) (RefEl _)    bnd ()
  fromLeq zero    (IdCode t u v) (IdCode t' u' v') bnd ()
  fromLeq (suc m) (IdCode t u v) (IdCode t' u' v') bnd le =
    let Xt≤m  = max-Le-l (max (RANK t) (max (RANK u) (RANK v)))
                        (max (RANK t') (max (RANK u') (RANK v'))) m bnd
        Xt'≤m = max-Le-r (max (RANK t) (max (RANK u) (RANK v)))
                        (max (RANK t') (max (RANK u') (RANK v'))) m bnd
        ...
        bv = Le-max-lub (RANK v) (RANK v') m
               (max-Le-r (RANK u) (RANK v) m ruv≤m)
               (max-Le-r (RANK u') (RANK v') m ruv'≤m)
    in mkSigma (fromLeq m t t' bt (fst le))
               (mkSigma (fromLeq m u u' bu (fst (snd le)))
                        (fromLeq m v v' bv (snd (snd le))))
  fromLeq zero    (RefEl _)    Bot          bnd ()
  fromLeq (suc m) (RefEl _)    Bot          bnd ()
  fromLeq zero    (RefEl _)    UCode        bnd ()
  fromLeq (suc m) (RefEl _)    UCode        bnd ()
  fromLeq zero    (RefEl _)    (FunEl _)    bnd ()
  fromLeq (suc m) (RefEl _)    (FunEl _)    bnd ()
  fromLeq zero    (RefEl _)    (PiCode _ _) bnd ()
  fromLeq (suc m) (RefEl _)    (PiCode _ _) bnd ()
  fromLeq zero    (RefEl _)    (IdCode _ _ _) bnd ()
  fromLeq (suc m) (RefEl _)    (IdCode _ _ _) bnd ()
  fromLeq zero    (RefEl w) (RefEl w') bnd ()
  fromLeq (suc m) (RefEl w) (RefEl w') bnd le = fromLeq m w w' bnd le
