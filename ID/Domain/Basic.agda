...
mutual
  data FinEl : Set where
    ...
    -- Identity type code: IdCode T u v  (code of Id A a b)
    IdCode   : FinEl -> FinEl -> FinEl -> FinEl
    -- Proof value: RefEl w  (a witness below both sides of an Id)
    RefEl    : FinEl -> FinEl
...
mutual
  ...
  rk (IdCode t u v) = suc (max (rk t) (max (rk u) (rk v)))
  rk (RefEl w)      = suc (rk w)
