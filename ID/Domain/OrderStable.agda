...
module ID.Domain.OrderStable where
...
goodStab zero = record { ev-st = evS0 ; lei-st = leiS0 ; lef-st = lefS0 }
  where
    ...
    leiS0 UCode        (IdCode _ _ _) _ = refl
    leiS0 UCode        (RefEl _)     _ = refl
    leiS0 (FunEl _)    (IdCode _ _ _) ()
    leiS0 (FunEl _)    (RefEl _)     ()
    leiS0 (PiCode _ _) (IdCode _ _ _) ()
    leiS0 (PiCode _ _) (RefEl _)     ()
    leiS0 (IdCode _ _ _) Bot          ()
    leiS0 (IdCode _ _ _) UCode        ()
    leiS0 (IdCode _ _ _) (FunEl _)    ()
    leiS0 (IdCode _ _ _) (PiCode _ _) ()
    leiS0 (IdCode _ _ _) (IdCode _ _ _) ()
    leiS0 (IdCode _ _ _) (RefEl _)    ()
    leiS0 (RefEl _)    Bot           ()
    leiS0 (RefEl _)    UCode         ()
    leiS0 (RefEl _)    (FunEl _)     ()
    leiS0 (RefEl _)    (PiCode _ _)  ()
    leiS0 (RefEl _)    (IdCode _ _ _) ()
    leiS0 (RefEl _)    (RefEl _)     ()
...
goodStab (suc m) = record { ev-st = evS ; lei-st = leiS ; lef-st = lefS }
  where
    ...
    leiS UCode        (IdCode _ _ _) _ = refl
    leiS UCode        (RefEl _)     _ = refl
    leiS (FunEl _)    (IdCode _ _ _) _ = refl
    leiS (FunEl _)    (RefEl _)     _ = refl
    leiS (PiCode _ _) (IdCode _ _ _) _ = refl
    leiS (PiCode _ _) (RefEl _)     _ = refl
    leiS (IdCode _ _ _) Bot          _ = refl
    leiS (IdCode _ _ _) UCode        _ = refl
    leiS (IdCode _ _ _) (FunEl _)    _ = refl
    leiS (IdCode _ _ _) (PiCode _ _) _ = refl
    leiS (IdCode _ _ _) (RefEl _)    _ = refl
    leiS (IdCode t u v) (IdCode t' u' v') cond =
      let Xt  = max (RANK t) (max (RANK u) (RANK v))
          Xt' = max (RANK t') (max (RANK u') (RANK v'))
          XtLe  = max-Le-l Xt Xt' m cond
          Xt'Le = max-Le-r Xt Xt' m cond
          rt≤m   = max-Le-l (RANK t) (max (RANK u) (RANK v)) m XtLe
          ruv≤m  = max-Le-r (RANK t) (max (RANK u) (RANK v)) m XtLe
          ru≤m   = max-Le-l (RANK u) (RANK v) m ruv≤m
          rv≤m   = max-Le-r (RANK u) (RANK v) m ruv≤m
          rt'≤m  = max-Le-l (RANK t') (max (RANK u') (RANK v')) m Xt'Le
          ruv'≤m = max-Le-r (RANK t') (max (RANK u') (RANK v')) m Xt'Le
          ru'≤m  = max-Le-l (RANK u') (RANK v') m ruv'≤m
          rv'≤m  = max-Le-r (RANK u') (RANK v') m ruv'≤m
      in min-cong
           (ihlei t t' (Le-max-lub (RANK t) (RANK t') m rt≤m rt'≤m))
           (min-cong (ihlei u u' (Le-max-lub (RANK u) (RANK u') m ru≤m ru'≤m))
                     (ihlei v v' (Le-max-lub (RANK v) (RANK v') m rv≤m rv'≤m)))
    leiS (RefEl _)    Bot          _ = refl
    leiS (RefEl _)    UCode        _ = refl
    leiS (RefEl _)    (FunEl _)    _ = refl
    leiS (RefEl _)    (PiCode _ _) _ = refl
    leiS (RefEl _)    (IdCode _ _ _) _ = refl
    leiS (RefEl w)    (RefEl w')   cond = ihlei w w' cond
