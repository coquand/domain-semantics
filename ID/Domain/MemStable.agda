...
module ID.Domain.MemStable where
...
goodMemStab (suc m) =
  ...
  where
    ...
    -- Id fragment
    fmF Bot (IdCode t u v) bu ba mem =
      let ruv = max-Le-r (RANK t) (max (RANK u) (RANK v)) m ba
          bt  = max-Le-l (RANK t) (max (RANK u) (RANK v)) m ba
      in mkSigma (ihmF t UCode bt (Le0 m) (fst mem))
                 (mkSigma (ihmF u t (max-Le-l (RANK u) (RANK v) m ruv) bt (fst (snd mem)))
                          (ihmF v t (max-Le-r (RANK u) (RANK v) m ruv) bt (snd (snd mem))))
    fmF Bot (RefEl _) bu ba ()
    fmF UCode (IdCode _ _ _) bu ba ()
    fmF UCode (RefEl _) bu ba ()
    fmF (FunEl g) (IdCode _ _ _) bu ba ()
    fmF (FunEl g) (RefEl _) bu ba ()
    fmF (PiCode a f) (IdCode _ _ _) bu ba ()
    fmF (PiCode a f) (RefEl _) bu ba ()
    fmF (IdCode t u v) UCode bu ba mem =
      let ruv = max-Le-r (RANK t) (max (RANK u) (RANK v)) m bu
          bt  = max-Le-l (RANK t) (max (RANK u) (RANK v)) m bu
      in mkSigma (ihmF t UCode bt (Le0 m) (fst mem))
                 (mkSigma (ihmF u t (max-Le-l (RANK u) (RANK v) m ruv) bt (fst (snd mem)))
                          (ihmF v t (max-Le-r (RANK u) (RANK v) m ruv) bt (snd (snd mem))))
    fmF (IdCode t u v) Bot bu ba ()
    fmF (IdCode t u v) (FunEl _) bu ba ()
    fmF (IdCode t u v) (PiCode _ _) bu ba ()
    fmF (IdCode t u v) (IdCode _ _ _) bu ba ()
    fmF (IdCode t u v) (RefEl _) bu ba ()
    fmF (RefEl w) (IdCode t u v) bu ba mem =
      let bw  = bu                                    -- Le (RANK w) m
          ruv = max-Le-r (RANK t) (max (RANK u) (RANK v)) m ba
          bt  = max-Le-l (RANK t) (max (RANK u) (RANK v)) m ba
          bu' = max-Le-l (RANK u) (RANK v) m ruv
          bv' = max-Le-r (RANK u) (RANK v) m ruv
      in mkSigma
           (mkSigma (fst (fst mem)) (ihmF w t bw bt (snd (fst mem))))
           (mkSigma (fst (snd mem))
                    (mkSigma (ihmF t UCode bt (Le0 m) (fst (snd (snd mem))))
                             (mkSigma (ihmF u t bu' bt (fst (snd (snd (snd mem)))))
                                      (ihmF v t bv' bt (snd (snd (snd (snd mem))))))))
    fmF (RefEl w) Bot bu ba ()
    fmF (RefEl w) UCode bu ba ()
    fmF (RefEl w) (FunEl _) bu ba ()
    fmF (RefEl w) (PiCode _ _) bu ba ()
    fmF (RefEl w) (RefEl _) bu ba ()
    ...
    -- Id fragment
    fmB Bot (IdCode t u v) bu ba mem =
      let ruv = max-Le-r (RANK t) (max (RANK u) (RANK v)) m ba
          bt  = max-Le-l (RANK t) (max (RANK u) (RANK v)) m ba
      in mkSigma (ihmB t UCode bt (Le0 m) (fst mem))
                 (mkSigma (ihmB u t (max-Le-l (RANK u) (RANK v) m ruv) bt (fst (snd mem)))
                          (ihmB v t (max-Le-r (RANK u) (RANK v) m ruv) bt (snd (snd mem))))
    fmB Bot (RefEl _) bu ba ()
    fmB UCode (IdCode _ _ _) bu ba ()
    fmB UCode (RefEl _) bu ba ()
    fmB (FunEl g) (IdCode _ _ _) bu ba ()
    fmB (FunEl g) (RefEl _) bu ba ()
    fmB (PiCode a f) (IdCode _ _ _) bu ba ()
    fmB (PiCode a f) (RefEl _) bu ba ()
    fmB (IdCode t u v) UCode bu ba mem =
      let ruv = max-Le-r (RANK t) (max (RANK u) (RANK v)) m bu
          bt  = max-Le-l (RANK t) (max (RANK u) (RANK v)) m bu
      in mkSigma (ihmB t UCode bt (Le0 m) (fst mem))
                 (mkSigma (ihmB u t (max-Le-l (RANK u) (RANK v) m ruv) bt (fst (snd mem)))
                          (ihmB v t (max-Le-r (RANK u) (RANK v) m ruv) bt (snd (snd mem))))
    fmB (IdCode t u v) Bot bu ba ()
    fmB (IdCode t u v) (FunEl _) bu ba ()
    fmB (IdCode t u v) (PiCode _ _) bu ba ()
    fmB (IdCode t u v) (IdCode _ _ _) bu ba ()
    fmB (IdCode t u v) (RefEl _) bu ba ()
    fmB (RefEl w) (IdCode t u v) bu ba mem =
      let bw  = bu
          ruv = max-Le-r (RANK t) (max (RANK u) (RANK v)) m ba
          bt  = max-Le-l (RANK t) (max (RANK u) (RANK v)) m ba
          bu' = max-Le-l (RANK u) (RANK v) m ruv
          bv' = max-Le-r (RANK u) (RANK v) m ruv
      in mkSigma
           (mkSigma (fst (fst mem)) (ihmB w t bw bt (snd (fst mem))))
           (mkSigma (fst (snd mem))
                    (mkSigma (ihmB t UCode bt (Le0 m) (fst (snd (snd mem))))
                             (mkSigma (ihmB u t bu' bt (fst (snd (snd (snd mem)))))
                                      (ihmB v t bv' bt (snd (snd (snd (snd mem))))))))
    fmB (RefEl w) Bot bu ba ()
    fmB (RefEl w) UCode bu ba ()
    fmB (RefEl w) (FunEl _) bu ba ()
    fmB (RefEl w) (PiCode _ _) bu ba ()
    fmB (RefEl w) (RefEl _) bu ba ()
