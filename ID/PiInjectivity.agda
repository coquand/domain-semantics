...
------------------------------------------------------------------------
-- ID.PiInjectivity.agda
--
-- Pi and Sigma injectivity for the Validity5/ID.Adequacy system.
--
...
open import ID.Domain.Kernel using (LeCode ; LeCode-Bot ; LeCode-refl ; LeCode-trans ; Coherent ; CoherentFun ; EvalFun ; FinMem ; FinMemFun ; FinMemAllU ; FinMem-coh-u ; coh-from-aU)
open import ID.Model.Eval using (EnvApprox ; emptyEnv ; extendEnv ; lookupEnv ; EvalRel ; EvalRel-coh ; CoherentEnv ; lookupEnv-coh ; EvalRel-Bot ; EvalRel-down ; EvalRel-mon-env ; EnvLe)
import ID.Syntax.Raw as RS
open RS using (Expr ; Var ; U ; Pi ; Lam ; App ; Fin ; fzero ; fsuc ; wkExpr ; subst1 ; Sub ; liftSub ; substExpr)
open import ID.Syntax.Typing using (Ctx ; empty ; extend ; lookup ; HasType ; ConvTm ; WfCtx ; ty-var ; ty-conv ; ty-U ; ty-Pi ; ty-Lam ; ty-App ; conv-refl ; conv-sym ; conv-trans ; conv-conv ; conv-beta ; conv-Pi ; conv-funext ; conv-App-fun ; conv-App-arg)
open import ID.Syntax.Reduction using (Red ; mkRed ; Red-hr ; HeadRed ; headred-refl ; headred-beta ; headred-step ; idSub ; substExpr-id)
open import ID.Validity.Core using (Red-unique-Pi)
open import ID.Validity.Public using (Val2 ; EqVal2 ; ValTy2 ; EqValTy2 ; Val2-Bot ; Red3 ; mkRed3 ; Red3-unique-Pi ; REqValTyPi ; un-REqValTyPi)
open import ID.Adequacy.Helpers using (EqVal2-transport-A ; ValidSub2 ; ValidSub2-empty ; idSub-WtSub)
open import ID.Model.SoundnessLemmas using (Fits)
open import ID.Adequacy.Value using (adequacySub2 ; adequacyEqSub2)
open import ID.Model.Soundness using (convSound')
open import ID.Syntax.Substitution using (typing-ConvTm ; typing-WfCtx)
import ID.Model.Selection
...
evalRel-Pi-trivial A B rho =
  ...
  where
    sel-body : (u v : FinEl) ->
      ID.Model.Selection.Selection nil u v ->
      Sigma FinEl (\ x -> Pair (LeCode x u)
        (Pair (FinMem x Bot) (EvalRel B (extendEnv rho x) v)))
    sel-body .Bot .Bot ID.Model.Selection.sel-nil =
