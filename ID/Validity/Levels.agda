...
module ID.Validity.Levels where
...
open import ID.Syntax.Raw using (Expr)
open import ID.Syntax.Typing using (Ctx ; HasType ; ConvTm)
open import ID.Domain.Kernel using (Coherent ; FinMem ; LeCode ; Comp ; Sup)
open import ID.Domain.Rank using (RANK ; Le-max-lub ; RANK-Sup)
open import ID.Validity.Stratified
open import ID.Validity.Stability using (vtyU ; vtyD ; evtyU ; evtyD ; vlU ; vlD ; evlU ; evlD)
open import ID.Validity.Mono using (MonoPack ; goodStage)
open import ID.Validity.Props using (FwdPack ; goodStageFwd ; SupPack ; goodStageSup ; ReflPack ; goodStageRefl ; TransportPack ; goodStageTransport ; BetaPack ; goodStageBeta)
...
EqValTy2-trans-pub u cu eqAB eqBC =
  FwdPack.EqValTy2-trans (goodStageFwd (suc (RANK u))) u cu eqAB eqBC
...
EqVal2-sym-pub u a cu ca ev =
  FwdPack.EqVal2-sym (goodStageFwd (suc (max (RANK u) (RANK a)))) u a cu ca ev
...
EqVal2-trans-pub u a cu ca ev1 ev2 =
  FwdPack.EqVal2-trans (goodStageFwd (suc (max (RANK u) (RANK a)))) u a cu ca ev1 ev2
...
-- Value-level compatible sup at a FIXED type-code a (Coquand): sup the value
-- codes u1,u2, keep the type-code a fixed.  Bridge through the 3-way-max level
-- K = suc (max (RANK a) (max (RANK u1) (RANK u2))), apply SupPack.Val2-Sup, then
-- shift the output (Sup u1 u2) down to its canonical level (RANK-Sup).
Val2-Sup-pub : {m : Nat} (G : Ctx m) (M A : Expr m) (u1 u2 a : FinEl) ->
  Comp u1 u2 -> Coherent a -> FinMem a UCode -> FinMem u1 a -> FinMem u2 a ->
  Val2 G M A u1 a -> Val2 G M A u2 a -> Val2 G M A (Sup u1 u2) a
Val2-Sup-pub G M A u1 u2 a comp ca fma fm1 fm2 v1 v2 =
  let v1-K = shiftVl (suc (max (RANK u1) (RANK a))) (suc (max (RANK a) (max (RANK u1) (RANK u2)))) G M A u1 a
               (b2-l u1 a) (b2-r u1 a) (b3-m a u1 u2) (b3-l a u1 u2) v1
      v2-K = shiftVl (suc (max (RANK u2) (RANK a))) (suc (max (RANK a) (max (RANK u1) (RANK u2)))) G M A u2 a
               (b2-l u2 a) (b2-r u2 a) (b3-r a u1 u2) (b3-l a u1 u2) v2
      sup-K = SupPack.Val2-Sup (goodStageSup (suc (max (RANK a) (max (RANK u1) (RANK u2)))))
                G M A u1 u2 a comp ca fma fm1 fm2 v1-K v2-K
  in shiftVl (suc (max (RANK a) (max (RANK u1) (RANK u2)))) (suc (max (RANK (Sup u1 u2)) (RANK a))) G M A (Sup u1 u2) a
       (Le-trans (RANK (Sup u1 u2)) (max (RANK u1) (RANK u2)) (max (RANK a) (max (RANK u1) (RANK u2)))
          (RANK-Sup u1 u2) (Le-max-r (RANK a) (max (RANK u1) (RANK u2))))
       (b3-l a u1 u2) (b2-l (Sup u1 u2) a) (b2-r (Sup u1 u2) a) sup-K

EqVal2-Sup-pub : {m : Nat} (G : Ctx m) (M N A : Expr m) (u1 u2 a : FinEl) ->
  Comp u1 u2 -> Coherent a -> FinMem a UCode -> FinMem u1 a -> FinMem u2 a ->
  EqVal2 G M N A u1 a -> EqVal2 G M N A u2 a -> EqVal2 G M N A (Sup u1 u2) a
EqVal2-Sup-pub G M N A u1 u2 a comp ca fma fm1 fm2 e1 e2 =
  let e1-K = shiftEVl (suc (max (RANK u1) (RANK a))) (suc (max (RANK a) (max (RANK u1) (RANK u2)))) G M N A u1 a
               (b2-l u1 a) (b2-r u1 a) (b3-m a u1 u2) (b3-l a u1 u2) e1
      e2-K = shiftEVl (suc (max (RANK u2) (RANK a))) (suc (max (RANK a) (max (RANK u1) (RANK u2)))) G M N A u2 a
               (b2-l u2 a) (b2-r u2 a) (b3-r a u1 u2) (b3-l a u1 u2) e2
      sup-K = SupPack.EqVal2-Sup (goodStageSup (suc (max (RANK a) (max (RANK u1) (RANK u2)))))
                G M N A u1 u2 a comp ca fma fm1 fm2 e1-K e2-K
  in shiftEVl (suc (max (RANK a) (max (RANK u1) (RANK u2)))) (suc (max (RANK (Sup u1 u2)) (RANK a))) G M N A (Sup u1 u2) a
       (Le-trans (RANK (Sup u1 u2)) (max (RANK u1) (RANK u2)) (max (RANK a) (max (RANK u1) (RANK u2)))
          (RANK-Sup u1 u2) (Le-max-r (RANK a) (max (RANK u1) (RANK u2))))
       (b3-l a u1 u2) (b2-l (Sup u1 u2) a) (b2-r (Sup u1 u2) a) sup-K
