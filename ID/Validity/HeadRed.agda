...
module ID.Validity.HeadRed where

open import ID.Validity.Mono
open import ID.Validity.Props using (BetaPack ; goodStageBeta)
open import ID.Validity.Stratified using (Red3 ; mkRed3 ; Val2 ; EqVal2)

import ID.Domain.Basic as S
open S using (Nat ; zero ; suc ; Top ; tt ; Empty ; Pair ; mkSigma ; fst ; snd ;
              max ; FinEl ; Bot ; UCode ; FunEl ; PiCode ; IdCode ; RefEl ; FinFun)
...
open import ID.Syntax.Reduction using (HeadRed ; HeadRed1 ; HeadRed1-det ;
  headred-refl ; headred-step ; HeadRed-trans ; HeadRed-App ; HeadRed-strip-Pi ; HeadRed-strip-Id ; HeadRed-strip-Ref)
open import ID.Domain.Kernel using (EvalFun)
open import ID.Syntax.Substitution using (typing-ConvTm ; typing-type ; subst1-cong-ConvTm)
...
goodStageHeadRed (suc n) = record
  ...
  where
    ...
    ValTy2-headred-contract (IdCode t u v) hr cv vt =
      record { domA = RValTyId.domA vt ; lhs = RValTyId.lhs vt ; rhs = RValTyId.rhs vt
             ; red = mkRed3 (HeadRed-strip-Id hr (Red3.hr (RValTyId.red vt)))
                            (conv-trans (conv-sym cv) (Red3.ct (RValTyId.red vt)))
             ; htA = RValTyId.htA vt ; htL = RValTyId.htL vt ; htR = RValTyId.htR vt
             ; valA = RValTyId.valA vt ; valL = RValTyId.valL vt ; valR = RValTyId.valR vt ; valLlog = RValTyId.valLlog vt ; valRlog = RValTyId.valRlog vt }
    ValTy2-headred-contract (RefEl w) hr cv vt = tt
    ...
    ValTy2-headred-expand (PiCode b f) hr cv vt =
      record { domA = RValTyPi.domA vt ; codB = RValTyPi.codB vt
             ...
             ; valA = RValTyPi.valA vt
             ; edgeV = RValTyPi.edgeV vt ; edgeE = RValTyPi.edgeE vt }
    ValTy2-headred-expand (IdCode t u v) hr cv vt =
      record { domA = RValTyId.domA vt ; lhs = RValTyId.lhs vt ; rhs = RValTyId.rhs vt
             ; red = mkRed3 (HeadRed-trans hr (Red3.hr (RValTyId.red vt)))
                            (conv-trans cv (Red3.ct (RValTyId.red vt)))
             ; htA = RValTyId.htA vt ; htL = RValTyId.htL vt ; htR = RValTyId.htR vt
             ; valA = RValTyId.valA vt ; valL = RValTyId.valL vt ; valR = RValTyId.valR vt ; valLlog = RValTyId.valLlog vt ; valRlog = RValTyId.valRlog vt }
    ValTy2-headred-expand (RefEl w) hr cv vt = tt
    ...
    EqValTy2-headred-contract (IdCode t u v) hr1 hr2 cv1 cv2 eqvt =
      let vt1 = fst eqvt ; vt2 = fst (snd eqvt) ; core = snd (snd eqvt)
      in mkSigma (ValTy2-headred-contract (IdCode t u v) hr1 cv1 vt1)
           (mkSigma (ValTy2-headred-contract (IdCode t u v) hr2 cv2 vt2)
             (record { domA = REqValTyId.domA core ; lhs = REqValTyId.lhs core ; rhs = REqValTyId.rhs core
                     ; domA' = REqValTyId.domA' core ; lhs' = REqValTyId.lhs' core ; rhs' = REqValTyId.rhs' core
                     ; redM = mkRed3 (HeadRed-strip-Id hr1 (Red3.hr (REqValTyId.redM core)))
                                     (conv-trans (conv-sym cv1) (Red3.ct (REqValTyId.redM core)))
                     ; redN = mkRed3 (HeadRed-strip-Id hr2 (Red3.hr (REqValTyId.redN core)))
                                     (conv-trans (conv-sym cv2) (Red3.ct (REqValTyId.redN core)))
                     ; convA = REqValTyId.convA core ; convL = REqValTyId.convL core ; convR = REqValTyId.convR core
                     ; eqA = REqValTyId.eqA core ; eqL = REqValTyId.eqL core ; eqR = REqValTyId.eqR core }))
    EqValTy2-headred-contract (RefEl w) hr1 hr2 cv1 cv2 eqvt = tt
      ...
      in mkSigma (ValTy2-headred-expand (PiCode b f) hr1 cv1 vt1)
           (mkSigma (ValTy2-headred-expand (PiCode b f) hr2 cv2 vt2)
             (record { domA = REqValTyPi.domA core ; codB = REqValTyPi.codB core
                     ...
                     ; convA = REqValTyPi.convA core ; convB = REqValTyPi.convB core
                     ; eqA = REqValTyPi.eqA core ; edgeET = REqValTyPi.edgeET core }))
    EqValTy2-headred-expand (IdCode t u v) hr1 hr2 cv1 cv2 eqvt =
      let vt1 = fst eqvt ; vt2 = fst (snd eqvt) ; core = snd (snd eqvt)
      in mkSigma (ValTy2-headred-expand (IdCode t u v) hr1 cv1 vt1)
           (mkSigma (ValTy2-headred-expand (IdCode t u v) hr2 cv2 vt2)
             (record { domA = REqValTyId.domA core ; lhs = REqValTyId.lhs core ; rhs = REqValTyId.rhs core
                     ; domA' = REqValTyId.domA' core ; lhs' = REqValTyId.lhs' core ; rhs' = REqValTyId.rhs' core
                     ; redM = mkRed3 (HeadRed-trans hr1 (Red3.hr (REqValTyId.redM core)))
                                     (conv-trans cv1 (Red3.ct (REqValTyId.redM core)))
                     ; redN = mkRed3 (HeadRed-trans hr2 (Red3.hr (REqValTyId.redN core)))
                                     (conv-trans cv2 (Red3.ct (REqValTyId.redN core)))
                     ; convA = REqValTyId.convA core ; convL = REqValTyId.convL core ; convR = REqValTyId.convR core
                     ; eqA = REqValTyId.eqA core ; eqL = REqValTyId.eqL core ; eqR = REqValTyId.eqR core }))
    EqValTy2-headred-expand (RefEl w) hr1 hr2 cv1 cv2 eqvt = tt
    ...
    Val2-headred-contract (IdCode t u v) UCode hr cv val =
      let vtA = fst val ; vtId = snd val
          ctU = conv-conv cv (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
      in mkSigma vtA (ValTy2-headred-contract (IdCode t u v) hr ctU vtId)
    Val2-headred-contract (RefEl w) UCode hr cv tt = tt
    Val2-headred-contract (IdCode t u v) (PiCode b f) hr cv tt = tt
    Val2-headred-contract (RefEl w) (PiCode b f) hr cv tt = tt
    Val2-headred-contract (RefEl w) (IdCode t' u' v') hr cv val =
      let rid = snd val
      in mkSigma (fst val)
        (record { domA0 = RValId.domA0 rid ; lhs0 = RValId.lhs0 rid
                ; rhs0 = RValId.rhs0 rid ; red = RValId.red rid ; wit0 = RValId.wit0 rid
                ; redTm = mkRed3 (HeadRed-strip-Ref hr (Red3.hr (RValId.redTm rid)))
                                 (conv-trans (conv-sym cv) (Red3.ct (RValId.redTm rid)))
                ; refConvL = RValId.refConvL rid ; refConvR = RValId.refConvR rid ; refMem = RValId.refMem rid ; endEqL = RValId.endEqL rid ; endEqR = RValId.endEqR rid })
    Val2-headred-contract Bot (IdCode a a1 a2) hr cv tt = tt
    Val2-headred-contract UCode (IdCode a a1 a2) hr cv tt = tt
    Val2-headred-contract (FunEl g) (IdCode a a1 a2) hr cv tt = tt
    Val2-headred-contract (PiCode b f) (IdCode a a1 a2) hr cv tt = tt
    Val2-headred-contract (IdCode s0 s1 s2) (IdCode a a1 a2) hr cv tt = tt
    Val2-headred-contract u (RefEl a) hr cv tt = tt
    ...
    EqVal2-headred-expand (IdCode t u v) UCode hr1 hr2 cv1 cv2 ev =
      let vtA = fst ev
          ctU1 = conv-conv cv1 (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
          ctU2 = conv-conv cv2 (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
          expand1 = ValTy2-headred-expand (IdCode t u v) hr1 ctU1 (fst (snd ev))
          expand2 = ValTy2-headred-expand (IdCode t u v) hr2 ctU2 (fst (snd (snd ev)))
          eqexpand = EqValTy2-headred-expand (IdCode t u v) hr1 hr2 ctU1 ctU2 (snd (snd (snd ev)))
      in mkSigma vtA (mkSigma expand1 (mkSigma expand2 eqexpand))
    EqVal2-headred-expand (RefEl w) UCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (IdCode t u v) (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (RefEl w) (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (RefEl w) (IdCode t' u' v') hr1 hr2 cv1 cv2 ev =
      let ridM = fst (snd ev) ; ridN = fst (snd (snd ev)) ; reid = snd (snd (snd ev))
          expM = mkRed3 (HeadRed-trans hr1 (Red3.hr (RValId.redTm ridM))) (conv-trans cv1 (Red3.ct (RValId.redTm ridM)))
          expN = mkRed3 (HeadRed-trans hr2 (Red3.hr (RValId.redTm ridN))) (conv-trans cv2 (Red3.ct (RValId.redTm ridN)))
          eqM  = mkRed3 (HeadRed-trans hr1 (Red3.hr (REqValId.redTmM reid))) (conv-trans cv1 (Red3.ct (REqValId.redTmM reid)))
          eqN  = mkRed3 (HeadRed-trans hr2 (Red3.hr (REqValId.redTmN reid))) (conv-trans cv2 (Red3.ct (REqValId.redTmN reid)))
      in mkSigma (fst ev)
        (mkSigma (record { domA0 = RValId.domA0 ridM ; lhs0 = RValId.lhs0 ridM
                         ; rhs0 = RValId.rhs0 ridM ; red = RValId.red ridM ; wit0 = RValId.wit0 ridM ; redTm = expM ; refConvL = RValId.refConvL ridM ; refConvR = RValId.refConvR ridM ; refMem = RValId.refMem ridM ; endEqL = RValId.endEqL ridM ; endEqR = RValId.endEqR ridM })
          (mkSigma (record { domA0 = RValId.domA0 ridN ; lhs0 = RValId.lhs0 ridN
                           ; rhs0 = RValId.rhs0 ridN ; red = RValId.red ridN ; wit0 = RValId.wit0 ridN ; redTm = expN ; refConvL = RValId.refConvL ridN ; refConvR = RValId.refConvR ridN ; refMem = RValId.refMem ridN ; endEqL = RValId.endEqL ridN ; endEqR = RValId.endEqR ridN })
            (record { domA0 = REqValId.domA0 reid ; lhs0 = REqValId.lhs0 reid
                    ; rhs0 = REqValId.rhs0 reid ; red = REqValId.red reid
                    ; wit0M = REqValId.wit0M reid ; wit0N = REqValId.wit0N reid ; redTmM = eqM ; redTmN = eqN ; refMem = REqValId.refMem reid ; endEqLM = REqValId.endEqLM reid ; endEqRM = REqValId.endEqRM reid ; endEqLN = REqValId.endEqLN reid ; endEqRN = REqValId.endEqRN reid })))
    EqVal2-headred-expand Bot (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand UCode (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (FunEl g) (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (PiCode b f) (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand (IdCode s0 s1 s2) (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-expand u (RefEl a) hr1 hr2 cv1 cv2 tt = tt
    ...
    EqVal2-headred-contract (IdCode t u v) UCode hr1 hr2 cv1 cv2 ev =
      let vtA = fst ev
          ctU1 = conv-conv cv1 (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
          ctU2 = conv-conv cv2 (Red3.ct vtA) (typing-type (snd (typing-ConvTm (Red3.ct vtA))))
      in mkSigma vtA
           (mkSigma (ValTy2-headred-contract (IdCode t u v) hr1 ctU1 (fst (snd ev)))
             (mkSigma (ValTy2-headred-contract (IdCode t u v) hr2 ctU2 (fst (snd (snd ev))))
               (EqValTy2-headred-contract (IdCode t u v) hr1 hr2 ctU1 ctU2 (snd (snd (snd ev))))))
    EqVal2-headred-contract (RefEl w) UCode hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (IdCode t u v) (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (RefEl w) (PiCode b f) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (RefEl w) (IdCode t' u' v') hr1 hr2 cv1 cv2 ev =
      let ridM = fst (snd ev) ; ridN = fst (snd (snd ev)) ; reid = snd (snd (snd ev))
          ctM = mkRed3 (HeadRed-strip-Ref hr1 (Red3.hr (RValId.redTm ridM))) (conv-trans (conv-sym cv1) (Red3.ct (RValId.redTm ridM)))
          ctN = mkRed3 (HeadRed-strip-Ref hr2 (Red3.hr (RValId.redTm ridN))) (conv-trans (conv-sym cv2) (Red3.ct (RValId.redTm ridN)))
          eqM = mkRed3 (HeadRed-strip-Ref hr1 (Red3.hr (REqValId.redTmM reid))) (conv-trans (conv-sym cv1) (Red3.ct (REqValId.redTmM reid)))
          eqN = mkRed3 (HeadRed-strip-Ref hr2 (Red3.hr (REqValId.redTmN reid))) (conv-trans (conv-sym cv2) (Red3.ct (REqValId.redTmN reid)))
      in mkSigma (fst ev)
        (mkSigma (record { domA0 = RValId.domA0 ridM ; lhs0 = RValId.lhs0 ridM
                         ; rhs0 = RValId.rhs0 ridM ; red = RValId.red ridM ; wit0 = RValId.wit0 ridM ; redTm = ctM ; refConvL = RValId.refConvL ridM ; refConvR = RValId.refConvR ridM ; refMem = RValId.refMem ridM ; endEqL = RValId.endEqL ridM ; endEqR = RValId.endEqR ridM })
          (mkSigma (record { domA0 = RValId.domA0 ridN ; lhs0 = RValId.lhs0 ridN
                           ; rhs0 = RValId.rhs0 ridN ; red = RValId.red ridN ; wit0 = RValId.wit0 ridN ; redTm = ctN ; refConvL = RValId.refConvL ridN ; refConvR = RValId.refConvR ridN ; refMem = RValId.refMem ridN ; endEqL = RValId.endEqL ridN ; endEqR = RValId.endEqR ridN })
            (record { domA0 = REqValId.domA0 reid ; lhs0 = REqValId.lhs0 reid
                    ; rhs0 = REqValId.rhs0 reid ; red = REqValId.red reid
                    ; wit0M = REqValId.wit0M reid ; wit0N = REqValId.wit0N reid ; redTmM = eqM ; redTmN = eqN ; refMem = REqValId.refMem reid ; endEqLM = REqValId.endEqLM reid ; endEqRM = REqValId.endEqRM reid ; endEqLN = REqValId.endEqLN reid ; endEqRN = REqValId.endEqRN reid })))
    EqVal2-headred-contract Bot (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract UCode (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (FunEl g) (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (PiCode b f) (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract (IdCode s0 s1 s2) (IdCode a a1 a2) hr1 hr2 cv1 cv2 tt = tt
    EqVal2-headred-contract u (RefEl a) hr1 hr2 cv1 cv2 tt = tt
