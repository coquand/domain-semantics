...
module ID.Adequacy.ArgCore where
open import ID.Adequacy.HeadRed

open import ID.Adequacy.App using (app-transport-EqVal2)
...
open import ID.Syntax.Reduction using (Red ; mkRed ; Red-refl ; HeadRed ; headred-step ; headred-beta ;
...
open import ID.Syntax.Raw using (Expr ; Var ; U ; Pi ; Lam ; App ; Fin ; fzero ; fsuc ; wkExpr ;
  subst1 ; Sub ; liftSub ; substExpr ; subst1Sub)
open import ID.Syntax.Typing using (Ctx ; extend ; HasType ; ConvTm ; WfCtx ; wf-extend ; ty-var ;
...
open import ID.Model.Selection using (FinMemAllU-Selection ; selectionBelow ; FinMem-Selection ;
  FinMem-Selection-codomain)
open import ID.Model.EvalSubstitution using (EvalRel-Pi-app-type ; EvalRel-subst1-forward)
open import ID.Model.Soundness using (convSound' ; theorem1)
open import ID.Syntax.Substitution using (typing-ConvTm ; WtSub ; subst-HasType ; subst-ConvTm ;
  ...
  where
    ...
    appEqVal-dispatch (FunEl g_big) (PiCode b_pi f_pi) lf evFb evPab fmba valba =
      let le_u1_vsel = fst lf
          ...
          vs_ext   = ValidSub2-extend {A = A} sigma sa rho v_fwd' vs hyp_ext
          ...
          hyp_ef    = \ u' cu' le_u' a_arg evA_aarg fm_u'_a ->
            ...
            in val_sa u' evA_u' a_arg evA_aarg fm_u'_a
          vs_ef     = ValidSub2-extend {A = A} sigma sa rho v_fwd_ef' vs hyp_ef
...
adequacyEqSub2-App-arg {H = H} dB df daa' sigma rho crho vs fits wtsub wfH IHfv IHBv IHaae (IdCode ib ic idd) ev UCode evAc fm =
  adequacyEqSub2-App-arg-core-body {H = H} dB df daa' sigma rho crho vs fits wtsub wfH (IdCode ib ic idd)
    (fst ev) (fst (snd ev)) (snd (snd ev)) UCode evAc fm IHfv IHBv IHaae
adequacyEqSub2-App-arg {H = H} dB df daa' sigma rho crho vs fits wtsub wfH IHfv IHBv IHaae (RefEl rw) ev (IdCode ac ac1 ac2) evAc fm =
  adequacyEqSub2-App-arg-core-body {H = H} dB df daa' sigma rho crho vs fits wtsub wfH (RefEl rw)
    (fst ev) (fst (snd ev)) (snd (snd ev)) (IdCode ac ac1 ac2) evAc fm IHfv IHBv IHaae
