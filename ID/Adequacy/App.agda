...
module ID.Adequacy.App where

open import ID.Adequacy.HeadRed
open import ID.Adequacy.Cases using (adequacyConvSub2-App-core-body ; adequacySub2-App-core-body)
open import ID.Adequacy.Pi using (Adq ; AdqConv ; Val2-U-to-ValTy2)

import ID.Domain.Basic as S
open S using (Nat ; zero ; suc ; tt ; mkSigma ; fst ; snd ; FinEl ; Bot ; UCode ; FunEl ; PiCode ; IdCode ; RefEl ;
  FinFun ; List ; nil ; cons)
open import ID.Domain.Kernel using (LeCode ; Coherent ; coh-from-aU ; FinMem ; FinMem-coh-u ;
...
open import ID.Syntax.Raw using (Expr ; U ; App ; Pi ; subst1 ; Sub ; liftSub ; substExpr)
open import ID.Syntax.Typing using (Ctx ; extend ; HasType ; WfCtx)
open import ID.Syntax.Substitution using (WtSub ; WtConvSub ; subst-HasType ; subst-ConvTm-cross)
open import ID.Model.SoundnessLemmas using (Fits)
open import ID.Model.Soundness using (theorem1)
open import ID.Model.EvalSubstitution using (EvalRel-subst1-forward)
open import ID.Syntax.Reduction using (subst-subst1-comm)
...
adequacyV-ty-App {H = H} {A = A} {B = B} {f = f0} {a = a} dA dB df da IHav IHBv IHfc IHac sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH (FunEl gfe) hu (PiCode bacfe facfe) evA fm =
  adequacyConvSub2-App-core-body {H = H} {A = A} {B = B} {f = f0} {a = a}
    ...
    IHav IHBv IHfc IHac Val2-U-to-ValTy2 app-transport-EqVal2
adequacyV-ty-App {H = H} {A = A} {B = B} {f = f0} {a = a} dA dB df da IHav IHBv IHfc IHac sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH (IdCode ib ic idd) hu UCode evA fm =
  adequacyConvSub2-App-core-body {H = H} {A = A} {B = B} {f = f0} {a = a}
    dA dB df da sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH
    (IdCode ib ic idd) (fst hu) (fst (snd hu)) (snd (snd hu)) UCode evA fm
    IHav IHBv IHfc IHac Val2-U-to-ValTy2 app-transport-EqVal2
adequacyV-ty-App {H = H} {A = A} {B = B} {f = f0} {a = a} dA dB df da IHav IHBv IHfc IHac sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH (RefEl rw) hu (IdCode ac ac1 ac2) evA fm =
  adequacyConvSub2-App-core-body {H = H} {A = A} {B = B} {f = f0} {a = a}
    dA dB df da sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH
    (RefEl rw) (fst hu) (fst (snd hu)) (snd (snd hu)) (IdCode ac ac1 ac2) evA fm
    IHav IHBv IHfc IHac Val2-U-to-ValTy2 app-transport-EqVal2
...
adequacySub2-App {H = H} dA dB d1 d2 IHf IHa IHB sigma rho crho vs fits wtsub wfH (FunEl gfe) cu1 ev1 (PiCode bacfe facfe) evAc1 fm1 =
  adequacySub2-App-core-body {H = H} dA dB d1 d2 sigma rho crho vs fits wtsub wfH (FunEl gfe) cu1
    ...
    IHf IHa IHB Val2-U-to-ValTy2 app-transport-Val2
-- App result is an Id-TYPE (IdCode value-code at UCode) or a PROOF (RefEl at IdCode)
adequacySub2-App {H = H} dA dB d1 d2 IHf IHa IHB sigma rho crho vs fits wtsub wfH (IdCode ib ic idd) cu1 ev1 UCode evAc1 fm1 =
  adequacySub2-App-core-body {H = H} dA dB d1 d2 sigma rho crho vs fits wtsub wfH (IdCode ib ic idd) cu1
    (fst ev1) (fst (snd ev1)) (snd (snd ev1)) UCode evAc1 fm1
    IHf IHa IHB Val2-U-to-ValTy2 app-transport-Val2
adequacySub2-App {H = H} dA dB d1 d2 IHf IHa IHB sigma rho crho vs fits wtsub wfH (RefEl rw) cu1 ev1 (IdCode ac ac1 ac2) evAc1 fm1 =
  adequacySub2-App-core-body {H = H} dA dB d1 d2 sigma rho crho vs fits wtsub wfH (RefEl rw) cu1
    (fst ev1) (fst (snd ev1)) (snd (snd ev1)) (IdCode ac ac1 ac2) evAc1 fm1
    IHf IHa IHB Val2-U-to-ValTy2 app-transport-Val2
