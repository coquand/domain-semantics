...
module ID.Adequacy.Lam where

open import ID.Adequacy.HeadRed
open import ID.Adequacy.Pi using (Adq ; AdqConv ; transportVal2' ; transportEqVal2' ;
...
open import ID.Syntax.Raw using (Expr ; U ; Pi ; Lam ; subst1 ; Sub ; liftSub ; substExpr)
...
open import ID.Syntax.Reduction using (headred-refl ; headred-step ; headred-beta)
open import ID.Model.Selection using (FinMem-Selection ; FinMem-Selection-codomain ; Coherent-Selection)
open import ID.Model.EvalSubstitution using (EvalRel-Pi-body)
    ...
    piAppVal u' v' sel N htN valN =
      let cu'       = Coherent-Selection sel (cft-from-cf g0 cg)
          ...
          hyp0      = \ u'' cu'' le_u'' a_arg evA_arg fm_u''_a ->
                        transportVal2' {A = A} IH-A sigma rho crho vs fits wtsub wfH b bU evAb u' fm_u'_b N valN u'' cu'' le_u'' a_arg evA_arg fm_u''_a
    ...
    piAppEq u' v' sel N1 N2 htN1 htN2 cvN eqvalN =
      let valN1     = Val2-from-EqVal2-first u' b eqvalN
          ...
          hyp0_N1   = \ u'' cu'' le_u'' a_arg evA_arg fm_u''_a ->
                        transportVal2' {A = A} IH-A sigma rho crho vs fits wtsub wfH b bU evAb u' fm_u'_b N1 valN1 u'' cu'' le_u'' a_arg evA_arg fm_u''_a
          hyp0_N2   = \ u'' cu'' le_u'' a_arg evA_arg fm_u''_a ->
                        transportVal2' {A = A} IH-A sigma rho crho vs fits wtsub wfH b bU evAb u' fm_u'_b N2 valN2 u'' cu'' le_u'' a_arg evA_arg fm_u''_a
          ...
          vcs_ext   = ValidConvSub2-extend sigma sigma N1 N2 rho u'
                        ...
                        (transportEqVal2' {A = A} IH-A sigma rho crho vs fits wtsub wfH b bU evAb u' fm_u'_b eqvalN)
  ...
  where
    ...
    buildAppEV u' v' sel P htP valP =
      let ctg = cft-from-cf g0 cg ; cu' = Coherent-Selection sel ctg
          ...
          fits' = mkSigma fits (mkSigma b (mkSigma fm_u'_b evAb))
          crho' = mkSigma crho cu'
          hyp0_s = \ u'' cu'' le a1 evA1 fm1 -> transportVal2' {A = A} IH-A sigma rho crho vs fits wtsub wfH b bU evAb u' fm_u'_b P valP u'' cu'' le a1 evA1 fm1
          ...
          hyp0_conv = \ u'' cu'' le a1 evA1 fm1 -> Val2-to-EqVal2 u'' a1 (transportVal2' {A = A} IH-A sigma rho crho vs fits wtsub wfH b bU evAb u' fm_u'_b P valP u'' cu'' le a1 evA1 fm1)
