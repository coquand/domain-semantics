...
module ID.Adequacy.Bundle where

open import ID.Adequacy.HeadRed
open import ID.Adequacy.VE public

import ID.Domain.Basic as S
open S using (Nat ; zero ; suc ; tt ; Pair ; mkSigma ; fst ; snd ; FinEl ; UCode ; Bot ; FunEl ; PiCode)
open import ID.Syntax.Raw using (Expr ; Var ; U ; Pi ; Lam ; App ; subst1 ; Fin ; fzero ; fsuc ; Sub ; liftSub ; substExpr ; wkExpr)
open import ID.Model.Eval using (EnvApprox ; EvalRel ; CoherentEnv ; lookupEnv ; extendEnv ;
  EvalRel-Comp ; EvalRel-mon-env ; EvalRel-down ; EnvLe-refl)
open import ID.Domain.Kernel using (Coherent ; FinMem ; LeCode ; LeCode-refl ; FinMem-a-in-U ;
...
open import ID.Syntax.Typing using (Ctx ; lookup ; HasType ; ConvTm ; WfCtx ; wf-extend ; ty-var ;
...
open import ID.Adequacy.Pi using (EqVal2-U-to-EqValTy2 ; AdqConv ; Adq ; transportVal2' ;
  Val2-U-to-ValTy2 ; sup-transport-EqVal2 ; adequacy-ty-Pi)
open import ID.Domain.Basic using (FinFun)
open import ID.Adequacy.Lam using (adequacyV-ty-Lam ; lamTypeTwoSub ; adequacy-ty-Lam)
open import ID.Adequacy.App using (adequacyV-ty-App ; adequacyV-subst1-cod ; AdSub2Rec ; AdConvSub2Rec)
open import ID.Adequacy.AppInj using (adequacyV-ty-App-inj ; adequacyV-subst1-cod-inj)
open import ID.Adequacy.ArgCore using (adequacyEqSub2-App-arg ; AdEqSub2Rec)
...
open import ID.Syntax.Substitution using (WtSub ; WtConvSub ; typing-ConvTm ; subst-HasType ;
...
open import ID.Model.Eval using (EvalRel-coh)
open import ID.Model.Soundness using (convSound ; convSound-inv)
open import ID.Validity.Core using (FinMem-Coherent)
open import ID.Model.EvalSubstitution using (EvalRel-unwk)
open import ID.Syntax.Typing using (extend)
...
adequacyV2-conv {G = G} {B = B} d1 d2 dB IH-V1 IH-E2 IH-VB
  ...
  let evA   = convSound-inv d2 rho fits a evB             -- EvalRel A rho a
      ca    = EvalRel-coh B rho a evB                     -- Coherent a
...
adequacyE2-conv {G = G} {B = B} d1 d2 dB IH-E1 IH-E2 IH-VB
    sigma sigma' rho crho vsσ vsσ' vcs tyσ tyσ' tycs fits wtσ wtσ' wtcs wfH u hu a evB fm =
  let evA   = convSound-inv d2 rho fits a evB             -- EvalRel A rho a
      ca    = EvalRel-coh B rho a evB                     -- Coherent a
      aU    = FinMem-a-in-U u a fm                        -- FinMem a UCode
  ...
  where
    ...
    buildEdgeEqB2 u0 v0 sel N1 N2 htN1 htN2 cvN eqvalN =
      let valN1    = Val2-from-EqVal2-first u0 b eqvalN
          ...
          vcs_ext  = ValidConvSub2-extend sigma sigma N1 N2 rho u0
                       ...
                       (transportEqVal2-tyval {A = A} sigma rho crho A-tyval b bU evAb u0 fm_u0_b eqvalN)
    ...
    buildEdgeEqB'2 u0 v0 sel N1 N2 htN1_A' htN2_A' cvN_A' eqvalN_A' =
      let fm_u0_b  = FinMemAllU-Selection b sel allU cf cb bU
          ...
          vcs_ext  = ValidConvSub2-extend sigma sigma N1 N2 rho u0
                       (ValidConvSub2-refl {G = G} vs)
                       (transportEqVal2-tyval {A = A} sigma rho crho A-tyval b bU evAb u0 fm_u0_b eqvalN_A)
    ...
    buildEdgeEq-σ u0 v0 sel N1 N2 htN1 htN2 cvN eqvalN =
      let valN1    = Val2-from-EqVal2-first u0 b eqvalN
          ...
          vcs_ext  = ValidConvSub2-extend sigma sigma N1 N2 rho u0
                       ...
                       (transportEqVal2-tyval {A = A} sigma rho crho A-tyval-σ b bU evAb u0 fm_u0_b eqvalN)
    ...
    buildEdgeCrossTy u0 v0 sel P htP valP =
      let fm_u0_b  = FinMemAllU-Selection b sel allU cf cb bU
          ...
          vcs_ext  = ValidConvSub2-extend sigma sigma' P P rho u0 vcs
                       (transportEqVal2-tyval {A = A} sigma rho crho A-tyval-σ b bU evAb u0 fm_u0_b
