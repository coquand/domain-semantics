...
module ID.Model.Soundness where
...
open S using (Nat ; zero ; suc ; Top ; tt ; Empty ; Pair ; mkSigma ; fst ; snd ;
              ...
              nil ; cons)
open import ID.Domain.Kernel using (LeCode ; LeCode-Bot ; LeCode-refl ;
...
open import ID.Syntax.Typing using (Ctx ; empty ; extend ; lookup ;
  ...
  conv-Id ; conv-Ref ; conv-J ; conv-J-beta)

-- All hard lemmas come from LemmaForTSSigma (0 postulates).
import ID.Model.SoundnessLemmas as LTS
...
LeCode-Bot-eq UCode (IdCode t x y) le ()
LeCode-Bot-eq UCode (RefEl r) le ()
LeCode-Bot-eq (FunEl g) (IdCode t x y) le ()
LeCode-Bot-eq (FunEl g) (RefEl r) le ()
LeCode-Bot-eq (PiCode a f) (IdCode t x y) le ()
LeCode-Bot-eq (PiCode a f) (RefEl r) le ()
LeCode-Bot-eq (IdCode t x y) Bot ()
LeCode-Bot-eq (IdCode t x y) UCode le ()
LeCode-Bot-eq (IdCode t x y) (FunEl h) le ()
LeCode-Bot-eq (IdCode t x y) (PiCode b g) le ()
LeCode-Bot-eq (IdCode t x y) (IdCode t2 x2 y2) le ()
LeCode-Bot-eq (IdCode t x y) (RefEl r) le ()
LeCode-Bot-eq (RefEl r) Bot ()
LeCode-Bot-eq (RefEl r) UCode le ()
LeCode-Bot-eq (RefEl r) (FunEl h) le ()
LeCode-Bot-eq (RefEl r) (PiCode b g) le ()
LeCode-Bot-eq (RefEl r) (IdCode t x y) le ()
LeCode-Bot-eq (RefEl r) (RefEl r2) le ()
...
mutual
  ...
  -- conv-Id: from LTS.InvConv-Id
  convSound' (conv-Id {A = A} {A' = A'} {a = a} {a' = a'} {b = b} {b' = b'}
    _ _ _ cA ca cb) rho fits =
    LTS.InvConv-Id A A' a a' b b' rho fits
      (convSound' cA rho fits)
      (convSound' ca rho fits)
      (convSound' cb rho fits)

  -- conv-Ref: from LTS.InvConv-Ref
  convSound' (conv-Ref {A = A} {a = a} {a' = a'} dA _ ca) rho fits =
    LTS.InvConv-Ref A a a' rho fits
      (theorem1 dA rho fits)
      (convSound' ca rho fits)

  -- conv-J: from LTS.InvConv-J
  convSound' (conv-J {A = A} {a = a} {b = b} {C = C} {C' = C'} {d = d} {d' = d'}
    {p = p} {p' = p'} _ _ _ _ _ _ cC cd cp) rho fits =
    LTS.InvConv-J A a b C C' d d' p p' rho fits
      (convSound' cC rho fits)
      (convSound' cd rho fits)
      (convSound' cp rho fits)

  -- conv-J-beta (ML): from LTS.InvConv-J-beta
  convSound' (conv-J-beta {A = A} {a0 = a0} {C = C} {d = d} dA da0 dC dd) rho fits =
    LTS.InvConv-J-beta A a0 C d rho fits
      (theorem1 dA rho fits)
      (theorem1 da0 rho fits)
      (theorem1 dC rho fits)
      (theorem1 dd rho fits)
  ...
  -- ty-Id
  theorem1 (ty-Id {A = A} {a = a} {b = b} dA da db) rho fits u ev =
    LTS.InvTyp-Id A a b rho fits
      (theorem1 dA rho fits) (theorem1 da rho fits) (theorem1 db rho fits)
      u ev

  -- ty-Ref
  theorem1 (ty-Ref {A = A} {a = a} dA da) rho fits u ev =
    LTS.InvTyp-Ref A a rho fits
      (theorem1 dA rho fits) (theorem1 da rho fits)
      u ev

  -- ty-J (ML): value is  ⟦d⟧ at the proof witness; needs p's and d's soundness.
  theorem1 (ty-J {A = A} {a = a} {b = b} {C = C} {d = d} {p = p} dA da db dC dd dp)
    rho fits u ev =
    LTS.InvTyp-J A a b C d p rho fits (theorem1 dp rho fits) (theorem1 dd rho fits) u ev
