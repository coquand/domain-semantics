...
------------------------------------------------------------------------
-- EvalSubstitution.agda (MIN — Pi+U only)
--
-- Architecture:
...
module ID.Model.EvalSubstitution where

import ID.Domain.Basic as S
open S using (Nat ; zero ; suc ; Top ; tt ; Empty ; Pair ; mkSigma ; fst ; snd ;
              Sigma ; Eq ; refl ; Eq-transport ; Eq-sym ;
              FinEl ; Bot ; UCode ; FunEl ; PiCode ; IdCode ; RefEl ; FinFun ; isPos)
open import ID.Domain.Kernel using (LeCode ; LeCode-refl ; LeCode-trans ;
  ...
  FinMem ; FinMem-coh-u ; FinMem-coh-a ; FinMem-a-in-U ; finMem-bot-from ;
  ...
  NotBot-Sup-Comp)
open import ID.Model.Eval using (EnvApprox ; emptyEnv ; extendEnv ; lookupEnv ;
  EvalRel ; JBranch ; EvalRel-coh ; CoherentEnv ; lookupEnv-coh ; EvalRel-down ;
  EvalRel-mon-env ; JBranch-mon-env ; EnvLe ; EnvLe-refl ;
...
open import ID.Model.Selection using (Edge ; EdgeIn ; here ; there ; Selection ;
...
JBranch-ren : {n m : Nat} (r : Ren n m)
  (C d : Expr n) (rho : EnvApprox n) (rho' : EnvApprox m) ->
  RenAgree r rho rho' -> (c w : FinEl) ->
  JBranch C d rho c w -> JBranch (renExpr r C) (renExpr r d) rho' c w
...
EvalRel-ren r (Pi A B) rho rho' eq (IdCode t u v) ()
EvalRel-ren r (Pi A B) rho rho' eq (RefEl w) ()

-- App new result codes
EvalRel-ren r (App M N) rho rho' eq (IdCode a' b' c')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren r N rho rho' eq v evN)
                      (EvalRel-ren r M rho rho' eq _ evM))
EvalRel-ren r (App M N) rho rho' eq (RefEl w')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren r N rho rho' eq v evN)
                      (EvalRel-ren r M rho rho' eq _ evM))

-- Lam cross
EvalRel-ren r (Lam A M) rho rho' eq (IdCode t u v) ()
EvalRel-ren r (Lam A M) rho rho' eq (RefEl w) ()

-- Id
EvalRel-ren r (Id A a b) rho rho' eq Bot ev = tt
EvalRel-ren r (Id A a b) rho rho' eq (IdCode t u v) ev =
  mkSigma (fst ev) (mkSigma (EvalRel-ren r A rho rho' eq t (fst (snd ev)))
    (mkSigma (EvalRel-ren r a rho rho' eq u (fst (snd (snd ev))))
             (EvalRel-ren r b rho rho' eq v (snd (snd (snd ev))))))
EvalRel-ren r (Id A a b) rho rho' eq UCode ()
EvalRel-ren r (Id A a b) rho rho' eq (FunEl g) ()
EvalRel-ren r (Id A a b) rho rho' eq (PiCode c f) ()
EvalRel-ren r (Id A a b) rho rho' eq (RefEl w) ()

-- Ref
EvalRel-ren r (Ref a) rho rho' eq Bot ev = tt
EvalRel-ren r (Ref a) rho rho' eq (RefEl w) ev = EvalRel-ren r a rho rho' eq w ev
EvalRel-ren r (Ref a) rho rho' eq UCode ()
EvalRel-ren r (Ref a) rho rho' eq (FunEl g) ()
EvalRel-ren r (Ref a) rho rho' eq (PiCode c f) ()
EvalRel-ren r (Ref a) rho rho' eq (IdCode t u v) ()

-- J
EvalRel-ren r (J C d p) rho rho' eq c (mkSigma w (mkSigma evP jb)) =
  mkSigma w (mkSigma (EvalRel-ren r p rho rho' eq w evP)
                     (JBranch-ren r C d rho rho' eq c w jb))

JBranch-ren r C d rho rho' eq c Bot            jb = jb
JBranch-ren r C d rho rho' eq c UCode          ()
JBranch-ren r C d rho rho' eq c (FunEl g)      ()
JBranch-ren r C d rho rho' eq c (PiCode e f)   ()
JBranch-ren r C d rho rho' eq c (IdCode t u v) ()
JBranch-ren r C d rho rho' eq c (RefEl w')     jb =
  EvalRel-ren r d rho rho' eq (FunEl (cons (mkSigma w' c) nil)) jb
...
JBranch-ren-inv : {n m : Nat} (r : Ren n m)
  (C d : Expr n) (rho : EnvApprox n) (rho' : EnvApprox m) ->
  RenAgree r rho rho' -> (c w : FinEl) ->
  JBranch (renExpr r C) (renExpr r d) rho' c w -> JBranch C d rho c w
...
EvalRel-ren-inv r (Pi A B) rho rho' eq (IdCode t u v) ()
EvalRel-ren-inv r (Pi A B) rho rho' eq (RefEl w) ()

-- App new result codes
EvalRel-ren-inv r (App M N) rho rho' eq (IdCode a' b' c')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren-inv r N rho rho' eq v evN)
                      (EvalRel-ren-inv r M rho rho' eq _ evM))
EvalRel-ren-inv r (App M N) rho rho' eq (RefEl w')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren-inv r N rho rho' eq v evN)
                      (EvalRel-ren-inv r M rho rho' eq _ evM))

-- Lam cross
EvalRel-ren-inv r (Lam A M) rho rho' eq (IdCode t u v) ()
EvalRel-ren-inv r (Lam A M) rho rho' eq (RefEl w) ()

-- Id
EvalRel-ren-inv r (Id A a b) rho rho' eq Bot ev = tt
EvalRel-ren-inv r (Id A a b) rho rho' eq (IdCode t u v) ev =
  mkSigma (fst ev) (mkSigma (EvalRel-ren-inv r A rho rho' eq t (fst (snd ev)))
    (mkSigma (EvalRel-ren-inv r a rho rho' eq u (fst (snd (snd ev))))
             (EvalRel-ren-inv r b rho rho' eq v (snd (snd (snd ev))))))
EvalRel-ren-inv r (Id A a b) rho rho' eq UCode ()
EvalRel-ren-inv r (Id A a b) rho rho' eq (FunEl g) ()
EvalRel-ren-inv r (Id A a b) rho rho' eq (PiCode c f) ()
EvalRel-ren-inv r (Id A a b) rho rho' eq (RefEl w) ()

-- Ref
EvalRel-ren-inv r (Ref a) rho rho' eq Bot ev = tt
EvalRel-ren-inv r (Ref a) rho rho' eq (RefEl w) ev = EvalRel-ren-inv r a rho rho' eq w ev
EvalRel-ren-inv r (Ref a) rho rho' eq UCode ()
EvalRel-ren-inv r (Ref a) rho rho' eq (FunEl g) ()
EvalRel-ren-inv r (Ref a) rho rho' eq (PiCode c f) ()
EvalRel-ren-inv r (Ref a) rho rho' eq (IdCode t u v) ()

-- J
EvalRel-ren-inv r (J C d p) rho rho' eq c (mkSigma w (mkSigma evP jb)) =
  mkSigma w (mkSigma (EvalRel-ren-inv r p rho rho' eq w evP)
                     (JBranch-ren-inv r C d rho rho' eq c w jb))

JBranch-ren-inv r C d rho rho' eq c Bot            jb = jb
JBranch-ren-inv r C d rho rho' eq c UCode          ()
JBranch-ren-inv r C d rho rho' eq c (FunEl g)      ()
JBranch-ren-inv r C d rho rho' eq c (PiCode e f)   ()
JBranch-ren-inv r C d rho rho' eq c (IdCode t u v) ()
JBranch-ren-inv r C d rho rho' eq c (RefEl w')     jb =
  EvalRel-ren-inv r d rho rho' eq (FunEl (cons (mkSigma w' c) nil)) jb
...
JBranch-subst : {h g : Nat} (sigma : Sub h g)
  (C d : Expr g) (rho : EnvApprox h) (rho' : EnvApprox g) ->
  CoherentEnv rho -> SubRel sigma rho rho' -> (c w : FinEl) ->
  JBranch C d rho' c w -> JBranch (substExpr sigma C) (substExpr sigma d) rho c w
...
EvalRel-subst sigma (Pi A B) rho rho' crho sr (IdCode t u v) ()
EvalRel-subst sigma (Pi A B) rho rho' crho sr (RefEl w) ()

-- App new result codes
EvalRel-subst sigma (App M N) rho rho' crho sr (IdCode a' b' c')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst sigma N rho rho' crho sr v evN)
                      (EvalRel-subst sigma M rho rho' crho sr _ evM))
EvalRel-subst sigma (App M N) rho rho' crho sr (RefEl w')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst sigma N rho rho' crho sr v evN)
                      (EvalRel-subst sigma M rho rho' crho sr _ evM))

-- Lam cross
EvalRel-subst sigma (Lam A M) rho rho' crho sr (IdCode t u v) ()
EvalRel-subst sigma (Lam A M) rho rho' crho sr (RefEl w) ()

-- Id
EvalRel-subst sigma (Id A a b) rho rho' crho sr Bot ev = tt
EvalRel-subst sigma (Id A a b) rho rho' crho sr (IdCode t u v) ev =
  mkSigma (fst ev) (mkSigma (EvalRel-subst sigma A rho rho' crho sr t (fst (snd ev)))
    (mkSigma (EvalRel-subst sigma a rho rho' crho sr u (fst (snd (snd ev))))
             (EvalRel-subst sigma b rho rho' crho sr v (snd (snd (snd ev))))))
EvalRel-subst sigma (Id A a b) rho rho' crho sr UCode ()
EvalRel-subst sigma (Id A a b) rho rho' crho sr (FunEl g) ()
EvalRel-subst sigma (Id A a b) rho rho' crho sr (PiCode c f) ()
EvalRel-subst sigma (Id A a b) rho rho' crho sr (RefEl w) ()

-- Ref
EvalRel-subst sigma (Ref a) rho rho' crho sr Bot ev = tt
EvalRel-subst sigma (Ref a) rho rho' crho sr (RefEl w) ev =
  EvalRel-subst sigma a rho rho' crho sr w ev
EvalRel-subst sigma (Ref a) rho rho' crho sr UCode ()
EvalRel-subst sigma (Ref a) rho rho' crho sr (FunEl g) ()
EvalRel-subst sigma (Ref a) rho rho' crho sr (PiCode c f) ()
EvalRel-subst sigma (Ref a) rho rho' crho sr (IdCode t u v) ()

-- J
EvalRel-subst sigma (J C d p) rho rho' crho sr c (mkSigma w (mkSigma evP jb)) =
  mkSigma w (mkSigma (EvalRel-subst sigma p rho rho' crho sr w evP)
                     (JBranch-subst sigma C d rho rho' crho sr c w jb))

JBranch-subst sigma C d rho rho' crho sr c Bot            jb = jb
JBranch-subst sigma C d rho rho' crho sr c UCode          ()
JBranch-subst sigma C d rho rho' crho sr c (FunEl g)      ()
JBranch-subst sigma C d rho rho' crho sr c (PiCode e f)   ()
JBranch-subst sigma C d rho rho' crho sr c (IdCode t u v) ()
JBranch-subst sigma C d rho rho' crho sr c (RefEl w')     jb =
  EvalRel-subst sigma d rho rho' crho sr (FunEl (cons (mkSigma w' c) nil)) jb
...
JBranch-subst-forward-max : {h g : Nat} (sigma : Sub h g)
  (C d : Expr g) (rho : EnvApprox h) (rho' : EnvApprox g) ->
  CoherentEnv rho -> CoherentEnv rho' -> MaxSubRel sigma rho rho' ->
  (c w : FinEl) ->
  JBranch (substExpr sigma C) (substExpr sigma d) rho c w -> JBranch C d rho' c w
...
EvalRel-subst-forward-max sigma (Pi A B) rho rho' crho crho' msr (IdCode t u v) ()
EvalRel-subst-forward-max sigma (Pi A B) rho rho' crho crho' msr (RefEl w) ()

-- App new result codes
EvalRel-subst-forward-max sigma (App M N) rho rho' crho crho' msr (IdCode a' b' c')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst-forward-max sigma N rho rho' crho crho' msr v evN)
                      (EvalRel-subst-forward-max sigma M rho rho' crho crho' msr _ evM))
EvalRel-subst-forward-max sigma (App M N) rho rho' crho crho' msr (RefEl w')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst-forward-max sigma N rho rho' crho crho' msr v evN)
                      (EvalRel-subst-forward-max sigma M rho rho' crho crho' msr _ evM))

-- Lam cross
EvalRel-subst-forward-max sigma (Lam A M) rho rho' crho crho' msr (IdCode t u v) ()
EvalRel-subst-forward-max sigma (Lam A M) rho rho' crho crho' msr (RefEl w) ()

-- Id
EvalRel-subst-forward-max sigma (Id A a b) rho rho' crho crho' msr Bot ev = tt
EvalRel-subst-forward-max sigma (Id A a b) rho rho' crho crho' msr (IdCode t u v) ev =
  mkSigma (fst ev) (mkSigma (EvalRel-subst-forward-max sigma A rho rho' crho crho' msr t (fst (snd ev)))
    (mkSigma (EvalRel-subst-forward-max sigma a rho rho' crho crho' msr u (fst (snd (snd ev))))
             (EvalRel-subst-forward-max sigma b rho rho' crho crho' msr v (snd (snd (snd ev))))))
EvalRel-subst-forward-max sigma (Id A a b) rho rho' crho crho' msr UCode ()
EvalRel-subst-forward-max sigma (Id A a b) rho rho' crho crho' msr (FunEl g) ()
EvalRel-subst-forward-max sigma (Id A a b) rho rho' crho crho' msr (PiCode c f) ()
EvalRel-subst-forward-max sigma (Id A a b) rho rho' crho crho' msr (RefEl w) ()

-- Ref
EvalRel-subst-forward-max sigma (Ref a) rho rho' crho crho' msr Bot ev = tt
EvalRel-subst-forward-max sigma (Ref a) rho rho' crho crho' msr (RefEl w) ev =
  EvalRel-subst-forward-max sigma a rho rho' crho crho' msr w ev
EvalRel-subst-forward-max sigma (Ref a) rho rho' crho crho' msr UCode ()
EvalRel-subst-forward-max sigma (Ref a) rho rho' crho crho' msr (FunEl g) ()
EvalRel-subst-forward-max sigma (Ref a) rho rho' crho crho' msr (PiCode c f) ()
EvalRel-subst-forward-max sigma (Ref a) rho rho' crho crho' msr (IdCode t u v) ()

-- J
EvalRel-subst-forward-max sigma (J C d p) rho rho' crho crho' msr c (mkSigma w (mkSigma evP jb)) =
  mkSigma w (mkSigma (EvalRel-subst-forward-max sigma p rho rho' crho crho' msr w evP)
                     (JBranch-subst-forward-max sigma C d rho rho' crho crho' msr c w jb))

JBranch-subst-forward-max sigma C d rho rho' crho crho' msr c Bot            jb = jb
JBranch-subst-forward-max sigma C d rho rho' crho crho' msr c UCode          ()
JBranch-subst-forward-max sigma C d rho rho' crho crho' msr c (FunEl g)      ()
JBranch-subst-forward-max sigma C d rho rho' crho crho' msr c (PiCode e f)   ()
JBranch-subst-forward-max sigma C d rho rho' crho crho' msr c (IdCode t u v) ()
JBranch-subst-forward-max sigma C d rho rho' crho crho' msr c (RefEl w')     jb =
  EvalRel-subst-forward-max sigma d rho rho' crho crho' msr (FunEl (cons (mkSigma w' c) nil)) jb
...
Comp-Bot-right (IdCode t u v)  = tt
Comp-Bot-right (RefEl w)       = tt
...
JBranch-fwd :
  {h g : Nat} (sigma : Sub h g)
  (C d : Expr g) (rho : EnvApprox h) (c w : FinEl) ->
  CoherentEnv rho ->
  JBranch (substExpr sigma C) (substExpr sigma d) rho c w ->
  Sigma (EnvApprox g) (\ rho' ->
    Pair (CoherentEnv rho')
    (Pair (SubRel sigma rho rho')
          (JBranch C d rho' c w)))
...
foldEdgeFwd sigma rho M accRho crho cAcc srAcc a (cons p ps) body =
  ...
  in mkSigma rho' (mkSigma crho' (mkSigma srho'
       (mkSigma (EnvLe-trans accRho midRho rho' leAcc leMid)
         (\ q ein -> edgeCase {M0 = M} {r = rho'} q ein
...
EvalRel-subst-forward-wit sigma (App M N) rho (IdCode a' b' c') crho
  (mkSigma v (mkSigma evN evM)) =
  let rN = EvalRel-subst-forward-wit sigma N rho v crho evN
      rM = EvalRel-subst-forward-wit sigma M rho _ crho evM
      rhoN = fst rN ; crhoN = fst (snd rN) ; srN = fst (snd (snd rN))
  ...
  in mkSigma rho' (mkSigma crho' (mkSigma sr'
       (mkSigma v (mkSigma (EvalRel-mon-env N rhoN rho' v evN' leN)
                           (EvalRel-mon-env M rhoM rho' _ evM' leM)))))
EvalRel-subst-forward-wit sigma (App M N) rho (RefEl w') crho
  (mkSigma v (mkSigma evN evM)) =
  let rN = EvalRel-subst-forward-wit sigma N rho v crho evN
      rM = EvalRel-subst-forward-wit sigma M rho _ crho evM
      rhoN = fst rN ; crhoN = fst (snd rN) ; srN = fst (snd (snd rN))
...
EvalRel-subst-forward-wit sigma (Lam A M) rho (FunEl g) crho ev =
  ...
  where
    ...
    sel-body-from-edges rho' crho' a aU0 .nil cft bodyMap .Bot .Bot sel-nil =
      mkSigma Bot (mkSigma tt (mkSigma (finMem-bot-from a aU0) (EvalRel-Bot M (extendEnv rho' Bot))))
...
EvalRel-subst-forward-wit sigma (Pi A B) rho (PiCode a f) crho ev =
  ...
  where
    ...
    pi-sel-body rho' crho' a' a'U0 .nil cft bodyMap .Bot .Bot sel-nil =
      mkSigma Bot (mkSigma tt (mkSigma (finMem-bot-from a' a'U0) (EvalRel-Bot B (extendEnv rho' Bot))))
...
EvalRel-subst-forward-wit sigma (Pi A B) rho (IdCode t u v) crho ()
EvalRel-subst-forward-wit sigma (Pi A B) rho (RefEl w) crho ()

-- Lam cross
EvalRel-subst-forward-wit sigma (Lam A M) rho (IdCode t u v) crho ()
EvalRel-subst-forward-wit sigma (Lam A M) rho (RefEl w) crho ()

-- Id  (combine the three witness environments for A@t, a@u, b@v)
EvalRel-subst-forward-wit sigma (Id A a b) rho Bot crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent (mkSigma (SubRel-botEnv sigma rho) tt))
EvalRel-subst-forward-wit sigma (Id A a b) rho (IdCode t u v) crho ev =
  let rA = EvalRel-subst-forward-wit sigma A rho t crho (fst (snd ev))
      ra = EvalRel-subst-forward-wit sigma a rho u crho (fst (snd (snd ev)))
      rb = EvalRel-subst-forward-wit sigma b rho v crho (snd (snd (snd ev)))
      rhoA = fst rA ; crhoA = fst (snd rA) ; srA = fst (snd (snd rA)) ; evA = snd (snd (snd rA))
      rha = fst ra ; crha = fst (snd ra) ; sra = fst (snd (snd ra)) ; eva = snd (snd (snd ra))
      rhb = fst rb ; crhb = fst (snd rb) ; srb = fst (snd (snd rb)) ; evb = snd (snd (snd rb))
      c1 = combineFwd sigma rho crho rhoA rha crhoA crha srA sra
      rho1 = fst c1 ; crho1 = fst (snd c1) ; sr1 = fst (snd (snd c1))
      leA1 = fst (snd (snd (snd c1))) ; lea1 = snd (snd (snd (snd c1)))
      c2 = combineFwd sigma rho crho rho1 rhb crho1 crhb sr1 srb
      rho2 = fst c2 ; crho2 = fst (snd c2) ; sr2 = fst (snd (snd c2))
      le12 = fst (snd (snd (snd c2))) ; leb2 = snd (snd (snd (snd c2)))
      evA' = EvalRel-mon-env A rhoA rho2 t evA (EnvLe-trans rhoA rho1 rho2 leA1 le12)
      eva' = EvalRel-mon-env a rha rho2 u eva (EnvLe-trans rha rho1 rho2 lea1 le12)
      evb' = EvalRel-mon-env b rhb rho2 v evb leb2
  in mkSigma rho2 (mkSigma crho2 (mkSigma sr2
       (mkSigma (fst ev) (mkSigma evA' (mkSigma eva' evb')))))
EvalRel-subst-forward-wit sigma (Id A a b) rho UCode crho ()
EvalRel-subst-forward-wit sigma (Id A a b) rho (FunEl g) crho ()
EvalRel-subst-forward-wit sigma (Id A a b) rho (PiCode c f) crho ()
EvalRel-subst-forward-wit sigma (Id A a b) rho (RefEl w) crho ()

-- Ref
EvalRel-subst-forward-wit sigma (Ref a) rho Bot crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent (mkSigma (SubRel-botEnv sigma rho) tt))
EvalRel-subst-forward-wit sigma (Ref a) rho (RefEl w) crho ev =
  EvalRel-subst-forward-wit sigma a rho w crho ev
EvalRel-subst-forward-wit sigma (Ref a) rho UCode crho ()
EvalRel-subst-forward-wit sigma (Ref a) rho (FunEl g) crho ()
EvalRel-subst-forward-wit sigma (Ref a) rho (PiCode c f) crho ()
EvalRel-subst-forward-wit sigma (Ref a) rho (IdCode t u v) crho ()

-- J  (combine the scrutinee-witness env with the branch-witness env)
EvalRel-subst-forward-wit sigma (J C d p) rho c crho (mkSigma w (mkSigma evP jb)) =
  let rP = EvalRel-subst-forward-wit sigma p rho w crho evP
      rBr = JBranch-fwd sigma C d rho c w crho jb
      rhoP = fst rP ; crhoP = fst (snd rP) ; srP = fst (snd (snd rP)) ; evP' = snd (snd (snd rP))
      rhoBr = fst rBr ; crhoBr = fst (snd rBr) ; srBr = fst (snd (snd rBr)) ; jb' = snd (snd (snd rBr))
      comb = combineFwd sigma rho crho rhoP rhoBr crhoP crhoBr srP srBr
      rho' = fst comb ; crho' = fst (snd comb) ; sr' = fst (snd (snd comb))
      leP = fst (snd (snd (snd comb))) ; leBr = snd (snd (snd (snd comb)))
      evP'' = EvalRel-mon-env p rhoP rho' w evP' leP
      jb'' = JBranch-mon-env C d rhoBr rho' c w jb' leBr
  in mkSigma rho' (mkSigma crho' (mkSigma sr' (mkSigma w (mkSigma evP'' jb''))))

JBranch-fwd sigma C d rho c Bot            crho jb =
  mkSigma botEnv (mkSigma botEnv-Coherent (mkSigma (SubRel-botEnv sigma rho) jb))
JBranch-fwd sigma C d rho c UCode          crho ()
JBranch-fwd sigma C d rho c (FunEl g)      crho ()
JBranch-fwd sigma C d rho c (PiCode e f)   crho ()
JBranch-fwd sigma C d rho c (IdCode t u v) crho ()
JBranch-fwd sigma C d rho c (RefEl w')     crho jb =
  EvalRel-subst-forward-wit sigma d rho (FunEl (cons (mkSigma w' c) nil)) crho jb
