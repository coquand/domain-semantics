...
mutual
  ...
  -- ty-Pi-Prop
  ren-HasType {r = r} rt wfH (ty-Pi-Prop d1 d2) =
    let d1' = ren-HasType rt wfH d1
  ...
  -- ty-Sigma
  ren-HasType {r = r} rt wfH (ty-Sigma d1 d2) =
    let d1' = ren-HasType rt wfH d1
    in ty-Sigma d1' (ren-HasType (liftRen-RenTypes rt) (wf-extend d1') d2)

  -- ty-MkPair
  ren-HasType {r = r} rt wfH
    (ty-MkPair {A = A} {B = B} {M = M} {N = N} d1 d2 d3 d4) =
    let d1' = ren-HasType rt wfH d1
    in ty-MkPair d1' (ren-HasType (liftRen-RenTypes rt) (wf-extend d1') d2)
                 (ren-HasType rt wfH d3)
                 (Eq-transport (\ T -> HasType _ (renExpr r N) T)
                   (ren-subst1 r B M)
                   (ren-HasType rt wfH d4))

  -- ty-Fst
  ren-HasType {r = r} rt wfH (ty-Fst d1 d2 d3) =
    let d1' = ren-HasType rt wfH d1
    in ty-Fst d1' (ren-HasType (liftRen-RenTypes rt) (wf-extend d1') d2)
              (ren-HasType rt wfH d3)

  -- ty-Snd
  ren-HasType {r = r} rt wfH
    (ty-Snd {A = A} {B = B} {M = M} d1 d2 d3) =
    let d1' = ren-HasType rt wfH d1
    in Eq-transport (\ T -> HasType _ (Snd (renExpr r M)) T)
         (Eq-sym (ren-subst1 r B (Fst M)))
         (ty-Snd d1' (ren-HasType (liftRen-RenTypes rt) (wf-extend d1') d2)
                 (ren-HasType rt wfH d3))
  ...
  ren-ConvTm rt wfH (conv-Prop d1 d2 d3) =
    conv-Prop (ren-HasType rt wfH d1)
              (ren-HasType rt wfH d2)
              (ren-HasType rt wfH d3)

  -- conv-Prop-U
  ren-ConvTm rt wfH (conv-Prop-U d) =
    conv-Prop-U (ren-ConvTm rt wfH d)
  ...
  -- conv-Sigma
  ren-ConvTm {r = r} rt wfH (conv-Sigma {A = A} d1 d2) =
    let dA = fst (typing-ConvTm d1)
        dA' = ren-HasType rt wfH dA
        wfH' = wf-extend dA'
        rt' = liftRen-RenTypes rt
    in conv-Sigma (ren-ConvTm rt wfH d1)
                  (ren-ConvTm rt' wfH' d2)

  -- conv-beta-fst
  ren-ConvTm {r = r} rt wfH
    (conv-beta-fst {A = A} {B = B} {M = M} {N = N} d1 d2 d3 d4) =
    let d1' = ren-HasType rt wfH d1
        wfH' = wf-extend d1'
        rt' = liftRen-RenTypes rt
    in conv-beta-fst d1'
  ...
  -- conv-beta-snd
  ren-ConvTm {r = r} rt wfH
    (conv-beta-snd {A = A} {B = B} {M = M} {N = N} d1 d2 d3 d4) =
    let d1' = ren-HasType rt wfH d1
        wfH' = wf-extend d1'
        rt' = liftRen-RenTypes rt
    in Eq-transport
         (\ T -> ConvTm _ (Snd (MkPair (renExpr r M) (renExpr r N)))
                           (renExpr r N) T)
         (Eq-sym (ren-subst1 r B M))
         (conv-beta-snd d1'
           (ren-HasType rt' wfH' d2)
           (ren-HasType rt wfH d3)
           (Eq-transport (\ T -> HasType _ (renExpr r N) T)
  ...
  -- conv-pair-eta
  ren-ConvTm {r = r} rt wfH
    (conv-pair-eta {A = A} {B = B} {M = M} d1 d2 d3) =
    let d1' = ren-HasType rt wfH d1
        wfH' = wf-extend d1'
        rt' = liftRen-RenTypes rt
    in conv-pair-eta d1'
  ...
  -- conv-MkPair-fst
  ren-ConvTm {r = r} rt wfH
    (conv-MkPair-fst {A = A} {B = B} {M = M} {M' = M'} {N = N} d1 d2 d3 d4) =
    let d1' = ren-HasType rt wfH d1
        wfH' = wf-extend d1'
        rt' = liftRen-RenTypes rt
    in conv-MkPair-fst d1'
         (ren-HasType rt' wfH' d2)
         (ren-ConvTm rt wfH d3)
         (Eq-transport (\ T -> HasType _ (renExpr r N) T)
           (ren-subst1 r B M)
           (ren-HasType rt wfH d4))
  ...
  -- conv-MkPair-snd
  ren-ConvTm {r = r} rt wfH
    (conv-MkPair-snd {A = A} {B = B} {M = M} {N = N} {N' = N'} d1 d2 d3 d4) =
    let d1' = ren-HasType rt wfH d1
        wfH' = wf-extend d1'
        rt' = liftRen-RenTypes rt
    in conv-MkPair-snd d1'
         (ren-HasType rt' wfH' d2)
         (ren-HasType rt wfH d3)
         (Eq-transport (\ T -> ConvTm _ (renExpr r N) (renExpr r N') T)
           (ren-subst1 r B M)
           (ren-ConvTm rt wfH d4))

  -- conv-Fst
  ren-ConvTm {r = r} rt wfH
    (conv-Fst {A = A} {B = B} {M = M} {M' = M'} d1 d2 d3) =
    let d1' = ren-HasType rt wfH d1
        wfH' = wf-extend d1'
        rt' = liftRen-RenTypes rt
    in conv-Fst d1'
  ...
  -- conv-Snd
  ren-ConvTm {r = r} rt wfH
    (conv-Snd {A = A} {B = B} {M = M} {M' = M'} d1 d2 d3) =
    let d1' = ren-HasType rt wfH d1
        wfH' = wf-extend d1'
        rt' = liftRen-RenTypes rt
    in Eq-transport
         (\ T -> ConvTm _ (Snd (renExpr r M)) (Snd (renExpr r M')) T)
         (Eq-sym (ren-subst1 r B (Fst M)))
         (conv-Snd d1'
           (ren-HasType rt' wfH' d2)
           (ren-ConvTm rt wfH d3))
  ...
  -- ty-Sigma
  subst-HasType {sigma = sigma} ws wfH (ty-Sigma {A = A} d1 d2) =
    let d1' = subst-HasType ws wfH d1
    in ty-Sigma d1' (subst-HasType (liftSub-WtSub ws wfH d1) (wf-extend d1') d2)

  -- ty-MkPair
  subst-HasType {sigma = sigma} ws wfH
    (ty-MkPair {A = A} {B = B} {M = M} {N = N} d1 d2 d3 d4) =
    let d1' = subst-HasType ws wfH d1
        d4' = subst-HasType ws wfH d4
        -- d4' : HasType H (substExpr sigma N) (substExpr sigma (subst1 B M))
        -- Need: HasType H (substExpr sigma N) (subst1 (substExpr (liftSub sigma) B) (substExpr sigma M))
        d4'' = Eq-transport (\ T -> HasType _ (substExpr sigma N) T)
                 (Eq-sym (subst-subst1-comm sigma B M))
                 d4'
    in ty-MkPair d1' (subst-HasType (liftSub-WtSub ws wfH d1) (wf-extend d1') d2)
                 (subst-HasType ws wfH d3) d4''

  -- ty-Fst
  subst-HasType {sigma = sigma} ws wfH (ty-Fst d1 d2 d3) =
    let d1' = subst-HasType ws wfH d1
    in ty-Fst d1' (subst-HasType (liftSub-WtSub ws wfH d1) (wf-extend d1') d2)
              (subst-HasType ws wfH d3)
  ...
  subst-HasType {sigma = sigma} ws wfH
    ...
    let d1' = subst-HasType ws wfH d1
    in Eq-transport
         (\ T -> HasType _ (Snd (substExpr sigma M)) T)
         (subst-subst1-comm sigma B (Fst M))
         (ty-Snd d1' (subst-HasType (liftSub-WtSub ws wfH d1) (wf-extend d1') d2)
                 (subst-HasType ws wfH d3))
  ...
  subst-ConvTm ws wfH (conv-Prop d1 d2 d3) =
    conv-Prop (subst-HasType ws wfH d1)
              (subst-HasType ws wfH d2)
              (subst-HasType ws wfH d3)

  -- conv-Prop-U
  subst-ConvTm ws wfH (conv-Prop-U d) =
    conv-Prop-U (subst-ConvTm ws wfH d)
  ...
  -- conv-Sigma
  subst-ConvTm {sigma = sigma} ws wfH (conv-Sigma {A = A} d1 d2) =
    let dA = fst (typing-ConvTm d1)
        dA' = subst-HasType ws wfH dA
    in conv-Sigma (subst-ConvTm ws wfH d1)
                  (subst-ConvTm (liftSub-WtSub ws wfH dA) (wf-extend dA') d2)
  ...
  subst-ConvTm {sigma = sigma} ws wfH
    ...
    let d1' = subst-HasType ws wfH d1
        ...
        ws' = liftSub-WtSub ws wfH d1
        d4' = subst-HasType ws wfH d4
        eq  = subst-subst1-comm sigma B M
        eqs = Eq-sym eq
        d4'' = Eq-transport (\ T -> HasType _ (substExpr sigma N) T) eqs d4'
        r = conv-beta-snd d1'
              (subst-HasType ws' wfH' d2)
              (subst-HasType ws wfH d3)
              d4''
    in Eq-transport
         (\ T -> ConvTm _ (Snd (MkPair (substExpr sigma M) (substExpr sigma N)))
                           (substExpr sigma N) T)
         eq r
  ...
  subst-ConvTm {sigma = sigma} ws wfH
    ...
    let d1' = subst-HasType ws wfH d1
        wfH' = wf-extend d1'
        ws' = liftSub-WtSub ws wfH d1
        d4' = subst-ConvTm ws wfH d4
        d4'' = Eq-transport (\ T -> ConvTm _ (substExpr sigma N) (substExpr sigma N') T)
                 (Eq-sym (subst-subst1-comm sigma B M))
                 d4'
    in conv-MkPair-snd d1'
         (subst-HasType ws' wfH' d2)
  ...
  subst-ConvTm {sigma = sigma} ws wfH
    ...
    let d1' = subst-HasType ws wfH d1
        ...
        r = conv-Snd d1'
              (subst-HasType ws' wfH' d2)
              (subst-ConvTm ws wfH d3)
        eq = subst-subst1-comm sigma B (Fst M)
    in Eq-transport
         (\ T -> ConvTm _ (Snd (substExpr sigma M)) (Snd (substExpr sigma M')) T)
         eq r
  ...
  -- conv-Prop
  typing-ConvTm (conv-Prop d1 d2 d3) = mkSigma d2 d3

  -- conv-Prop-U
  typing-ConvTm (conv-Prop-U d) =
    let mkSigma dM dN = typing-ConvTm d
    in mkSigma (ty-Prop-U dM) (ty-Prop-U dN)
  ...
  -- conv-Sigma
  typing-ConvTm (conv-Sigma d1 d2) =
    let dA  = fst (typing-ConvTm d1)
        dA' = snd (typing-ConvTm d1)
        dB  = fst (typing-ConvTm d2)
        dB' = ctx-conv-HasType dA dA' d1 (snd (typing-ConvTm d2))
    in mkSigma (ty-Sigma dA dB) (ty-Sigma dA' dB')

  -- conv-beta-fst
  typing-ConvTm (conv-beta-fst d1 d2 d3 d4) =
    mkSigma (ty-Fst d1 d2 (ty-MkPair d1 d2 d3 d4)) d3

  -- conv-beta-snd
  typing-ConvTm (conv-beta-snd {A = A} {B = B} {M = M} {N = N} d1 d2 d3 d4) =
    let htPair = ty-MkPair d1 d2 d3 d4
        -- ty-Snd gives: HasType G (Snd (MkPair M N)) (subst1 B (Fst (MkPair M N)))
        htSnd = ty-Snd d1 d2 htPair
        -- conv-beta-fst gives: ConvTm G (Fst (MkPair M N)) M A
        betaFst = conv-beta-fst d1 d2 d3 d4
        -- subst1-cong: ConvTm G (subst1 B (Fst (MkPair M N))) (subst1 B M) U
        dFstPair = ty-Fst d1 d2 htPair
        convB = subst1-cong-ConvTm d1 d2 dFstPair d3 betaFst
        dBM = subst-HasType (subst1-WtSub d1 d3) (typing-WfCtx d1) d2
        htSnd' = ty-conv htSnd convB dBM
    in mkSigma htSnd' d4

  -- conv-pair-eta
  typing-ConvTm (conv-pair-eta {A = A} {B = B} {M = M} d1 d2 d3) =
    let dFst = ty-Fst d1 d2 d3
        dSnd = ty-Snd d1 d2 d3  -- HasType G (Snd M) (subst1 B (Fst M))
    in mkSigma (ty-MkPair d1 d2 dFst dSnd) d3

  -- conv-MkPair-fst
  typing-ConvTm (conv-MkPair-fst {A = A} {B = B} {M = M} {M' = M'} {N = N} dA dB d1 d2) =
    let dM  = fst (typing-ConvTm d1)
        dM' = snd (typing-ConvTm d1)
        -- d2 : HasType G N (subst1 B M)
        -- For the left side: ty-MkPair dA dB dM d2
        -- For the right side: need HasType G N (subst1 B M')
        wfG = typing-WfCtx dA
        convBM'M = subst1-cong-ConvTm dA dB dM' dM (conv-sym d1)
        dBM = subst-HasType (subst1-WtSub dA dM) wfG dB
        dBM' = subst-HasType (subst1-WtSub dA dM') wfG dB
        dN' = ty-conv d2 (subst1-cong-ConvTm dA dB dM dM' d1) dBM'
    in mkSigma (ty-MkPair dA dB dM d2) (ty-MkPair dA dB dM' dN')

  -- conv-MkPair-snd
  typing-ConvTm (conv-MkPair-snd {A = A} {B = B} {M = M} {N = N} {N' = N'} dA dB d1 d2) =
    let dN  = fst (typing-ConvTm d2)
        dN' = snd (typing-ConvTm d2)
    in mkSigma (ty-MkPair dA dB d1 dN) (ty-MkPair dA dB d1 dN')

  -- conv-Fst
  typing-ConvTm (conv-Fst {A = A} {B = B} dA dB d1) =
    let dM  = fst (typing-ConvTm d1)
        dM' = snd (typing-ConvTm d1)
    in mkSigma (ty-Fst dA dB dM) (ty-Fst dA dB dM')

  -- conv-Snd
  typing-ConvTm (conv-Snd {A = A} {B = B} {M = M} {M' = M'} dA dB d1) =
    let dM  = fst (typing-ConvTm d1)
        dM' = snd (typing-ConvTm d1)
        -- ty-Snd gives HasType G (Snd M') (subst1 B (Fst M'))
        -- Need HasType G (Snd M') (subst1 B (Fst M))
        -- conv-Fst gives ConvTm G (Fst M) (Fst M') A
        convFst = conv-Fst dA dB d1
        -- subst1-cong gives ConvTm G (subst1 B (Fst M)) (subst1 B (Fst M')) U
        wfG = typing-WfCtx dA
        dFstM  = ty-Fst dA dB dM
        dFstM' = ty-Fst dA dB dM'
        convB = subst1-cong-ConvTm dA dB dFstM dFstM' convFst
        dBFstM = subst-HasType (subst1-WtSub dA dFstM) wfG dB
        dBFstM' = subst-HasType (subst1-WtSub dA dFstM') wfG dB
        -- ty-conv to get HasType G (Snd M') (subst1 B (Fst M))
        sndD = ty-conv (ty-Snd dA dB dM') (conv-sym convB) dBFstM
    in mkSigma (ty-Snd dA dB dM) sndD
  ...
  typing-WfCtx (ty-Sigma d _)      = typing-WfCtx d
  typing-WfCtx (ty-MkPair d _ _ _) = typing-WfCtx d
  typing-WfCtx (ty-Fst d _ _)      = typing-WfCtx d
  typing-WfCtx (ty-Snd d _ _)      = typing-WfCtx d
  ...
  typing-type (ty-Sigma d1 _)         = ty-U (typing-WfCtx d1)
  typing-type (ty-MkPair d1 d2 _ _)   = ty-Sigma d1 d2
  typing-type (ty-Fst d1 _ _)         = d1
  typing-type (ty-Snd {B = B} d1 d2 d3) =
    subst-HasType (subst1-WtSub d1 (ty-Fst d1 d2 d3)) (typing-WfCtx d1) d2
  ...
  -- ty-Pi-Prop: mirrors ty-Pi but produces conv-Pi-Prop (result at Prop)
  ...
  -- ty-Sigma: mirrors ty-Pi
  subst-ConvTm-cross {H = H} {sigma = sigma} {sigma' = sigma'} (ty-Sigma {A = A} d1 d2) ws ws' wcs wfH =
    let sA   = substExpr sigma A
        dA'  = subst-HasType ws wfH d1
        ws2  = liftSub-WtSub ws wfH d1
        cvA  = subst-ConvTm-cross d1 ws ws' wcs wfH
        ...
        wfH' = wf-extend dA'
        ihB  = subst-ConvTm-cross d2 ws2 ws2'_ctx wcs2 wfH'
    in conv-Sigma cvA ihB

  -- ty-MkPair: two components, mirrors ty-Lam but simpler (no funext needed)
  subst-ConvTm-cross {H = H} {sigma = sigma} {sigma' = sigma'}
    (ty-MkPair {A = A} {B = B} {M = M} {N = N} d1 d2 d3 d4) ws ws' wcs wfH =
    let dA   = subst-HasType ws wfH d1
        dA'  = subst-HasType ws' wfH d1
        wfH' = wf-extend dA
        ws2  = liftSub-WtSub ws wfH d1
        dB   = subst-HasType ws2 wfH' d2
        cvA  = subst-ConvTm-cross d1 ws ws' wcs wfH
        ihM  = subst-ConvTm-cross d3 ws ws' wcs wfH
        -- d4 : HasType G N (subst1 B M)
        -- subst-ConvTm-cross d4 gives ConvTm H (Nσ) (Nσ') (subst1 B M)σ
        ihN_raw = subst-ConvTm-cross d4 ws ws' wcs wfH
        -- Transport type from (substExpr sigma (subst1 B M)) to (subst1 (substExpr (liftSub sigma) B) (substExpr sigma M))
        ihN = Eq-transport
                (\ T -> ConvTm _ (substExpr sigma N) (substExpr sigma' N) T)
                (Eq-sym (subst-subst1-comm sigma B M))
                ihN_raw
        -- Build: MkPair (Mσ) (Nσ) : Sigma (Aσ) (Bσ)
        dM   = subst-HasType ws wfH d3
        dN_raw = subst-HasType ws wfH d4
        dN   = Eq-transport (\ T -> HasType _ (substExpr sigma N) T)
                 (Eq-sym (subst-subst1-comm sigma B M)) dN_raw
        htP  = ty-MkPair dA dB dM dN
        -- Build: MkPair (Mσ') (Nσ') : Sigma (Aσ) (Bσ) via ty-conv
        dM'  = subst-HasType ws' wfH d3
        ws2' = liftSub-WtSub ws' wfH d1
        wfH'2 = wf-extend dA'
        dB'  = subst-HasType ws2' wfH'2 d2
        dN'_raw = subst-HasType ws' wfH d4
        dN'  = Eq-transport (\ T -> HasType _ (substExpr sigma' N) T)
                 (Eq-sym (subst-subst1-comm sigma' B M)) dN'_raw
        htP' = ty-MkPair dA' dB' dM' dN'
        -- Need HasType H (MkPair (Mσ') (Nσ')) (Sigma (Aσ) (Bσ))
        ws2'_ctx : WtSub (extend H (substExpr sigma A)) (extend _ A) (liftSub sigma')
        ws2'_ctx = \ i ->
          ctx-conv-HasType dA' dA (conv-sym cvA) (liftSub-WtSub ws' wfH d1 i)
        wcs2 = liftSub-WtConvSub ws wcs wfH d1
        cvB  = subst-ConvTm-cross d2 ws2 ws2'_ctx wcs2 wfH'
        cvB_ctx = ctx-conv-ConvTm dA dA' cvA (conv-sym cvB)
        htP'_conv = ty-conv htP' (conv-Sigma (conv-sym cvA) cvB_ctx) (ty-Sigma dA dB)
        -- conv-MkPair-fst: MkPair (Mσ) (Nσ) conv MkPair (Mσ') (Nσ) via first component
        step1 = conv-MkPair-fst dA dB ihM dN
        -- conv-MkPair-snd: MkPair (Mσ') (Nσ) conv MkPair (Mσ') (Nσ') via second component
        -- dM' is at type (Aσ'), need at type (Aσ) for conv-MkPair-snd
        dM'_conv = ty-conv dM' (conv-sym cvA) dA
        -- subst1-cong for B at Mσ vs Mσ'
        dBsM  = subst-HasType (subst1-WtSub dA dM) (typing-WfCtx dA) dB
        convBM'M = subst1-cong-ConvTm dA dB dM dM'_conv ihM
        dBsM' = subst-HasType (subst1-WtSub dA dM'_conv) (typing-WfCtx dA) dB
        -- ihN : ConvTm H (Nσ) (Nσ') (subst1 sB (Mσ))
        -- Need: ConvTm H (Nσ) (Nσ') (subst1 sB (Mσ'))
        ihN_at_M' = conv-conv ihN convBM'M dBsM'
        step2 = conv-MkPair-snd dA dB dM'_conv ihN_at_M'
    in conv-trans step1 step2

  -- ty-Fst
  subst-ConvTm-cross {sigma = sigma} {sigma' = sigma'}
    (ty-Fst {A = A} {B = B} d1 d2 d3) ws ws' wcs wfH =
    let dA   = subst-HasType ws wfH d1
        wfH' = wf-extend dA
        ws2  = liftSub-WtSub ws wfH d1
        dB   = subst-HasType ws2 wfH' d2
        ihM  = subst-ConvTm-cross d3 ws ws' wcs wfH
    in conv-Fst dA dB ihM

  -- ty-Snd
  subst-ConvTm-cross {sigma = sigma} {sigma' = sigma'}
    (ty-Snd {A = A} {B = B} {M = M} d1 d2 d3) ws ws' wcs wfH =
    let dA   = subst-HasType ws wfH d1
        wfH' = wf-extend dA
        ws2  = liftSub-WtSub ws wfH d1
        dB   = subst-HasType ws2 wfH' d2
        ihM  = subst-ConvTm-cross d3 ws ws' wcs wfH
        r    = conv-Snd dA dB ihM
        eq   = subst-subst1-comm sigma B (Fst M)
    in Eq-transport
         (\ T -> ConvTm _ (Snd (substExpr sigma M)) (Snd (substExpr sigma' M)) T)
         eq r
