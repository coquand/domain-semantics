...
We study a dependent type theory with a universe $\U:\U$ and dependent
function ($\Pi$) types, in the form mechanised in the \texttt{MIN/} Agda
development.  The main goal is a \emph{syntactic} result --- a statement about
the conversion (definitional-equality) judgment alone --- namely
\emph{injectivity of $\Pi$}:
\[
  \conveq{\Gamma}{\Pitype{x{:}A_0}{B_0}}{\Pitype{x{:}A_1}{B_1}}{\U}
  \;\Longrightarrow\;
...
has no normalisation, so the usual syntactic route to injectivity (via
confluence and normal forms) is unavailable.  We therefore prove the result
\emph{semantically}, by \emph{domain theory}: we build a domain-theoretic
model whose elements are finite approximations (finite elements of a Scott
domain), define a logical relation linking the model to the syntax, prove
\emph{adequacy} (every well-typed term is valid), and then read $\Pi$-injectivity
off the logical relation by evaluating at the bottom environment.  This is the
method of~\cite{CH2018}; the point of interest is that a purely syntactic
fact is obtained from the semantic model.

The presentation here is methodological.  The domain is the limit of an
increasing chain of finite stages, and we define the information order $\leq$
and the semantic membership $u : a$ accordingly --- \emph{by stratification}:
a family of relations $\lest{n}$, $\memst{\cdot}{n}{\cdot}$ indexed by a stage
$n\in\Nat$, each obtained from the one below, together with a \emph{stability}
lemma that lets us pass to the limit.  This avoids reasoning through
projections (see Section~\ref{sec:stages}), and the same idea organises the
logical relation.
...
\end{mathpar}
\emph{Convention.}  In \textsc{App-Fun}, \textsc{App-Arg}, and \textsc{Funext}
the ellipsis ``$\cdots$'' abbreviates the two formation premises
$\hastype{\Gamma}{A}{\U}$ and $\hastype{\Gamma, x{:}A}{B}{\U}$, which every
$\Pi$-rule carries; we leave them implicit to keep the rules readable.
...
\]
which are well-defined by Lemma~\ref{lem:stab}.  Since each relation is given
by recursion on the stage $n$, the construction is manifestly well-founded.
...
All of the following are established in the formalisation:
\begin{itemize}
...
functions hold $\Val$ only as \emph{stored data} --- hypotheses of the form
``for all $N$ with $\Val\;\Gamma\;N\;A\;u\;b$, \ldots'' --- never inspecting it,
so the records are well-formed.
