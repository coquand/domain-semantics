...
module MIN.Validity.Public where

open import MIN.Domain.Basic using (Nat ; zero ; suc ; max ; FinEl)
open import MIN.Syntax.Raw using (Expr)
open import MIN.Syntax.Typing using (Ctx ; HasType ; ConvTm)
open import MIN.Domain.Kernel using (Coherent)
open import MIN.Domain.Rank using (RANK)

-- relations + Red3 (Stage/Bundle/OpenRecords come along, harmlessly)
open import MIN.Validity.Stratified public

-- public records + (un)builders, under the old names
open import MIN.Adequacy.Records public
...
open import MIN.Validity.Mono using (MonoPack ; goodStage) public
open import MIN.Validity.Mono using (Red3-unique-Pi) public

-- property lemmas whose *-pub signature already matches the originals
open import MIN.Validity.Levels public
