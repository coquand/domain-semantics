...
module MIN.AdequacyHelpers where
...
open import MIN.PaperSemantics using (LeCode ; LeCode-Bot ; LeCode-refl ; LeCode-trans ; Coherent ; CoherentFun ; Comp ; Comp-down ; Sup ; LeCode-Sup-left ; LeCode-Sup-right ; Coherent-Sup ; EvalFun ; EvalFun-in-UCode ; Coherent-EvalFun ; EvalFun-mon-arg ; LeFunCode ; LeFunCode-refl ; FinMem ; FinMemFun ; FinMemAllU ; FinMem-a-in-U ; finMemUCode-Sup ; finMem-upward ; finMem-Sup-left ; finMem-Sup-right ; coh-from-aU ; FinMem-coh-u ; cft-from-cf ; CoherentFunTail ; CoherentFunTail-append ; mkCFT ; NotBot ; absurdEl)
open import MIN.Reduction using (Red ; mkRed ; Red-refl ; Red-hr ; HeadRed ; HeadRed-trans ; HeadRed-App ; HeadRed-strip-Pi ; headred-step ; headred-beta ; headred-refl ; subst-subst1-comm ; idSub ; substExpr-id)
open import MIN.RawSemantics using (EnvApprox ; emptyEnv ; extendEnv ; lookupEnv ; EvalRel ; Pi-edgewise ; EvalRel-coh ; CoherentEnv ; lookupEnv-coh ; EvalRel-Comp ; EvalRel-Sup ; EvalRel-down ; EvalRel-mon-env ; EnvLe ; EnvLe-refl)
open import MIN.RawSyntax using (Expr ; Var ; U ; Pi ; Lam ; App ; Fin ; fzero ; fsuc ; wkExpr ; subst1 ; Sub ; liftSub ; substExpr ; subst1Sub)
  renaming ()
open import MIN.TypingRules using (Ctx ; empty ; extend ; lookup ; HasType ; ConvTm ; WfCtx ; wf-empty ; wf-extend ; ty-var ; ty-conv ; ty-U ; ty-Pi ; ty-Lam ; ty-App ; conv-refl ; conv-sym ; conv-trans ; conv-conv ; conv-beta ; conv-Pi ; conv-funext ; conv-App-fun ; conv-App-arg)
open import MIN.ValidityPublic public
open import MIN.Validity using (Edge ; EdgeIn ; here ; there ; Red-unique-Pi ; FinMem-Coherent ; Selection ; sel-nil ; sel-skip ; sel-take ; Selection-le-EvalFun ; Coherent-Selection ; Coherent-Selection-val ; bU-from-cf-fmU)
...
open import MIN.Selection using (FinMemAllU-Selection ; FinMem-Selection-UCode ; FinMem-Selection ; FinMem-Selection-codomain ; selectionBelow)
open import MIN.TypingSemantics using (convSound ; convSound-inv ; convSound' ; theorem1 ; LeCode-Bot-eq)
open import MIN.LemmaForTS using (Fits ; Typed ; Fits-CoherentEnv)
open import MIN.EvalSubstitution using (EvalRel-subst1-backward ; EvalRel-wk ; EvalRel-unwk ; EvalRel-Pi-app-type ; EvalRel-Pi-body ; EvalRel-subst1-forward)

open import MIN.RawSyntax using (Ren ; liftRen ; renExpr ; wkRen)
open import MIN.SubstitutionLemma using (typing-ConvTm ; WtSub ; subst-HasType ; subst-ConvTm ; liftSub-WtSub ; subst1-WtSub ; typing-WfCtx ; typing-type ; ctx-conv-HasType ; ctx-conv-ConvTm ; subst1-cong-ConvTm ; wk-HasType ; wk-ConvTm ; WtConvSub ; subst-ConvTm-cross ; liftSub-WtConvSub)
...
EvalRel-bot (Var i)     rho = mkSigma tt (MIN.PaperSemantics.LeCode-Bot (lookupEnv i rho))
EvalRel-bot U           rho = mkSigma tt (MIN.PaperSemantics.LeCode-Bot UCode)
