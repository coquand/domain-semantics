...
------------------------------------------------------------------------
module MIN.Domain.RankSandwichFunBuild where
...
open import MIN.Domain.MemProps
...
open import MIN.Domain.FamU using ( FamToU ; mkFamToU ; edge-le )
open import MIN.Model.Selection using ( here )
open import MIN.Domain.RankSandwichCore using ( SStmt )
open import MIN.Domain.RankSandwichFam
  using ( FamOut ; famBuild ; compStep-cap ; compFun-cap
        ; lefun-append-mono-L ; lefun-append-mono-R ; finMemAllU-up )
open import MIN.Domain.RankSandwichFun using ( FunEdgeOut ; funEdge )
...
funBuild m ih z0 c0 d0 zf gb df g
  ...
  where
    ...
    zf-df = lefun-append-bf zf'r feT df (FunBuildOut.fb-zf-df rec) (mkSigma (FunEdgeOut.fe-T'df fe) tt)
      where
        open import MIN.Domain.RankSandwichFam using ( lefun-append-bf )
    -- T' <= EvalFun zf' k'
...
------------------------------------------------------------------------
open import MIN.Domain.Order using ( NotBot )
...
------------------------------------------------------------------------
open import MIN.Domain.Basic using ( Empty )
open import MIN.Domain.Order using ( LeCode-Bot )
open import MIN.Domain.MemUnfold
...
open import MIN.Domain.Membership using ( finMem-funel-mk )
open import MIN.Domain.RankSandwichCore using ( SOut )

exF : {A : Set} -> Empty -> A
exF ()

open import MIN.Domain.Order using ( cft-from-cf )
open import MIN.Domain.FamU using ( piU-intro )
