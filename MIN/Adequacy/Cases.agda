...
------------------------------------------------------------------------
-- MIN.Adequacy.Cases.agda
--
-- Helper for Hole 6: ty-MkPair cross-sub at (PairCode, SigmaCode).
-- Takes IH and sigEdgeEq as explicit arguments to avoid opacity.
------------------------------------------------------------------------

module MIN.Adequacy.Cases where
open import MIN.Adequacy.HeadRed public
open import MIN.Adequacy.Pi using (Adq ; AdqConv)
open import MIN.Adequacy.VE using (AdqE1)
...
open import MIN.Domain.Kernel using (LeCode ; LeCode-refl ; Coherent ; CoherentFun ; CoherentFunTail ; EvalFun ; Comp ; Sup ; LeCode-Sup-left ; LeCode-Sup-right ; Coherent-Sup ; EvalFun-in-UCode ; Coherent-EvalFun ; EvalFun-mon-arg ; FinMem ; FinMemFun ; FinMemAllU ; FinMem-a-in-U ; finMemUCode-Sup ; finMem-upward ; coh-from-aU ; FinMem-coh-u ; cft-from-cf ; NotBot ; absurdEl ; finMem-piU-dom ; finMem-piU-allU ; finMem-piU-cft ; finMem-funel-fun ; finMem-funel-coh ; finMem-funel-wf)
open import MIN.Syntax.Reduction using (Red ; mkRed ; Red-refl ; HeadRed ; headred-step ; headred-beta ; headred-refl ; subst-subst1-comm)
open import MIN.Model.Eval using (EnvApprox ; extendEnv ; EvalRel ; EvalRel-coh ; CoherentEnv ; EvalRel-Comp ; EvalRel-down ; EvalRel-mon-env ; EnvLe ; EnvLe-refl)
open import MIN.Syntax.Raw using (Expr ; Var ; U ; Pi ; Lam ; App ; Fin ; fzero ; fsuc ; wkExpr ; subst1 ; Sub ; liftSub ; substExpr ; subst1Sub)
  renaming ()
open import MIN.Syntax.Typing using (Ctx ; extend ; HasType ; ConvTm ; WfCtx ; wf-extend ; ty-var ; ty-conv ; ty-U ; conv-refl ; conv-sym ; conv-conv)
open import MIN.Validity.Core using (Selection ; Coherent-Selection ; Coherent-Selection-val ; FinMem-Coherent)
open import MIN.Validity.Core using () renaming (Red-unique-Pi to Red-unique-Pi2)
open import MIN.Model.Selection using (FinMemAllU-Selection ; selectionBelow ; FinMem-Selection ; FinMem-Selection-codomain)
open import MIN.Model.EvalSubstitution using (EvalRel-subst1-backward ; EvalRel-body-EvalFun ; EvalRel-Pi-app-type ; EvalRel-subst1-forward)
open import MIN.Model.Soundness using (convSound' ; theorem1)
open import MIN.Syntax.Substitution using (typing-ConvTm ; WtSub ; subst-HasType ; subst-ConvTm ; liftSub-WtSub ; typing-WfCtx ; typing-type ; subst1-cong-ConvTm ; WtConvSub ; subst-ConvTm-cross)
