...
module MIN.TypingSemantics where
...
open S using (Nat ; zero ; suc ; Top ; tt ; Empty ; Pair ; mkSigma ; fst ; snd ;
              ...
              nil ; cons)
open import MIN.PaperSemantics using (LeCode ; LeCode-Bot ; LeCode-refl ;
...
open import MIN.RawSyntax using (Expr ; Var ; U ; Pi ; Lam ; App ;
  Fin ; fzero ; fsuc ; wkExpr ; subst1)
open import MIN.TypingRules using (Ctx ; empty ; extend ; lookup ;
...
import MIN.LemmaForTS as LTS
open LTS using (Fits ; Typed ; InvTyp ; InvConv ;
  Fits-CoherentEnv ; Fits-var)
open import MIN.EvalSubstitution using (EvalRel-ren ; EvalRel-wk ; EvalRel-unwk ;
