...
-- Validity4.agda
--
-- Record-based validity relation for Pi + Sigma + U.
-- Uses Agda records with named fields instead of nested Sigma tuples.
--
-- 0 postulates.
------------------------------------------------------------------------

module Validity4 where
...
mutual
  ...
  -- RValPair: term validity at (PairCode u' v', SigmaCode b f)
  {-# NO_POSITIVITY_CHECK #-}
  record RValPair {n : Nat} (G : Ctx n) (M A : Expr n) (u' v' : FinEl) (b : FinEl) (f : FinFun) : Set where
    inductive
    field
      domA   : Expr n
      codB   : Expr (suc n)
      ...
      cohU   : Coherent u'
      fmU    : FinMem u' b
      valFst : Val2 G (Fst M) domA u' b
      valSnd : Val2 G (Snd M) (subst1 codB (Fst M)) v' (EvalFun f u')

  -- REqValPair: equality at (PairCode u' v', SigmaCode b f)
  {-# NO_POSITIVITY_CHECK #-}
  record REqValPair {n : Nat} (G : Ctx n) (M N A : Expr n) (u' v' : FinEl) (b : FinEl) (f : FinFun) : Set where
    inductive
    field
      domA    : Expr n
      codB    : Expr (suc n)
      ...
      cohU    : Coherent u'
      fmU     : FinMem u' b
      valFstM : Val2 G (Fst M) domA u' b
      valSndM : Val2 G (Snd M) (subst1 codB (Fst M)) v' (EvalFun f u')
      htFstN  : HasType G (Fst N) domA
      valFstN : Val2 G (Fst N) domA u' b
      valSndN : Val2 G (Snd N) (subst1 codB (Fst N)) v' (EvalFun f u')
      eqFst   : EqVal2 G (Fst M) (Fst N) domA u' b
  ...
  Val2-to-EqVal2 (PairCode u' v') (SigmaCode b f) (mkSigma vty vpair) =
    mkSigma vty (mkSigma vpair (mkSigma vpair (ValPair2-to-EqValPair2 u' v' b f vpair)))
  Val2-to-EqVal2 u (PairCode x y) v = tt

  ------------------------------------------------------------------
  -- ValTy2-to-EqValTy2: reflexivity for types
  ------------------------------------------------------------------

  ValPair2-to-EqValPair2 : {n : Nat} {G : Ctx n} {M A : Expr n}
    (u' v' : FinEl) (b : FinEl) (f : FinFun) ->
    RValPair G M A u' v' b f -> REqValPair G M M A u' v' b f
  ValPair2-to-EqValPair2 u' v' b f vpair = record
    { domA    = RValPair.domA vpair
    ; codB    = RValPair.codB vpair
    ; red     = RValPair.red vpair
    ; htFstM  = RValPair.htFst vpair
    ; cohU    = RValPair.cohU vpair
    ; fmU     = RValPair.fmU vpair
    ; valFstM = RValPair.valFst vpair
    ; valSndM = RValPair.valSnd vpair
    ; htFstN  = RValPair.htFst vpair
    ; valFstN = RValPair.valFst vpair
    ; valSndN = RValPair.valSnd vpair
    ; eqFst   = Val2-to-EqVal2 u' b (RValPair.valFst vpair)
    }
  ...
  Val2-from-EqVal2-first u (FunEl h) ev = tt
  ...
  Val2-from-EqVal2-first (PairCode u' v') (SigmaCode b f) ev =
    mkSigma (fst ev) (fst (snd ev))
  Val2-from-EqVal2-first u (PairCode x y) ev = tt
  ...
  Val2-from-EqVal2-second u (FunEl h) ev = tt
  ...
  Val2-from-EqVal2-second (PairCode u' v') (SigmaCode b f) ev =
    mkSigma (fst ev) (fst (snd (snd ev)))
  Val2-from-EqVal2-second u (PairCode x y) ev = tt

  ------------------------------------------------------------------
  -- Signatures for complex transport functions
  -- (need restrictVal2 which requires ~200 lines of case splitting)
  -- These will be ported from Validity2.agda with identical logic.
  ------------------------------------------------------------------
  ...
  EqVal2-sym (PairCode u' v') (SigmaCode b f) cu ca ev =
    let vty   = fst ev
        ...
        vpN   = fst (snd (snd ev))
        eqp   = snd (snd (snd ev))
        A0    = REqValPair.domA eqp
        B0    = REqValPair.codB eqp
        redA  = REqValPair.red eqp
        cf    = snd ca
        eqFst' = EqVal2-sym u' b (RValPair.cohU vpM) (fst ca) (REqValPair.eqFst eqp)
        eqp'  = record
          { domA    = A0 ; codB = B0 ; red = redA
          ; htFstM  = REqValPair.htFstN eqp
          ; cohU    = REqValPair.cohU eqp
          ; fmU     = REqValPair.fmU eqp
          ; valFstM = REqValPair.valFstN eqp
          ; valSndM = REqValPair.valSndN eqp
          ; htFstN  = REqValPair.htFstM eqp
          ; valFstN = REqValPair.valFstM eqp
          ; valSndN = REqValPair.valSndM eqp
          ; eqFst   = eqFst'
          }
  ...
  EqVal2-trans (PairCode u' v') (SigmaCode b f) cu ca ev1 ev2 =
    let vty     = fst ev1
        ...
        Ax      = REqValPair.domA eqp1
        Bx      = REqValPair.codB eqp1
        redAx   = REqValPair.red eqp1
        Ay      = REqValPair.domA eqp2
        By      = REqValPair.codB eqp2
        redAy   = REqValPair.red eqp2
        uniq    = Red3-unique-Sigma redAx redAy
        eqA0    = fst uniq
        eqB0    = snd uniq
        eqFst1  = REqValPair.eqFst eqp1
        eqFst2  = REqValPair.eqFst eqp2
        eqFst2' = Eq-transport (\ X -> EqVal2 _ (Fst _) (Fst _) X u' b) (Eq-sym eqA0) eqFst2
        eqFst'  = EqVal2-trans u' b (RValPair.cohU vpM1) (fst ca) eqFst1 eqFst2'
        eqp'    = record
          { domA    = Ax ; codB = Bx ; red = redAx
          ; htFstM  = REqValPair.htFstM eqp1
          ; cohU    = REqValPair.cohU eqp1
          ; fmU     = REqValPair.fmU eqp1
          ; valFstM = REqValPair.valFstM eqp1
          ; valSndM = REqValPair.valSndM eqp1
          ; htFstN  = Eq-transport (\ X -> HasType _ _ X) (Eq-sym eqA0) (REqValPair.htFstN eqp2)
          ; valFstN = Eq-transport (\ X -> Val2 _ _ X u' b) (Eq-sym eqA0) (REqValPair.valFstN eqp2)
          ; valSndN = Eq-transport (\ X -> Val2 _ _ (subst1 X _) v' (EvalFun f u')) (Eq-sym eqB0)
                        (REqValPair.valSndN eqp2)
          ; eqFst   = eqFst'
          }
  ...
  downVal2 G M T (PairCode u' v') (SigmaCode b0 f0) (SigmaCode b1 f1) le mem ca0 ca1 src =
    let vty   = fst src
        vpair = snd src
        fmu'0 = fst (fst mem)
        v2Fst' = downVal2 G (Fst _) (RValPair.domA vpair) u' b0 b1 (fst le) fmu'0 (fst ca0) (fst ca1) (RValPair.valFst vpair)
        ctf0  = snd ca0
        ctf1  = snd (snd ca1)
        le_ef = EvalFun-mon f0 f1 u' ctf0 ctf1 (RValPair.cohU vpair) (snd le)
        ca0_ef = Coherent-EvalFun f0 u' ctf0 (RValPair.cohU vpair)
        ca1_ef = EvalFun-in-UCode f1 u' b1 ctf1 (RValPair.cohU vpair) (fst (snd ca1))
        v2Snd' = downVal2 G (Snd _) (subst1 (RValPair.codB vpair) (Fst _)) v' (EvalFun f0 u') (EvalFun f1 u')
                   le_ef (snd (fst mem)) ca0_ef ca1_ef (RValPair.valSnd vpair)
        vty'  = downValTy2 G _ (SigmaCode b0 f0) (SigmaCode b1 f1) le (snd (snd mem)) ca1 vty
        vpair' = record
          { domA   = RValPair.domA vpair
          ; codB   = RValPair.codB vpair
          ; red    = RValPair.red vpair
          ; htFst  = RValPair.htFst vpair
          ; cohU   = RValPair.cohU vpair
          ; fmU    = fmu'0
          ; valFst = v2Fst'
          ; valSnd = v2Snd'
  ...
  downEqVal2 G M N T (PairCode u' v') (SigmaCode b0 f0) (SigmaCode b1 f1) le mem ca0 ca1 src =
    let vpairM = fst (snd src)
        ...
        ctf0  = snd ca0
        ctf1  = snd (snd ca1)
        cu'   = REqValPair.cohU eqp
        v2FstM' = downVal2 G (Fst M) (RValPair.domA vpairM) u' b0 b1 (fst le) fmu'0 (fst ca0) (fst ca1) (RValPair.valFst vpairM)
        ...
        v2SndM' = downVal2 G (Snd M) (subst1 (RValPair.codB vpairM) (Fst M)) v' (EvalFun f0 u') (EvalFun f1 u')
                    le_ef (snd (fst mem)) ca0_ef ca1_ef (RValPair.valSnd vpairM)
        v2FstN' = downVal2 G (Fst N) (RValPair.domA vpairN) u' b0 b1 (fst le) fmu'0 (fst ca0) (fst ca1) (RValPair.valFst vpairN)
        v2SndN' = downVal2 G (Snd N) (subst1 (RValPair.codB vpairN) (Fst N)) v' (EvalFun f0 u') (EvalFun f1 u')
                    le_ef (snd (fst mem)) ca0_ef ca1_ef (RValPair.valSnd vpairN)
        vty'  = downValTy2 G _ (SigmaCode b0 f0) (SigmaCode b1 f1) le (snd (snd mem)) ca1 (fst src)
        eqFst' = downEqVal2 G (Fst M) (Fst N) (REqValPair.domA eqp) u' b0 b1 (fst le) fmu'0 (fst ca0) (fst ca1) (REqValPair.eqFst eqp)
        -- Compute val for eqp' separately from the REqValPair fields
        v2FstM-eq = downVal2 G (Fst M) (REqValPair.domA eqp) u' b0 b1 (fst le) fmu'0 (fst ca0) (fst ca1) (REqValPair.valFstM eqp)
        v2SndM-eq = downVal2 G (Snd M) (subst1 (REqValPair.codB eqp) (Fst M)) v' (EvalFun f0 u') (EvalFun f1 u')
                      le_ef (snd (fst mem)) ca0_ef ca1_ef (REqValPair.valSndM eqp)
        v2FstN-eq = downVal2 G (Fst N) (REqValPair.domA eqp) u' b0 b1 (fst le) fmu'0 (fst ca0) (fst ca1) (REqValPair.valFstN eqp)
        v2SndN-eq = downVal2 G (Snd N) (subst1 (REqValPair.codB eqp) (Fst N)) v' (EvalFun f0 u') (EvalFun f1 u')
                      le_ef (snd (fst mem)) ca0_ef ca1_ef (REqValPair.valSndN eqp)
        vpairM' = record
          { domA = RValPair.domA vpairM ; codB = RValPair.codB vpairM ; red = RValPair.red vpairM
          ; htFst = RValPair.htFst vpairM ; cohU = RValPair.cohU vpairM ; fmU = fmu'0
          ; valFst = v2FstM' ; valSnd = v2SndM'
          }
        vpairN' = record
          { domA = RValPair.domA vpairN ; codB = RValPair.codB vpairN ; red = RValPair.red vpairN
          ; htFst = RValPair.htFst vpairN ; cohU = RValPair.cohU vpairN ; fmU = fmu'0
          ; valFst = v2FstN' ; valSnd = v2SndN'
          }
        eqp'  = record
          { domA    = REqValPair.domA eqp ; codB = REqValPair.codB eqp ; red = REqValPair.red eqp
          ; htFstM  = REqValPair.htFstM eqp
          ; cohU    = cu'
          ; fmU     = fmu'0
          ; valFstM = v2FstM-eq
          ; valSndM = v2SndM-eq
          ; htFstN  = REqValPair.htFstN eqp
          ; valFstN = v2FstN-eq
          ; valSndN = v2SndN-eq
          ; eqFst   = eqFst'
          }
  ...
  upVal2 G M T (PairCode u' v') (SigmaCode b0 f0) (SigmaCode b1 f1) le mem0 mem1 ca0 ca1 src vta1 =
    let vty   = fst src
        ...
        fmu'0 = fst (fst mem0)
        fmu'1 = fst (fst mem1)
        uniq  = Red3-unique-Sigma (RValPair.red vpair) (RValTySigma.red vta1)
        sigEV1 : SigmaEdgeVal2 G (RValPair.domA vpair) (RValPair.codB vpair) b1 f1
        sigEV1 = Eq-transport (\ Y -> SigmaEdgeVal2 G (RValPair.domA vpair) Y b1 f1) (Eq-sym (snd uniq))
                   (Eq-transport (\ X -> SigmaEdgeVal2 G X (RValTySigma.codB vta1) b1 f1) (Eq-sym (fst uniq)) (RValTySigma.edgeV vta1))
        ctf0  = snd ca0
        ctf1  = snd ca1
        le_ef = EvalFun-mon f0 f1 u' ctf0 ctf1 (RValPair.cohU vpair) (snd le)
        ca0_ef = Coherent-EvalFun f0 u' ctf0 (RValPair.cohU vpair)
        ca1_ef = Coherent-EvalFun f1 u' ctf1 (RValPair.cohU vpair)
        ef1U  = EvalFun-in-UCode f1 u' b1 ctf1 (RValPair.cohU vpair) (fst (snd pf1))
        v2Fst' = upVal2 G (Fst _) (RValPair.domA vpair) u' b0 b1 (fst le) fmu'0 fmu'1 cb0 cb1 (RValPair.valFst vpair)
                   (Eq-transport (\ X -> ValTy2 G X b1) (Eq-sym (fst uniq)) (RValTySigma.valA vta1))
        v2Snd' = upVal2 G (Snd _) (subst1 (RValPair.codB vpair) (Fst _)) v' (EvalFun f0 u') (EvalFun f1 u')
                   le_ef (snd (fst mem0)) (snd (fst mem1)) ca0_ef ca1_ef (RValPair.valSnd vpair)
                   (let sb = selectionBelow f1 u' ctf1 (RValPair.cohU vpair)
                        u1 = fst sb
                        v1 = fst (snd sb)
                        sel1 = fst (snd (snd sb))
                        ...
                        valFst-u1 = restrictVal2 _ _ (RValPair.domA vpair) u' u1 b1 le-u1 fmu1-b1 fmu-b1 v2Fst'
                        vty-v1 = sigEV1 u1 v1 sel1 (Fst _) (RValPair.htFst vpair) valFst-u1
                    in Eq-transport (\ x -> ValTy2 G (subst1 (RValPair.codB vpair) (Fst _)) x) (Eq-sym eq-v1) vty-v1)
        vpair' = record
          { domA   = RValPair.domA vpair
          ; codB   = RValPair.codB vpair
          ; red    = RValPair.red vpair
  ...
  upEqVal2 G M N T (PairCode u' v') (SigmaCode b0 f0) (SigmaCode b1 f1) le mem0 mem1 ca0 ca1 src vta1 =
    let vpairM = fst (snd src)
        ...
        vpairM-eq : RValPair G M T u' v' b0 f0
        vpairM-eq = record { domA = REqValPair.domA eqp ; codB = REqValPair.codB eqp ; red = REqValPair.red eqp ; htFst = REqValPair.htFstM eqp ; cohU = REqValPair.cohU eqp ; fmU = REqValPair.fmU eqp ; valFst = REqValPair.valFstM eqp ; valSnd = REqValPair.valSndM eqp }
        valM-eq' = upVal2 G M T (PairCode u' v') (SigmaCode b0 f0) (SigmaCode b1 f1) le mem0 mem1 ca0 ca1 (mkSigma (fst src) vpairM-eq) vta1
        vpairN-eq : RValPair G N T u' v' b0 f0
        vpairN-eq = record { domA = REqValPair.domA eqp ; codB = REqValPair.codB eqp ; red = REqValPair.red eqp ; htFst = REqValPair.htFstN eqp ; cohU = REqValPair.cohU eqp ; fmU = REqValPair.fmU eqp ; valFst = REqValPair.valFstN eqp ; valSnd = REqValPair.valSndN eqp }
        valN-eq' = upVal2 G N T (PairCode u' v') (SigmaCode b0 f0) (SigmaCode b1 f1) le mem0 mem1 ca0 ca1 (mkSigma (fst src) vpairN-eq) vta1
        eqFst' = upEqVal2 G (Fst M) (Fst N) (REqValPair.domA eqp) u' b0 b1 (fst le) fmu'0 fmu'1 (fst ca0) (fst ca1) (REqValPair.eqFst eqp)
                   (Eq-transport (\ X -> ValTy2 G X b1) (Eq-sym (fst (Red3-unique-Sigma (REqValPair.red eqp) (RValTySigma.red vta1))))
                      (RValTySigma.valA vta1))
        eqp'   = record
          { domA    = REqValPair.domA eqp ; codB = REqValPair.codB eqp ; red = REqValPair.red eqp
          ; htFstM  = RValPair.htFst (snd valM-eq')
          ; cohU    = REqValPair.cohU eqp
          ; fmU     = fmu'1
          ; valFstM = RValPair.valFst (snd valM-eq')
          ; valSndM = RValPair.valSnd (snd valM-eq')
          ; htFstN  = RValPair.htFst (snd valN-eq')
          ; valFstN = RValPair.valFst (snd valN-eq')
          ; valSndN = RValPair.valSnd (snd valN-eq')
          ; eqFst   = eqFst'
          }
  ...
  Val2-EqValTy2-fwd (PairCode u' v') (SigmaCode b0 f0) cb eqv val =
    let vtyC  = fst eqv
        ...
        valFst-E = RValPair.valFst vpair
        ...
        vpair' = record
          ...
          ; valSnd = let valSnd-F = RValPair.valSnd vpair
                         ...
                         b0U' = RValTySigma.fmBU vtyC
                         fmu-f = FinMemAllU-Selection b0 sel-f fmU cf0 cb0 b0U'
                         valFst-uf = restrictVal2 _ _ E u' u-f b0 le-uf fmu-f (RValPair.fmU vpair) valFst-E'
  ...
  EqVal2-EqValTy2-fwd (PairCode u' v') (SigmaCode b0 f0) cb eqv ev =
    let valM-C  = Val2-from-EqVal2-first (PairCode u' v') (SigmaCode b0 f0) ev
        ...
        eqp    = snd (snd (snd ev))
        core   = snd (snd eqv)
        uniqE  = Red3-unique-Sigma (REqValPair.red eqp) (REqValTySigma.redM core)
        eqFst' = EqVal2-EqValTy2-fwd u' b0 (fst cb) (REqValTySigma.eqA core)
                   (Eq-transport (\ X -> EqVal2 _ (Fst _) (Fst _) X u' b0) (fst uniqE)
                     (REqValPair.eqFst eqp))
        -- htFstM needs to go from REqValPair.domA to REqValTySigma.domA' via convA
        vtyC  = fst eqv
        htAc  = RValTySigma.htA vtyC
        ...
        htE'  = Eq-transport (\ X -> HasType _ X _) (fst uniqC') htAc'
        htFstM-E = Eq-transport (\ X -> HasType _ _ X) (fst uniqE) (REqValPair.htFstM eqp)
        htFstM-E' = ty-conv htFstM-E (REqValTySigma.convA core) htE'
        htFstN-E = Eq-transport (\ X -> HasType _ _ X) (fst uniqE) (REqValPair.htFstN eqp)
        htFstN-E' = ty-conv htFstN-E (REqValTySigma.convA core) htE'
        -- Compute Val2-EqValTy2-fwd separately for REqValPair M/N fields
        vpairM-eq = record { domA = REqValPair.domA eqp ; codB = REqValPair.codB eqp ; red = REqValPair.red eqp ; htFst = REqValPair.htFstM eqp ; cohU = REqValPair.cohU eqp ; fmU = REqValPair.fmU eqp ; valFst = REqValPair.valFstM eqp ; valSnd = REqValPair.valSndM eqp }
        ...
        eqp'   = record
          ...
          ; htFstM  = RValPair.htFst (snd valM-eq-C')
          ; cohU    = REqValPair.cohU eqp
          ; fmU     = REqValPair.fmU eqp
          ; valFstM = RValPair.valFst (snd valM-eq-C')
          ; valSndM = RValPair.valSnd (snd valM-eq-C')
          ; htFstN  = RValPair.htFst (snd valN-eq-C')
          ; valFstN = RValPair.valFst (snd valN-eq-C')
          ; valSndN = RValPair.valSnd (snd valN-eq-C')
          ; eqFst   = eqFst'
          }
  ...
  restrictVal2 G M T (PairCode u' v') (PairCode u2 v2) (SigmaCode b f) le mem fmu src =
    let fmu2b = fst (fst mem)
        ...
        v2Fst' = restrictVal2 _ _ (RValPair.domA vpair) u' u2 b (fst le) fmu2b (RValPair.fmU vpair) (RValPair.valFst vpair)
        ...
        uniqS = Red3-unique-Sigma (RValPair.red vpair) (RValTySigma.red (fst src))
        eqAS = fst uniqS
        valFst-u1 = restrictVal2 _ _ (RValPair.domA vpair) u' u1 b le-u1 fmu1-b (RValPair.fmU vpair) (RValPair.valFst vpair)
        htFst-S = Eq-transport (\ X -> HasType _ _ X) eqAS (RValPair.htFst vpair)
        ...
        vty-efu' = Eq-transport (\ x -> ValTy2 _ (subst1 (RValPair.codB vpair) (Fst _)) x) (Eq-sym eq-v1) vty-v1'
        -- Step: restrict v'->v2 at ef u', then downVal2 to ef u2
        sndRestricted = restrictVal2 _ _ (subst1 (RValPair.codB vpair) (Fst _)) v' v2 (EvalFun f u') (snd le)
                    fmv2efu' fmv'efu' (RValPair.valSnd vpair)
        sndDown = downVal2 _ _ (subst1 (RValPair.codB vpair) (Fst _)) v2 (EvalFun f u2) (EvalFun f u') le_ef fmv2efu2 c_ef_u2 ef_u'_U sndRestricted
        vpair' = record
          { domA = RValPair.domA vpair ; codB = RValPair.codB vpair ; red = RValPair.red vpair
          ; htFst = RValPair.htFst vpair ; cohU = cu2 ; fmU = fmu2b
