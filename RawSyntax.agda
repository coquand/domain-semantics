...
open import Basic using (Nat ; zero ; suc ; Eq ; refl ; Eq-cong ; Eq-transport ; Eq-sym)
...
Eq-cong2-Fin : {n m : Nat} (c : Fin n -> Fin m -> Fin n) ->
  {a a' : Fin n} {b b' : Fin m} ->
  Eq a a' -> Eq b b' -> Eq (c a b) (c a' b')
Eq-cong2-Fin c refl refl = refl
...
------------------------------------------------------------------------
-- ren-subst: renaming after substitution
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- subst-subst: substitution composition
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- subst-var-ren: substitution by variables = renaming
------------------------------------------------------------------------

liftSub-var-ren : {n m : Nat} (r : Ren n m) ->
  (j : Fin (suc n)) ->
  Eq (liftSub (\ i -> Var (r i)) j) (Var (liftRen r j))
liftSub-var-ren r fzero    = refl
liftSub-var-ren r (fsuc i) = refl

subst-var-ren : {n m : Nat} (r : Ren n m) (e : Expr n) ->
...
subst-var-ren r (Pi A B)  =
  let ihA = subst-var-ren r A
      ihB = Eq-trans
              (substExpr-ext (liftSub (\ i -> Var (r i)))
                             (\ j -> Var (liftRen r j))
                             (liftSub-var-ren r) B)
              (subst-var-ren (liftRen r) B)
  in Eq-cong2-Expr Pi ihA ihB
subst-var-ren r (Lam A M) =
  let ihA = subst-var-ren r A
      ihM = Eq-trans
              (substExpr-ext (liftSub (\ i -> Var (r i)))
                             (\ j -> Var (liftRen r j))
                             (liftSub-var-ren r) M)
              (subst-var-ren (liftRen r) M)
  in Eq-cong2-Expr Lam ihA ihM
subst-var-ren r (App f a) =
  Eq-cong2-Expr App (subst-var-ren r f) (subst-var-ren r a)

wkExpr-is-subst : {n : Nat} (e : Expr n) ->
  Eq (substExpr (\ i -> Var (fsuc i)) e) (wkExpr e)
wkExpr-is-subst e = subst-var-ren wkRen e
