...
-- RawSyntaxSigma.agda
--
-- Raw syntax extended with Sigma types: Sigma, MkPair, Fst, Snd.
...
data Expr : Nat -> Set where
  ...
  Sigma  : {n : Nat} -> Expr n -> Expr (suc n) -> Expr n
  MkPair : {n : Nat} -> Expr n -> Expr n -> Expr n
...
renExpr r (Sigma A B)  = Sigma (renExpr r A) (renExpr (liftRen r) B)
renExpr r (MkPair a b) = MkPair (renExpr r a) (renExpr r b)
renExpr r (Fst M)      = Fst (renExpr r M)
renExpr r (Snd M)      = Snd (renExpr r M)
...
substExpr sigma (Sigma A B)  = Sigma (substExpr sigma A) (substExpr (liftSub sigma) B)
substExpr sigma (MkPair a b) = MkPair (substExpr sigma a) (substExpr sigma b)
substExpr sigma (Fst M)      = Fst (substExpr sigma M)
substExpr sigma (Snd M)      = Snd (substExpr sigma M)
...
substExpr-ext sigma tau ext (Sigma A B)  =
  Eq-cong2-Expr Sigma (substExpr-ext sigma tau ext A)
    (substExpr-ext (liftSub sigma) (liftSub tau) (liftSub-ext sigma tau ext) B)
substExpr-ext sigma tau ext (MkPair a b) =
  Eq-cong2-Expr MkPair (substExpr-ext sigma tau ext a)
    (substExpr-ext sigma tau ext b)
substExpr-ext sigma tau ext (Fst M)      =
  Eq-cong Fst (substExpr-ext sigma tau ext M)
substExpr-ext sigma tau ext (Snd M)      =
  Eq-cong Snd (substExpr-ext sigma tau ext M)

------------------------------------------------------------------------
-- Renaming extensionality and composition (needed before ren-subst)
------------------------------------------------------------------------
...
renExpr-ext r1 r2 ext (Sigma A B)  =
  Eq-cong2-Expr Sigma (renExpr-ext r1 r2 ext A)
    (renExpr-ext (liftRen r1) (liftRen r2) (liftRen-ext r1 r2 ext) B)
renExpr-ext r1 r2 ext (MkPair a b) =
  Eq-cong2-Expr MkPair (renExpr-ext r1 r2 ext a)
    (renExpr-ext r1 r2 ext b)
renExpr-ext r1 r2 ext (Fst M)      = Eq-cong Fst (renExpr-ext r1 r2 ext M)
renExpr-ext r1 r2 ext (Snd M)      = Eq-cong Snd (renExpr-ext r1 r2 ext M)
...
ren-ren r1 r2 (Sigma A B)  =
  let ihA = ren-ren r1 r2 A
      ...
      ihB' = renExpr-ext
               ...
               (\ { fzero -> refl ; (fsuc i) -> refl })
               B
  in Eq-cong2-Expr Sigma ihA (Eq-trans ihB ihB')
ren-ren r1 r2 (MkPair a b) =
  Eq-cong2-Expr MkPair (ren-ren r1 r2 a) (ren-ren r1 r2 b)
ren-ren r1 r2 (Fst M)      = Eq-cong Fst (ren-ren r1 r2 M)
ren-ren r1 r2 (Snd M)      = Eq-cong Snd (ren-ren r1 r2 M)
...
subst-ren sigma r (Sigma A B)  =
  let ihA = subst-ren sigma r A
      ...
      ihB' = substExpr-ext
               ...
               B
  in Eq-cong2-Expr Sigma ihA (Eq-trans ihB ihB')
subst-ren sigma r (MkPair a b) =
  Eq-cong2-Expr MkPair (subst-ren sigma r a) (subst-ren sigma r b)
subst-ren sigma r (Fst M)      = Eq-cong Fst (subst-ren sigma r M)
subst-ren sigma r (Snd M)      = Eq-cong Snd (subst-ren sigma r M)
...
ren-subst r sigma (Sigma A B)  =
  let ihA = ren-subst r sigma A
      ...
      ihB' = substExpr-ext
               ...
               B
  in Eq-cong2-Expr Sigma ihA (Eq-trans ihB ihB')
ren-subst r sigma (MkPair a b) =
  Eq-cong2-Expr MkPair (ren-subst r sigma a) (ren-subst r sigma b)
ren-subst r sigma (Fst M)      = Eq-cong Fst (ren-subst r sigma M)
ren-subst r sigma (Snd M)      = Eq-cong Snd (ren-subst r sigma M)
...
subst-subst tau sigma (Sigma A B)  =
  let ihA = subst-subst tau sigma A
      ihB = subst-subst (liftSub tau) (liftSub sigma) B
      ihB' = substExpr-ext
               (\ j -> substExpr (liftSub tau) (liftSub sigma j))
               ...
               B
  in Eq-cong2-Expr Sigma ihA (Eq-trans ihB ihB')
subst-subst tau sigma (MkPair a b) =
  Eq-cong2-Expr MkPair (subst-subst tau sigma a) (subst-subst tau sigma b)
subst-subst tau sigma (Fst M)      = Eq-cong Fst (subst-subst tau sigma M)
subst-subst tau sigma (Snd M)      = Eq-cong Snd (subst-subst tau sigma M)
