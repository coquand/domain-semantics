...
-- LemmaForTS.agda
--
-- Lemmas for TypingSemantics, building on RawSemantics.
--
-- Lam-L1: If u <= (Lam A M) rho and u is not Bot, then there exist
-- a and g' such that:
--   (1) EvalRel A rho a
--   (2) LeCode u (FunEl g')
--   (3) EvalRel (Lam A M) rho (FunEl g')
--   (4) For all (x,y) in g': FinMem x a and EvalRel M (rho,x) y
------------------------------------------------------------------------

module LemmaForTS where
...
-- If edge (x,v) is in g and x <= u, then v <= EvalFun g u.
------------------------------------------------------------------------
...
-- For each edge (u,v) in g, the replacement has (x,v) with x <= u.
-- So v <= EvalFun g' x <= EvalFun g' u.
------------------------------------------------------------------------
...
-- By induction on Selection, taking x = Sup of selected keys.
------------------------------------------------------------------------
...
-- sel-nil: u = Bot, v = Bot, take x = Bot
...
replaceKeys-selection-body M rho a (cons p g') .(Sup (fst p) u') .(Sup (snd p) v')
  ...
  let -- IH
      ...
      ca   = coh-from-aU a aU
      -- Comp (fst p) x': from comp-k and le-x'
...
--
-- If u <= (Lam A M) rho and u is not Bot, then there exist
-- a and g' such that:
--   (1) EvalRel A rho a
--   (2) FinMem a UCode
--   (3) LeCode u (FunEl g')
--   (4) EvalRel (Lam A M) rho (FunEl g')
--   (5) For all (x,y) in g': FinMem x a and EvalRel M (rho,x) y
------------------------------------------------------------------------
...
-- Typed M A rho u : there exist u ≤ u' ≤ M rho and a with FinMem u' a and ⟦A⟧ρ = a
...
------------------------------------------------------------------------
-- Part 2b: Key intermediate lemmas
------------------------------------------------------------------------
...
InvTyp-Lam A B M body-ih rho fits (FunEl g) ev =
  let crho = Fits-CoherentEnv rho fits
      ...
      -- Step 4: Build all needed properties
  ...
  where
    -- Abbreviation for the witness type
    ...
    -- For non-nil graphs, CoherentFunTail = CoherentFun
    -- We case-split on g to expose the cons structure
    ...
    -- CoherentFunTail for value-replaced graph
    rv-cft : (E : Expr (suc _)) ->
    ...
    -- Edgewise for f-graph (bi values)
    ...
    -- CoherentWith for f-graph (bi values)
    ...
    -- CoherentFunTail for f-graph (bi values)
    ...
    -- FinMemFun h a0 ff (full f-graph ff fixed, iterate over g to build h)
    rv-fmFun : (a0 : FinEl) ->
      (g : FinFun) ->
      ((p : Edge) -> EdgeIn p g ->
        Pair (FinMem (fst p) a0) (EvalRel M (extendEnv rho (fst p)) (snd p))) ->
...
InvTyp-App A B M N rho fits invM invN = invTyp-App-aux
  where
    ...
    -- Case split on h (the enlarged value from InvTyp on M)
    app-h-case :
    ...
    -- h = Bot: LeCode (FunEl _) Bot = Empty
    app-h-case w v nbv cv evN evM-sing Bot piaf () evM-h fm-h evPi
    -- h = UCode/PropCode/PiCode: FinMem impossible
    ...
    app-h-case w v nbv cv evN evM-sing (FunEl g') (PiCode a f) le-g' evM-g' fm-g' evPi =
      let -- Extract from FinMem (FunEl g') (PiCode a f)
          fmFun-g' = fst fm-g'
          cf-g'    = fst (snd fm-g')
          piU      = snd (snd fm-g')
          aU       = fst piU
          ...
          -- selbody-f applied: z <= x-f, FinMem z a', EvalRel B (rho,z) w-f
          ...
          -- EvalRel (subst1 B N) rho fu0
          evBN-fu0 = EvalRel-subst1-backward B N rho w fu0 crho evN evB-w-fu0
          -- EvalRel (App M N) rho v0: FunEl [(w,v0)] <= g' <= M rho
...
InvConv-beta {G = G} A B M N rho fits invN body-ih =
  ...
  where
    ...
    -- Helper: coherence of selection key from singleton tail
    sel-key-coh : (x0 v0 : FinEl) ->
      Selection nil x0 v0 -> Coherent x0
    sel-key-coh .Bot .Bot sel-nil = tt

    -- Backward nonbot helper
    ...
    conv-app-to-sub-nonbot u nbu ev =
      let dec = App-decompose (Lam A M) N rho u nbu ev
          ...
          -- Lam-edgewise on FunEl [(v,u)] <= Lam A M
          ew  = Lam-edgewise A M rho (cons (mkSigma v u) nil) evLam-sing
          a   = fst ew
          -- edge (v,u) witness: z <= v, FinMem z a, EvalRel M (rho,z) u
          ...
          -- EvalRel N rho z from EvalRel N rho v and z <= v
    ...
    -- Forward helper: builds Lam evidence for singleton graph
...
InvConv-funext {G = G} A B M N rho fits invM invN ih =
  ...
  where
    ...
    -- Build EvalRel (App (wkExpr F) (Var fzero)) (extendEnv rho x) vi
    -- from EvalRel F rho (FunEl [(x, vi)]) and Coherent x, for vi not Bot
    ...
    -- Extract from App(wk F)(v0) evidence: get w with EvalRel F rho (FunEl [(w, vi)])
    ...
    -- Per-edge transfer: given edge (ui,vi) in g' with g' <= F rho and
    -- FinMem (FunEl g') (PiCode a f), produce EvalRel F' rho (FunEl [(wi, vi)])
    -- using convDir.
    ...
    edge-transfer F F' convDir a f evA g' cfg fmf evF-g' p ein =
      ...
      where
        ...
        -- Extract edge-level CoherentFunTail data
    ...
    -- g-full is the full original graph (for EvalRel-down).
    -- For each edge (ui,vi) in g', produce EvalRel F' rho (FunEl [(ui,vi)])
    -- by going through edge-transfer (gives wi) then EvalRel-down back to ui.
    ...
    -- So folding singletons over g' reconstructs exactly FunEl g'.
    ...
    -- Case analysis on u' and piaf from InvTyp
    ...
    -- Main conversion direction (dispatches to conv-nonbot)
...
-- App congruence — function slot
-- Given f = f' : Pi A B and a : A, prove App f a = App f' a : B[a]
...
-- App congruence — argument slot
-- Given f : Pi A B and a = a' : A, prove App f a = App f a' : B[a]
...
InvConv-App-arg {G = G} A B f a a' rho fits invF invAA' =
  ...
  where
    ...
    -- InvTyp RHS: App f a' at type subst1 B a
    invLHS = InvTyp-App A B f a rho fits invF invA
...
InvTyp-Pi-Prop A B rho fits invA body-ih (PiCode b f) ev = result
  where
    ...
    -- Step 4: per-edge IH — for each edge (ui, vi) in f,
    -- get xi <= ui with FinMem xi a', EvalRel B (rho, xi) vi,
    -- then type vi using body-ih to get vi' >= vi with FinMem vi' PropCode
    -- Helper: extract FinMem y PropCode from EvalRel Prop (ext) x and FinMem y x
    ...
    le-vi-p : (p : Edge) -> (ein : EdgeIn p f) -> LeCode (snd p) (vi'-p p ein)
    le-vi-p p ein = fst (snd (snd (snd (snd (snd (edge-data p ein))))))
    vi'P-p : (p : Edge) -> (ein : EdgeIn p f) -> FinMem (vi'-p p ein) PropCode
    vi'P-p p ein = snd (snd (snd (snd (snd (snd (snd (edge-data p ein)))))))
    ...
    -- FinMem (PiCode b_new f') PropCode
