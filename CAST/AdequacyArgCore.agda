...
module CAST.AdequacyArgCore where
open import CAST.AdequacyHeadRed

open import CAST.AdequacyApp using (app-transport-EqVal2)
...
open import CAST.Reduction using (Red ; mkRed ; Red-refl ; HeadRed ; headred-step ; headred-beta ;
  headred-refl ; subst-subst1-comm)
open import CAST.RawSemantics using (EnvApprox ; extendEnv ; EvalRel ; EvalRel-coh ; CoherentEnv ;
  EvalRel-Comp ; EvalRel-down ; EvalRel-mon-env ; EnvLe ; EnvLe-refl)
open import CAST.RawSyntax using (Expr ; Var ; U ; Pi ; Lam ; App ; Fin ; fzero ; fsuc ; wkExpr ;
  subst1 ; Sub ; liftSub ; substExpr ; subst1Sub)
open import CAST.TypingRules using (Ctx ; extend ; HasType ; ConvTm ; WfCtx ; wf-extend ; ty-var ;
...
open import CAST.Selection using (FinMemAllU-Selection ; selectionBelow ; FinMem-Selection ;
  FinMem-Selection-codomain)
open import CAST.EvalSubstitution using (EvalRel-Pi-app-type ; EvalRel-subst1-forward)
open import CAST.TypingSemantics using (convSound' ; theorem1)
open import CAST.SubstitutionLemma using (typing-ConvTm ; WtSub ; subst-HasType ; subst-ConvTm ;
  ...
  where
    ...
    appEqVal-dispatch (FunEl g_big) (PiCode b_pi f_pi) lf evFb evPab fmba valba =
      let le_u1_vsel = fst lf
          ...
          uniq_fun = Red-unique-Pi2 (Red-refl {G = H} {M = Pi sA sB} {A = U}) (mkRed (Red3-hr-from-Pi red_fun))
...
adequacyEqSub2-App-arg dB df daa' sigma rho crho vs fits wtsub wfH IHfv IHBv IHaae (IdCode _ _) ev Bot evAc fm = tt
adequacyEqSub2-App-arg dB df daa' sigma rho crho vs fits wtsub wfH IHfv IHBv IHaae (IdCode _ _) ev UCode evAc fm = tt
adequacyEqSub2-App-arg dB df daa' sigma rho crho vs fits wtsub wfH IHfv IHBv IHaae (IdCode _ _) ev (FunEl _) evAc fm = tt
adequacyEqSub2-App-arg dB df daa' sigma rho crho vs fits wtsub wfH IHfv IHBv IHaae (IdCode _ _) ev (PiCode _ _) evAc fm = tt
adequacyEqSub2-App-arg dB df daa' sigma rho crho vs fits wtsub wfH IHfv IHBv IHaae (IdCode _ _) ev (IdCode _ _) evAc fm = tt
