...
module CAST.AdequacyAppInj where

open import CAST.AdequacyHeadRed

import CAST.Basic as S
open S using (Nat ; zero ; suc ; Top ; tt ; Empty ; Sigma ; mkSigma ; fst ; snd ; Pair ; Eq ; FinEl ; Bot ; UCode ; FunEl ; PiCode ; IdCode ; FinFun ; List ; nil ; cons)
open import CAST.PaperSemantics using (LeCode ; LeCode-refl ; Coherent ; CoherentFun ; CoherentFunTail ; EvalFun ; Comp ; Sup ; LeCode-Sup-left ; LeCode-Sup-right ; Coherent-Sup ; EvalFun-in-UCode ; Coherent-EvalFun ; EvalFun-mon-arg ; FinMem ; FinMemFun ; FinMemAllU ; FinMem-a-in-U ; finMemUCode-Sup ; finMem-upward ; coh-from-aU ; FinMem-coh-u ; cft-from-cf ; NotBot ; absurdEl ; finMem-piU-dom ; finMem-piU-allU ; finMem-piU-cft ; finMem-funel-fun ; finMem-funel-coh ; finMem-funel-wf)
open import CAST.Reduction using (Red ; mkRed ; Red-refl ; HeadRed ; headred-step ; headred-beta ; headred-refl ; subst-subst1-comm)
open import CAST.RawSemantics using (EnvApprox ; extendEnv ; EvalRel ; EvalRel-coh ; CoherentEnv ; EvalRel-Comp ; EvalRel-down ; EvalRel-mon-env ; EnvLe ; EnvLe-refl)
open import CAST.RawSyntax using (Expr ; Var ; U ; Pi ; Lam ; App ; Fin ; fzero ; fsuc ; wkExpr ; subst1 ; Sub ; liftSub ; substExpr ; subst1Sub)
open import CAST.TypingRules using (Ctx ; extend ; HasType ; ConvTm ; WfCtx ; wf-extend ; ty-var ; ty-conv ; ty-U ; conv-refl ; conv-sym ; conv-conv)
open import CAST.Validity using (Selection ; Coherent-Selection ; Coherent-Selection-val ; FinMem-Coherent)
open import CAST.Validity using () renaming (Red-unique-Pi to Red-unique-Pi2)
open import CAST.Selection using (FinMemAllU-Selection ; selectionBelow ; FinMem-Selection ; FinMem-Selection-codomain)
open import CAST.EvalSubstitution using (EvalRel-subst1-backward ; EvalRel-body-EvalFun ; EvalRel-Pi-app-type ; EvalRel-subst1-forward)
open import CAST.TypingSemantics using (convSound' ; theorem1)
open import CAST.SubstitutionLemma using (typing-ConvTm ; WtSub ; subst-HasType ; subst-ConvTm ; liftSub-WtSub ; typing-WfCtx ; typing-type ; subst1-cong-ConvTm ; WtConvSub ; subst-ConvTm-cross)
open import CAST.LemmaForTS using (Fits)
open import CAST.AdequacyApp using (AdSub2Rec ; AdConvSub2Rec ; app-transport-EqVal2)
open import CAST.AdequacyPi using (Val2-U-to-ValTy2)
  ...
  where
    ...
    appEqVal-dispatch (FunEl g_big) (PiCode b_pi f_pi) lf evFb evPab fmba eqvba =
      let le_u1_vsel = fst lf
          ...
          uniq_eqfun = Red-unique-Pi2 (Red-refl {G = H} {M = Pi sA sB} {A = U}) (mkRed (Red3-hr-from-Pi red_eqfun))
          ...
          uniq_sf' = Red-unique-Pi2 (Red-refl {G = H} {M = Pi sA sB} {A = U}) (mkRed (Red3-hr-from-Pi red_sf'))
          eqA_sf'  = fst uniq_sf'
          eqB_sf'  = snd uniq_sf'
...
adequacyV-ty-App-inj dA dB df da adSub2 sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH funcross argsingle argcross Bot hu a evA fm =
  EqVal2-Bot a
adequacyV-ty-App-inj dA dB df da adSub2 sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH funcross argsingle argcross (IdCode _ _) hu Bot evA fm = tt
adequacyV-ty-App-inj dA dB df da adSub2 sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH funcross argsingle argcross (IdCode _ _) hu UCode evA fm = tt
adequacyV-ty-App-inj dA dB df da adSub2 sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH funcross argsingle argcross (IdCode _ _) hu (FunEl _) evA fm = tt
adequacyV-ty-App-inj dA dB df da adSub2 sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH funcross argsingle argcross (IdCode _ _) hu (PiCode _ _) evA fm = tt
adequacyV-ty-App-inj dA dB df da adSub2 sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH funcross argsingle argcross (IdCode _ _) hu (IdCode _ _) evA fm = tt
