...
module CAST.ValidityMono where
...
open import CAST.TypingRules using (Ctx ; empty ; extend ;
  HasType ; ConvTm ;
  WfCtx ;
...
open import CAST.Reduction using (Red ; mkRed ; Red-hr ; HeadRed ; HeadRed1 ;
  headred-refl ; headred-step ; HeadRed-trans ;
  HeadRed-App ; HeadRed-NF-unique ; HeadRed1-not-Pi ;
  HeadRed-strip-Pi )
...
open import CAST.Validity using (Red-unique-Pi ;
  bU-from-cf-fmFun ;
  FinMem-Coherent)
open import CAST.SubstitutionLemma using (typing-ConvTm ; ctx-conv-ConvTm ; ctx-conv-HasType ;
  subst1-cong-ConvTm)
...
Pi≢cast : {n : Nat} {B : Expr n} {F : Expr (suc n)} {A B' p M0 : Expr n} ->
  Eq (RS.Pi B F) (RS.cast A B' p M0) -> Empty
Pi≢cast ()

cast-inj-M0 : {n : Nat} {A1 B1 p1 M01 A2 B2 p2 M02 : Expr n} ->
  Eq (RS.cast A1 B1 p1 M01) (RS.cast A2 B2 p2 M02) -> Eq M01 M02
cast-inj-M0 refl = refl

-- type indices T, T' are general (was U): red3-conv only re-indexes the type
-- and leaves the head reduction / target Pi unchanged, so uniqueness ignores it.
Red3-unique-Pi : {n : Nat} {G : Ctx n} {A B B' T T' : Expr n}
  {F : Expr (suc n)} {F' : Expr (suc n)} ->
  Red3 G A (Pi B F) T -> Red3 G A (Pi B' F') T' ->
  Pair (Eq B B') (Eq F F')
Red3-unique-Pi (red3-conv _ r1) r2 = Red3-unique-Pi r1 r2
Red3-unique-Pi r1 (red3-conv _ r2) = Red3-unique-Pi r1 r2
Red3-unique-Pi {n} {G} {A} {B} {B'} {T} {T'} {F} {F'} (mkRed3 h1 _) (mkRed3 h2 _) =
  Red-unique-Pi {n} {G} {A} {B} {B'} {F} {F'} (mkRed h1) (mkRed h2)
...
goodStage (suc n) = record
  ...
  where
    ...
    dV G M T u (IdCode _ _) _ _ _ _ _ _ = tt
    ...
    dEV G M N T u (IdCode _ _) _ _ _ _ _ _ = tt
    ...
    dVT G M (IdCode _ _) _ _ _ _ _ = tt
    dVT G M UCode        Bot            ()
    ...
    dEVT G M N (IdCode _ _) _ _ _ _ _ = tt
    ...
    uV G M T Bot _ (IdCode _ _) _ _ _ _ _ _ _ = tt
    uV G M T (IdCode _ _) UCode UCode _ _ _ _ _ _ _ = tt
    ...
    uEV G M N T Bot _ (IdCode _ _) _ _ _ _ _ _ _ = tt
    uEV G M N T (IdCode _ _) UCode UCode _ _ _ _ _ _ _ = tt
    ...
    rV G M T (IdCode _ _) Bot UCode _ _ _ _ = tt
    rV G M T u (IdCode _ _) UCode _ _ _ _ = tt
    rV G M T u u' (IdCode _ _) _ _ _ _ = tt
    ...
    rEV G M N T (IdCode _ _) Bot UCode _ _ _ _ = tt
    rEV G M N T u (IdCode _ _) UCode _ _ _ _ = tt
    rEV G M N T u u' (IdCode _ _) _ _ _ _ = tt
    ...
    vfe1 (IdCode _ _) UCode ev = tt
    vfe1 (IdCode _ _) (FunEl h) ev = tt
    vfe1 (IdCode _ _) (PiCode b f) ev = tt
    vfe1 u (IdCode a a₁) ev = tt
    ...
    vfe2 (IdCode _ _) UCode ev = tt
    vfe2 (IdCode _ _) (FunEl h) ev = tt
    vfe2 (IdCode _ _) (PiCode b f) ev = tt
    vfe2 u (IdCode a a₁) ev = tt
