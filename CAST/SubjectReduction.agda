...
module CAST.SubjectReduction where

open import CAST.Basic using ( Nat ; suc ; mkSigma ; fst ; snd ; Pair )
open import CAST.RawSyntax using ( Expr ; U ; Pi ; Lam ; App ; subst1 )
open import CAST.TypingRules using
  ( Ctx ; extend ; HasType ; ConvTm
  ; ty-conv ; ty-Pi ; ty-Lam ; ty-App
  ; conv-refl ; conv-trans )
open import CAST.Reduction using ( HeadRed1 ; headred-beta ; headred-app )
open import CAST.SubstitutionLemma using
  ( typing-WfCtx ; subst-HasType ; subst1-WtSub
  ; ctx-conv-HasType ; ctx-conv-ConvTm )
open import CAST.PiInjectivity using ( piInjectivity )
