...
module CAST.AdequacyApp where

open import CAST.AdequacyHeadRed
open import CAST.AdequacyCases using (adequacyConvSub2-App-core-body ; adequacySub2-App-core-body)
open import CAST.AdequacyPi using (Adq ; AdqConv ; Val2-U-to-ValTy2)

import CAST.Basic as S
open S using (Nat ; zero ; suc ; tt ; mkSigma ; fst ; snd ; FinEl ; Bot ; UCode ; FunEl ; PiCode ; IdCode ;
  FinFun ; List ; nil ; cons)
open import CAST.PaperSemantics using (LeCode ; Coherent ; coh-from-aU ; FinMem ; FinMem-coh-u ;
  Comp ; Sup ; LeCode-Sup-left ; LeCode-Sup-right ; Coherent-Sup ;
  finMemUCode-Sup ; finMem-upward)
open import CAST.RawSemantics using (EnvApprox ; extendEnv ; EvalRel ; CoherentEnv ;
  EvalRel-coh ; EvalRel-mon-env ; EvalRel-down ; EnvLe-refl)
open import CAST.RawSyntax using (Expr ; U ; App ; Pi ; subst1 ; Sub ; liftSub ; substExpr)
open import CAST.TypingRules using (Ctx ; extend ; HasType ; WfCtx)
open import CAST.SubstitutionLemma using (WtSub ; WtConvSub ; subst-HasType ; subst-ConvTm-cross)
...
open import CAST.Reduction using (subst-subst1-comm)
...
adequacyV-ty-App dA dB df da IHav IHBv IHfc IHac sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH Bot hu a evA fm =
  EqVal2-Bot a
adequacyV-ty-App dA dB df da IHav IHBv IHfc IHac sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH (IdCode _ _) hu Bot evA fm = tt
adequacyV-ty-App dA dB df da IHav IHBv IHfc IHac sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH (IdCode _ _) hu UCode evA fm = tt
adequacyV-ty-App dA dB df da IHav IHBv IHfc IHac sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH (IdCode _ _) hu (FunEl _) evA fm = tt
adequacyV-ty-App dA dB df da IHav IHBv IHfc IHac sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH (IdCode _ _) hu (PiCode _ _) evA fm = tt
adequacyV-ty-App dA dB df da IHav IHBv IHfc IHac sigma sigma' rho crho vs vs' vcs fits wtsub wtsub' wcs wfH (IdCode _ _) hu (IdCode _ _) evA fm = tt
...
adequacySub2-App dA dB d1 d2 IHf IHa IHB sigma rho crho vs fits wtsub wfH (IdCode _ _) cu ev Bot evAc fm = tt
adequacySub2-App dA dB d1 d2 IHf IHa IHB sigma rho crho vs fits wtsub wfH (IdCode _ _) cu ev UCode evAc fm = tt
adequacySub2-App dA dB d1 d2 IHf IHa IHB sigma rho crho vs fits wtsub wfH (IdCode _ _) cu ev (FunEl _) evAc fm = tt
adequacySub2-App dA dB d1 d2 IHf IHa IHB sigma rho crho vs fits wtsub wfH (IdCode _ _) cu ev (PiCode _ _) evAc fm = tt
adequacySub2-App dA dB d1 d2 IHf IHa IHB sigma rho crho vs fits wtsub wfH (IdCode _ _) cu ev (IdCode _ _) evAc fm = tt
