...
module CAST.FinMemStageUnfold where
...
open import CAST.FinMemStageShift
...
finMemC-bot-to (IdCode a b) mem = mem
...
finMemC-bot-from (IdCode a b) mem = mem
...
private
  ...
  -- RANK a / RANK b  <=  RANK (IdCode a b)
  bDomI : (a b : FinEl) -> Le (RANK a) (RANK (IdCode a b))
  bDomI a b = Le-suc (RANK a) (max (RANK a) (RANK b)) (Le-max-l (RANK a) (RANK b))

  bCodI : (a b : FinEl) -> Le (RANK b) (RANK (IdCode a b))
  bCodI a b = Le-suc (RANK b) (max (RANK a) (RANK b)) (Le-max-r (RANK a) (RANK b))

-- Id-type well-formedness:  finMemC (IdCode a b) UCode = Pair (a:U) (b:U)
finMemC-idU-dom : (a b : FinEl) -> finMemC (IdCode a b) UCode -> finMemC a UCode
finMemC-idU-dom a b mem =
  finMemC-from (RANK (IdCode a b)) a UCode (bDomI a b) (Le0 (RANK (IdCode a b))) (fst mem)

finMemC-idU-cod : (a b : FinEl) -> finMemC (IdCode a b) UCode -> finMemC b UCode
finMemC-idU-cod a b mem =
  finMemC-from (RANK (IdCode a b)) b UCode (bCodI a b) (Le0 (RANK (IdCode a b))) (snd mem)

finMemC-idU-mk : (a b : FinEl) ->
  finMemC a UCode -> finMemC b UCode -> finMemC (IdCode a b) UCode
finMemC-idU-mk a b dom cod =
  mkSigma (finMemC-to (RANK (IdCode a b)) a UCode (bDomI a b) (Le0 (RANK (IdCode a b))) dom)
          (finMemC-to (RANK (IdCode a b)) b UCode (bCodI a b) (Le0 (RANK (IdCode a b))) cod)
...
FinMem-coh-u (FunEl g)    (IdCode a b) ()
...
FinMem-coh-u (PiCode a f) (IdCode b d) ()
FinMem-coh-u (IdCode a b) UCode        mem =
  mkSigma (FinMem-coh-u a UCode (finMemC-idU-dom a b mem))
          (FinMem-coh-u b UCode (finMemC-idU-cod a b mem))
FinMem-coh-u (IdCode a b) Bot          ()
FinMem-coh-u (IdCode a b) (FunEl g)    ()
FinMem-coh-u (IdCode a b) (PiCode c g) ()
FinMem-coh-u (IdCode a b) (IdCode c d) ()
...
FinMem-a-in-U UCode        (IdCode a b) ()
...
FinMem-a-in-U (FunEl g)    (IdCode a b) ()
...
FinMem-a-in-U (PiCode a f) (IdCode b d) ()
FinMem-a-in-U (IdCode a b) UCode        mem = tt
FinMem-a-in-U (IdCode a b) Bot          ()
FinMem-a-in-U (IdCode a b) (FunEl g)    ()
FinMem-a-in-U (IdCode a b) (PiCode c g) ()
FinMem-a-in-U (IdCode a b) (IdCode c d) ()
