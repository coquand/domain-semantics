...
module CAST.LeqStageBridge where
...
open import CAST.LeqStageProps using ( leq-Bot-any )
open import CAST.LeqStageStable using ( leqC-from ; leqC-to ; lei-shift )
...
mutual
  ...
  LeCode (PiCode a f)    (IdCode b d)  = Empty
  LeCode (IdCode a b)    Bot           = Empty
  LeCode (IdCode a b)    UCode         = Empty
  LeCode (IdCode a b)    (FunEl h)     = Empty
  LeCode (IdCode a b)    (PiCode c g)  = Empty
  LeCode (IdCode a b)    (IdCode c d)  = Pair (LeCode a c) (LeCode b d)
...
mutual
  ...
  toLeq n (FunEl g)    (IdCode b d)  bnd ()
  ...
  toLeq n (PiCode a f) (IdCode b d)  bnd ()
  toLeq n (IdCode a b) Bot           bnd ()
  toLeq n (IdCode a b) UCode         bnd ()
  toLeq n (IdCode a b) (FunEl h)     bnd ()
  toLeq n (IdCode a b) (PiCode c g)  bnd ()
  toLeq zero    (IdCode a b) (IdCode c d) bnd lc = bnd
  toLeq (suc m) (IdCode a b) (IdCode c d) bnd lc =
    mkSigma
      (toLeq m a c (max-mono-Id-l a b c d m bnd) (fst lc))
      (toLeq m b d (max-mono-Id-r a b c d m bnd) (snd lc))
  ...
  fromLeq zero    UCode  (IdCode b d)  bnd ()
  fromLeq (suc m) UCode  (IdCode b d)  bnd ()
  ...
  fromLeq zero    (FunEl g) (IdCode b d) bnd ()
  fromLeq (suc m) (FunEl g) (IdCode b d) bnd ()
  ...
  fromLeq zero    (PiCode a f) (IdCode b d) bnd ()
  fromLeq (suc m) (PiCode a f) (IdCode b d) bnd ()
  fromLeq zero    (IdCode a b) Bot     bnd ()
  fromLeq (suc m) (IdCode a b) Bot     bnd ()
  fromLeq zero    (IdCode a b) UCode   bnd ()
  fromLeq (suc m) (IdCode a b) UCode   bnd ()
  fromLeq zero    (IdCode a b) (FunEl h) bnd ()
  fromLeq (suc m) (IdCode a b) (FunEl h) bnd ()
  fromLeq zero    (IdCode a b) (PiCode c g) bnd ()
  fromLeq (suc m) (IdCode a b) (PiCode c g) bnd ()
  fromLeq zero    (IdCode a b) (IdCode c d) bnd ()
  fromLeq (suc m) (IdCode a b) (IdCode c d) bnd le =
    mkSigma
      (fromLeq m a c (max-mono-Id-l a b c d m bnd) (fst le))
      (fromLeq m b d (max-mono-Id-r a b c d m bnd) (snd le))
  ...
  -- bounds for the IdCode descent: RANK (IdCode a b) = suc (max (RANK a) (RANK b))
  max-mono-Id-l : (a b c d : FinEl) (m : Nat) ->
    Le (max (RANK (IdCode a b)) (RANK (IdCode c d))) (suc m) ->
    Le (max (RANK a) (RANK c)) m
  max-mono-Id-l a b c d m bnd =
    Le-max-lub (RANK a) (RANK c) m
      (Le-trans (RANK a) (max (RANK a) (RANK b)) m
        (Le-max-l (RANK a) (RANK b))
        (max-Le-l (max (RANK a) (RANK b)) (max (RANK c) (RANK d)) m bnd))
      (Le-trans (RANK c) (max (RANK c) (RANK d)) m
        (Le-max-l (RANK c) (RANK d))
        (max-Le-r (max (RANK a) (RANK b)) (max (RANK c) (RANK d)) m bnd))

  max-mono-Id-r : (a b c d : FinEl) (m : Nat) ->
    Le (max (RANK (IdCode a b)) (RANK (IdCode c d))) (suc m) ->
    Le (max (RANK b) (RANK d)) m
  max-mono-Id-r a b c d m bnd =
    Le-max-lub (RANK b) (RANK d) m
      (Le-trans (RANK b) (max (RANK a) (RANK b)) m
        (Le-max-r (RANK a) (RANK b))
        (max-Le-l (max (RANK a) (RANK b)) (max (RANK c) (RANK d)) m bnd))
      (Le-trans (RANK d) (max (RANK c) (RANK d)) m
        (Le-max-r (RANK c) (RANK d))
        (max-Le-r (max (RANK a) (RANK b)) (max (RANK c) (RANK d)) m bnd))
