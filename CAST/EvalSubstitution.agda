...
module CAST.EvalSubstitution where

import CAST.Basic as S
open S using (Nat ; zero ; suc ; Top ; tt ; Empty ; Pair ; mkSigma ; fst ; snd ;
              Sigma ; Eq ; refl ; Eq-transport ; Eq-sym ;
              FinEl ; Bot ; UCode ; FunEl ; PiCode ; IdCode ; FinFun ; isPos)
open import CAST.PaperSemantics using (LeCode ; LeCode-refl ; LeCode-trans ;
...
open import CAST.Selection using (Edge ; EdgeIn ; here ; there ; Selection ;
...
EvalRel-ren r (App M N) rho rho' eq (PiCode a' f')
  ...
  mkSigma v (mkSigma (EvalRel-ren r N rho rho' eq v evN)
                      (EvalRel-ren r M rho rho' eq _ evM))
EvalRel-ren r (App M N) rho rho' eq (IdCode a' b')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren r N rho rho' eq v evN)
                      (EvalRel-ren r M rho rho' eq _ evM))
...
EvalRel-ren r (Lam A M) rho rho' eq (IdCode a b) ()
...
EvalRel-ren r (Pi A B) rho rho' eq (IdCode a f) ()

-- Id
EvalRel-ren r (Id A B) rho rho' eq Bot ev = tt
EvalRel-ren r (Id A B) rho rho' eq (IdCode a b)
  (mkSigma cab (mkSigma evA evB)) =
  mkSigma cab (mkSigma (EvalRel-ren r A rho rho' eq a evA)
                       (EvalRel-ren r B rho rho' eq b evB))
EvalRel-ren r (Id A B) rho rho' eq UCode ()
EvalRel-ren r (Id A B) rho rho' eq (FunEl g) ()
EvalRel-ren r (Id A B) rho rho' eq (PiCode a f) ()

-- proofs
EvalRel-ren r refl rho rho' eq Bot ev = tt
EvalRel-ren r refl rho rho' eq UCode ()
EvalRel-ren r refl rho rho' eq (FunEl g) ()
EvalRel-ren r refl rho rho' eq (PiCode a f) ()
EvalRel-ren r refl rho rho' eq (IdCode a b) ()
EvalRel-ren r (sym p) rho rho' eq Bot ev = tt
EvalRel-ren r (sym p) rho rho' eq UCode ()
EvalRel-ren r (sym p) rho rho' eq (FunEl g) ()
EvalRel-ren r (sym p) rho rho' eq (PiCode a f) ()
EvalRel-ren r (sym p) rho rho' eq (IdCode a b) ()
EvalRel-ren r (pi1 p) rho rho' eq Bot ev = tt
EvalRel-ren r (pi1 p) rho rho' eq UCode ()
EvalRel-ren r (pi1 p) rho rho' eq (FunEl g) ()
EvalRel-ren r (pi1 p) rho rho' eq (PiCode a f) ()
EvalRel-ren r (pi1 p) rho rho' eq (IdCode a b) ()
EvalRel-ren r (pi2 p N) rho rho' eq Bot ev = tt
EvalRel-ren r (pi2 p N) rho rho' eq UCode ()
EvalRel-ren r (pi2 p N) rho rho' eq (FunEl g) ()
EvalRel-ren r (pi2 p N) rho rho' eq (PiCode a f) ()
EvalRel-ren r (pi2 p N) rho rho' eq (IdCode a b) ()

-- cast: transport the M-value and the B-membership; coherence/LeCode/FinMem fixed
EvalRel-ren r (cast A B p M) rho rho' eq u
  (mkSigma cu (mkSigma v (mkSigma le (mkSigma evM-v (mkSigma b (mkSigma evB-b fm)))))) =
  mkSigma cu (mkSigma v (mkSigma le
    (mkSigma (EvalRel-ren r M rho rho' eq v evM-v)
      (mkSigma b (mkSigma (EvalRel-ren r B rho rho' eq b evB-b) fm)))))
...
EvalRel-ren-inv r (App M N) rho rho' eq (PiCode a' f')
  ...
  mkSigma v (mkSigma (EvalRel-ren-inv r N rho rho' eq v evN)
                      (EvalRel-ren-inv r M rho rho' eq _ evM))
EvalRel-ren-inv r (App M N) rho rho' eq (IdCode a' b')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-ren-inv r N rho rho' eq v evN)
                      (EvalRel-ren-inv r M rho rho' eq _ evM))
...
EvalRel-ren-inv r (Lam A M) rho rho' eq (IdCode a b) ()
...
EvalRel-ren-inv r (Pi A B) rho rho' eq (IdCode a f) ()

-- Id
EvalRel-ren-inv r (Id A B) rho rho' eq Bot ev = tt
EvalRel-ren-inv r (Id A B) rho rho' eq (IdCode a b)
  (mkSigma cab (mkSigma evA evB)) =
  mkSigma cab (mkSigma (EvalRel-ren-inv r A rho rho' eq a evA)
                       (EvalRel-ren-inv r B rho rho' eq b evB))
EvalRel-ren-inv r (Id A B) rho rho' eq UCode ()
EvalRel-ren-inv r (Id A B) rho rho' eq (FunEl g) ()
EvalRel-ren-inv r (Id A B) rho rho' eq (PiCode a f) ()

-- proofs
EvalRel-ren-inv r refl rho rho' eq Bot ev = tt
EvalRel-ren-inv r refl rho rho' eq UCode ()
EvalRel-ren-inv r refl rho rho' eq (FunEl g) ()
EvalRel-ren-inv r refl rho rho' eq (PiCode a f) ()
EvalRel-ren-inv r refl rho rho' eq (IdCode a b) ()
EvalRel-ren-inv r (sym p) rho rho' eq Bot ev = tt
EvalRel-ren-inv r (sym p) rho rho' eq UCode ()
EvalRel-ren-inv r (sym p) rho rho' eq (FunEl g) ()
EvalRel-ren-inv r (sym p) rho rho' eq (PiCode a f) ()
EvalRel-ren-inv r (sym p) rho rho' eq (IdCode a b) ()
EvalRel-ren-inv r (pi1 p) rho rho' eq Bot ev = tt
EvalRel-ren-inv r (pi1 p) rho rho' eq UCode ()
EvalRel-ren-inv r (pi1 p) rho rho' eq (FunEl g) ()
EvalRel-ren-inv r (pi1 p) rho rho' eq (PiCode a f) ()
EvalRel-ren-inv r (pi1 p) rho rho' eq (IdCode a b) ()
EvalRel-ren-inv r (pi2 p N) rho rho' eq Bot ev = tt
EvalRel-ren-inv r (pi2 p N) rho rho' eq UCode ()
EvalRel-ren-inv r (pi2 p N) rho rho' eq (FunEl g) ()
EvalRel-ren-inv r (pi2 p N) rho rho' eq (PiCode a f) ()
EvalRel-ren-inv r (pi2 p N) rho rho' eq (IdCode a b) ()

-- cast
EvalRel-ren-inv r (cast A B p M) rho rho' eq u
  (mkSigma cu (mkSigma v (mkSigma le (mkSigma evM-v (mkSigma b (mkSigma evB-b fm)))))) =
  mkSigma cu (mkSigma v (mkSigma le
    (mkSigma (EvalRel-ren-inv r M rho rho' eq v evM-v)
      (mkSigma b (mkSigma (EvalRel-ren-inv r B rho rho' eq b evB-b) fm)))))
...
EvalRel-subst sigma (App M N) rho rho' crho sr (PiCode a' f')
  ...
  mkSigma v (mkSigma (EvalRel-subst sigma N rho rho' crho sr v evN)
                      (EvalRel-subst sigma M rho rho' crho sr _ evM))
EvalRel-subst sigma (App M N) rho rho' crho sr (IdCode a' b')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst sigma N rho rho' crho sr v evN)
                      (EvalRel-subst sigma M rho rho' crho sr _ evM))
...
EvalRel-subst sigma (Lam A M) rho rho' crho sr (IdCode a b) ()
...
EvalRel-subst sigma (Pi A B) rho rho' crho sr (IdCode a f) ()

-- Id
EvalRel-subst sigma (Id A B) rho rho' crho sr Bot ev = tt
EvalRel-subst sigma (Id A B) rho rho' crho sr (IdCode a b)
  (mkSigma cab (mkSigma evA evB)) =
  mkSigma cab (mkSigma (EvalRel-subst sigma A rho rho' crho sr a evA)
                       (EvalRel-subst sigma B rho rho' crho sr b evB))
EvalRel-subst sigma (Id A B) rho rho' crho sr UCode ()
EvalRel-subst sigma (Id A B) rho rho' crho sr (FunEl g) ()
EvalRel-subst sigma (Id A B) rho rho' crho sr (PiCode a f) ()

-- proofs
EvalRel-subst sigma refl rho rho' crho sr Bot ev = tt
EvalRel-subst sigma refl rho rho' crho sr UCode ()
EvalRel-subst sigma refl rho rho' crho sr (FunEl g) ()
EvalRel-subst sigma refl rho rho' crho sr (PiCode a f) ()
EvalRel-subst sigma refl rho rho' crho sr (IdCode a b) ()
EvalRel-subst sigma (sym p) rho rho' crho sr Bot ev = tt
EvalRel-subst sigma (sym p) rho rho' crho sr UCode ()
EvalRel-subst sigma (sym p) rho rho' crho sr (FunEl g) ()
EvalRel-subst sigma (sym p) rho rho' crho sr (PiCode a f) ()
EvalRel-subst sigma (sym p) rho rho' crho sr (IdCode a b) ()
EvalRel-subst sigma (pi1 p) rho rho' crho sr Bot ev = tt
EvalRel-subst sigma (pi1 p) rho rho' crho sr UCode ()
EvalRel-subst sigma (pi1 p) rho rho' crho sr (FunEl g) ()
EvalRel-subst sigma (pi1 p) rho rho' crho sr (PiCode a f) ()
EvalRel-subst sigma (pi1 p) rho rho' crho sr (IdCode a b) ()
EvalRel-subst sigma (pi2 p N) rho rho' crho sr Bot ev = tt
EvalRel-subst sigma (pi2 p N) rho rho' crho sr UCode ()
EvalRel-subst sigma (pi2 p N) rho rho' crho sr (FunEl g) ()
EvalRel-subst sigma (pi2 p N) rho rho' crho sr (PiCode a f) ()
EvalRel-subst sigma (pi2 p N) rho rho' crho sr (IdCode a b) ()

-- cast
EvalRel-subst sigma (cast A B p M) rho rho' crho sr u
  (mkSigma cu (mkSigma v (mkSigma le (mkSigma evM-v (mkSigma b (mkSigma evB-b fm)))))) =
  mkSigma cu (mkSigma v (mkSigma le
    (mkSigma (EvalRel-subst sigma M rho rho' crho sr v evM-v)
      (mkSigma b (mkSigma (EvalRel-subst sigma B rho rho' crho sr b evB-b) fm)))))
...
EvalRel-subst-forward-max sigma (App M N) rho rho' crho crho' msr (PiCode a' f')
  ...
  mkSigma v (mkSigma (EvalRel-subst-forward-max sigma N rho rho' crho crho' msr v evN)
                      (EvalRel-subst-forward-max sigma M rho rho' crho crho' msr _ evM))
EvalRel-subst-forward-max sigma (App M N) rho rho' crho crho' msr (IdCode a' b')
  (mkSigma v (mkSigma evN evM)) =
  mkSigma v (mkSigma (EvalRel-subst-forward-max sigma N rho rho' crho crho' msr v evN)
                      (EvalRel-subst-forward-max sigma M rho rho' crho crho' msr _ evM))
...
EvalRel-subst-forward-max sigma (Lam A M) rho rho' crho crho' msr (IdCode a b) ()
...
EvalRel-subst-forward-max sigma (Pi A B) rho rho' crho crho' msr (IdCode a f) ()

-- Id
EvalRel-subst-forward-max sigma (Id A B) rho rho' crho crho' msr Bot ev = tt
EvalRel-subst-forward-max sigma (Id A B) rho rho' crho crho' msr (IdCode a b)
  (mkSigma cab (mkSigma evA evB)) =
  mkSigma cab (mkSigma (EvalRel-subst-forward-max sigma A rho rho' crho crho' msr a evA)
                       (EvalRel-subst-forward-max sigma B rho rho' crho crho' msr b evB))
EvalRel-subst-forward-max sigma (Id A B) rho rho' crho crho' msr UCode ()
EvalRel-subst-forward-max sigma (Id A B) rho rho' crho crho' msr (FunEl g) ()
EvalRel-subst-forward-max sigma (Id A B) rho rho' crho crho' msr (PiCode a f) ()

-- proofs
EvalRel-subst-forward-max sigma refl rho rho' crho crho' msr Bot ev = tt
EvalRel-subst-forward-max sigma refl rho rho' crho crho' msr UCode ()
EvalRel-subst-forward-max sigma refl rho rho' crho crho' msr (FunEl g) ()
EvalRel-subst-forward-max sigma refl rho rho' crho crho' msr (PiCode a f) ()
EvalRel-subst-forward-max sigma refl rho rho' crho crho' msr (IdCode a b) ()
EvalRel-subst-forward-max sigma (sym p) rho rho' crho crho' msr Bot ev = tt
EvalRel-subst-forward-max sigma (sym p) rho rho' crho crho' msr UCode ()
EvalRel-subst-forward-max sigma (sym p) rho rho' crho crho' msr (FunEl g) ()
EvalRel-subst-forward-max sigma (sym p) rho rho' crho crho' msr (PiCode a f) ()
EvalRel-subst-forward-max sigma (sym p) rho rho' crho crho' msr (IdCode a b) ()
EvalRel-subst-forward-max sigma (pi1 p) rho rho' crho crho' msr Bot ev = tt
EvalRel-subst-forward-max sigma (pi1 p) rho rho' crho crho' msr UCode ()
EvalRel-subst-forward-max sigma (pi1 p) rho rho' crho crho' msr (FunEl g) ()
EvalRel-subst-forward-max sigma (pi1 p) rho rho' crho crho' msr (PiCode a f) ()
EvalRel-subst-forward-max sigma (pi1 p) rho rho' crho crho' msr (IdCode a b) ()
EvalRel-subst-forward-max sigma (pi2 p N) rho rho' crho crho' msr Bot ev = tt
EvalRel-subst-forward-max sigma (pi2 p N) rho rho' crho crho' msr UCode ()
EvalRel-subst-forward-max sigma (pi2 p N) rho rho' crho crho' msr (FunEl g) ()
EvalRel-subst-forward-max sigma (pi2 p N) rho rho' crho crho' msr (PiCode a f) ()
EvalRel-subst-forward-max sigma (pi2 p N) rho rho' crho crho' msr (IdCode a b) ()

-- cast
EvalRel-subst-forward-max sigma (cast A B p M) rho rho' crho crho' msr u
  (mkSigma cu (mkSigma v (mkSigma le (mkSigma evM-v (mkSigma b (mkSigma evB-b fm)))))) =
  mkSigma cu (mkSigma v (mkSigma le
    (mkSigma (EvalRel-subst-forward-max sigma M rho rho' crho crho' msr v evM-v)
      (mkSigma b (mkSigma (EvalRel-subst-forward-max sigma B rho rho' crho crho' msr b evB-b) fm)))))
...
foldEdgeFwd sigma rho M accRho crho cAcc srAcc a (cons p ps) body =
  ...
  in mkSigma rho' (mkSigma crho' (mkSigma srho'
       (mkSigma (EnvLe-trans accRho midRho rho' leAcc leMid)
         (\ q ein -> edgeCase {M0 = M} {a0 = a} {r = rho'} q ein
...
EvalRel-subst-forward-wit sigma (App M N) rho (IdCode a' b') crho
  (mkSigma v (mkSigma evN evM)) =
  let rN = EvalRel-subst-forward-wit sigma N rho v crho evN
      rM = EvalRel-subst-forward-wit sigma M rho _ crho evM
      rhoN = fst rN ; crhoN = fst (snd rN) ; srN = fst (snd (snd rN))
...
EvalRel-subst-forward-wit sigma (Lam A M) rho (IdCode a b) crho ()
...
EvalRel-subst-forward-wit sigma (Pi A B) rho (IdCode a f) crho ()

-- Id: combine witnesses for A (at a) and B (at b), like App
EvalRel-subst-forward-wit sigma (Id A B) rho Bot crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent
    (mkSigma (SubRel-botEnv sigma rho) tt))
EvalRel-subst-forward-wit sigma (Id A B) rho (IdCode a b) crho
  (mkSigma cab (mkSigma evA evB)) =
  let rA = EvalRel-subst-forward-wit sigma A rho a crho evA
      rB = EvalRel-subst-forward-wit sigma B rho b crho evB
      rhoA = fst rA ; crhoA = fst (snd rA) ; srA = fst (snd (snd rA))
      evA' = snd (snd (snd rA))
      rhoB = fst rB ; crhoB = fst (snd rB) ; srB = fst (snd (snd rB))
      evB' = snd (snd (snd rB))
      comb = combineFwd sigma rho crho rhoA rhoB crhoA crhoB srA srB
      rho' = fst comb ; crho' = fst (snd comb)
      sr'  = fst (snd (snd comb))
      leA  = fst (snd (snd (snd comb)))
      leB  = snd (snd (snd (snd comb)))
  in mkSigma rho' (mkSigma crho' (mkSigma sr'
       (mkSigma cab (mkSigma (EvalRel-mon-env A rhoA rho' a evA' leA)
                             (EvalRel-mon-env B rhoB rho' b evB' leB)))))
EvalRel-subst-forward-wit sigma (Id A B) rho UCode crho ()
EvalRel-subst-forward-wit sigma (Id A B) rho (FunEl g) crho ()
EvalRel-subst-forward-wit sigma (Id A B) rho (PiCode a f) crho ()

-- proofs: only Bot output; botEnv witness
EvalRel-subst-forward-wit sigma refl rho Bot crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent
    (mkSigma (SubRel-botEnv sigma rho) tt))
EvalRel-subst-forward-wit sigma refl rho UCode crho ()
EvalRel-subst-forward-wit sigma refl rho (FunEl g) crho ()
EvalRel-subst-forward-wit sigma refl rho (PiCode a f) crho ()
EvalRel-subst-forward-wit sigma refl rho (IdCode a b) crho ()
EvalRel-subst-forward-wit sigma (sym p) rho Bot crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent
    (mkSigma (SubRel-botEnv sigma rho) tt))
EvalRel-subst-forward-wit sigma (sym p) rho UCode crho ()
EvalRel-subst-forward-wit sigma (sym p) rho (FunEl g) crho ()
EvalRel-subst-forward-wit sigma (sym p) rho (PiCode a f) crho ()
EvalRel-subst-forward-wit sigma (sym p) rho (IdCode a b) crho ()
EvalRel-subst-forward-wit sigma (pi1 p) rho Bot crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent
    (mkSigma (SubRel-botEnv sigma rho) tt))
EvalRel-subst-forward-wit sigma (pi1 p) rho UCode crho ()
EvalRel-subst-forward-wit sigma (pi1 p) rho (FunEl g) crho ()
EvalRel-subst-forward-wit sigma (pi1 p) rho (PiCode a f) crho ()
EvalRel-subst-forward-wit sigma (pi1 p) rho (IdCode a b) crho ()
EvalRel-subst-forward-wit sigma (pi2 p N) rho Bot crho ev =
  mkSigma botEnv (mkSigma botEnv-Coherent
    (mkSigma (SubRel-botEnv sigma rho) tt))
EvalRel-subst-forward-wit sigma (pi2 p N) rho UCode crho ()
EvalRel-subst-forward-wit sigma (pi2 p N) rho (FunEl g) crho ()
EvalRel-subst-forward-wit sigma (pi2 p N) rho (PiCode a f) crho ()
EvalRel-subst-forward-wit sigma (pi2 p N) rho (IdCode a b) crho ()

-- cast: combine the witness env for the M-value with the one for the B-member
EvalRel-subst-forward-wit sigma (cast A B p M) rho u crho
  (mkSigma cu (mkSigma v (mkSigma le (mkSigma evM-v (mkSigma b (mkSigma evB-b fm)))))) =
  let rM = EvalRel-subst-forward-wit sigma M rho v crho evM-v
      rB = EvalRel-subst-forward-wit sigma B rho b crho evB-b
      rhoM = fst rM ; crhoM = fst (snd rM) ; srM = fst (snd (snd rM))
      evM' = snd (snd (snd rM))
      rhoB = fst rB ; crhoB = fst (snd rB) ; srB = fst (snd (snd rB))
      evB' = snd (snd (snd rB))
      comb = combineFwd sigma rho crho rhoM rhoB crhoM crhoB srM srB
      rho' = fst comb ; crho' = fst (snd comb)
      sr'  = fst (snd (snd comb))
      leM  = fst (snd (snd (snd comb)))
      leB  = snd (snd (snd (snd comb)))
  in mkSigma rho' (mkSigma crho' (mkSigma sr'
       (mkSigma cu (mkSigma v (mkSigma le
         (mkSigma (EvalRel-mon-env M rhoM rho' v evM' leM)
           (mkSigma b (mkSigma (EvalRel-mon-env B rhoB rho' b evB' leB) fm))))))))
