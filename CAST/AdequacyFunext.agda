...
module CAST.AdequacyFunext where
...
open import CAST.AdequacyPi using (Adq ; transportVal2')
open import CAST.AdequacyVE using (AdqE1)

import CAST.Basic as S
open S using (Nat ; suc ; tt ; mkSigma ; fst ; snd ; FinEl ; Bot ; UCode ; FunEl ; PiCode ; IdCode ; FinFun ; nil ; cons)
open import CAST.PaperSemantics using (LeCode ; LeCode-refl ; Coherent ; EvalFun ; FinMem ;
...
open import CAST.RawSyntax using (Expr ; Var ; U ; Pi ; Lam ; App ; fzero ; wkExpr ; subst1 ;
  Sub ; liftSub ; substExpr)
open import CAST.TypingRules using (Ctx ; extend ; HasType ; ConvTm ; WfCtx ; wf-extend ;
  ty-Pi ; conv-refl ; conv-funext)
open import CAST.Reduction using (headred-refl)
open import CAST.SubstitutionLemma using (WtSub ; subst-HasType ; liftSub-WtSub)
open import CAST.TypingSemantics using (convSound)
open import CAST.EvalSubstitution using (EvalRel-Pi-body ; EvalRel-wk)
open import CAST.Validity using (Selection ; Coherent-Selection ; Coherent-Selection-val ;
  Selection-le-EvalFun)
open import CAST.Selection using (FinMem-Selection ; FinMem-Selection-codomain)
...
adequacyEqSub2-funext {H = H} {G = G} {A = A} {B = B} {f = f} {g' = g'} dA d df dg sigma rho crho vs fits wtsub wfH IHfv IHgv IHbe IH-A (FunEl g0) hu (PiCode b f0) evA fm =
  ...
  where
    ...
    buildEqBody vps u0 (IdCode v0 v1) sel P htP valP =
      let cu0 = Coherent-Selection sel ctg0'
          cv0 = Coherent-Selection-val sel ctg0'
          le  = Selection-le-EvalFun g0 sel (LeFunCode-refl g0 ctg0') ctg0' ctg0' cu0
          ev_f_sing = mkSingEvApp u0 (IdCode v0 v1) cu0 cv0 tt le
          ev_wkf    = EvalRel-wk f rho u0 (FunEl (cons (mkSigma u0 (IdCode v0 v1)) nil)) ev_f_sing
          ev_var    = mkSigma cu0 (LeCode-refl u0 cu0)
          ev_app    = mkSigma u0 (mkSigma ev_var ev_wkf)
      in buildEqBodyCore u0 (IdCode v0 v1) sel cu0 ev_app P htP valP
