...
module CAST.AdequacyHelpers where
...
open import CAST.PaperSemantics using (LeCode ; LeCode-Bot ; LeCode-refl ; LeCode-trans ; Coherent ; CoherentFun ; Comp ; Comp-down ; Sup ; LeCode-Sup-left ; LeCode-Sup-right ; Coherent-Sup ; EvalFun ; EvalFun-in-UCode ; Coherent-EvalFun ; EvalFun-mon-arg ; LeFunCode ; LeFunCode-refl ; FinMem ; FinMemFun ; FinMemAllU ; FinMem-a-in-U ; finMemUCode-Sup ; finMem-upward ; finMem-Sup-left ; finMem-Sup-right ; coh-from-aU ; FinMem-coh-u ; cft-from-cf ; CoherentFunTail ; CoherentFunTail-append ; mkCFT ; NotBot ; absurdEl)
open import CAST.Reduction using (Red ; mkRed ; Red-refl ; Red-hr ; HeadRed ; HeadRed-trans ; HeadRed-App ; HeadRed-strip-Pi ; headred-step ; headred-beta ; headred-refl ; subst-subst1-comm ; idSub ; substExpr-id)
open import CAST.RawSemantics using (EnvApprox ; emptyEnv ; extendEnv ; lookupEnv ; EvalRel ; Pi-edgewise ; EvalRel-coh ; CoherentEnv ; lookupEnv-coh ; EvalRel-Comp ; EvalRel-Sup ; EvalRel-down ; EvalRel-mon-env ; EnvLe ; EnvLe-refl)
open import CAST.RawSyntax using (Expr ; Var ; U ; Pi ; Lam ; App ; Fin ; fzero ; fsuc ; wkExpr ; subst1 ; Sub ; liftSub ; substExpr ; subst1Sub ;
  Id ; sym ; pi1 ; pi2 ; cast ; Eq-cong4-Expr)
  renaming (refl to reflE)
open import CAST.TypingRules using (Ctx ; empty ; extend ; lookup ; HasType ; ConvTm ; WfCtx ; wf-empty ; wf-extend ; ty-var ; ty-conv ; ty-U ; ty-Pi ; ty-Lam ; ty-App ; conv-refl ; conv-sym ; conv-trans ; conv-conv ; conv-beta ; conv-Pi ; conv-funext ; conv-App-fun ; conv-App-arg)
open import CAST.ValidityPublic public
open import CAST.Validity using (Edge ; EdgeIn ; here ; there ; Red-unique-Pi ; FinMem-Coherent ; Selection ; sel-nil ; sel-skip ; sel-take ; Selection-le-EvalFun ; Coherent-Selection ; Coherent-Selection-val ; bU-from-cf-fmU)
-- Re-import directly (Validity2Sigma re-exports don't always resolve)
open import CAST.Validity using () renaming (Red-unique-Pi to Red-unique-Pi2)
open import CAST.Selection using (FinMemAllU-Selection ; FinMem-Selection-UCode ; FinMem-Selection ; FinMem-Selection-codomain ; selectionBelow)
open import CAST.TypingSemantics using (convSound ; convSound-inv ; convSound' ; theorem1 ; LeCode-Bot-eq)
open import CAST.LemmaForTS using (Fits ; Typed ; Fits-CoherentEnv)
open import CAST.EvalSubstitution using (EvalRel-subst1-backward ; EvalRel-wk ; EvalRel-unwk ; EvalRel-Pi-app-type ; EvalRel-Pi-body ; EvalRel-subst1-forward)

open import CAST.RawSyntax using (Ren ; liftRen ; renExpr ; wkRen)
open import CAST.SubstitutionLemma using (typing-ConvTm ; WtSub ; subst-HasType ; subst-ConvTm ; liftSub-WtSub ; subst1-WtSub ; typing-WfCtx ; typing-type ; ctx-conv-HasType ; ctx-conv-ConvTm ; subst1-cong-ConvTm ; wk-HasType ; wk-ConvTm ; WtConvSub ; subst-ConvTm-cross ; liftSub-WtConvSub)
...
substExpr-ext sigma sigma' hyp (Id A B) =
  Eq-cong2 Id (substExpr-ext sigma sigma' hyp A) (substExpr-ext sigma sigma' hyp B)
substExpr-ext sigma sigma' hyp reflE = S.refl
substExpr-ext sigma sigma' hyp (sym p) = S.Eq-cong sym (substExpr-ext sigma sigma' hyp p)
substExpr-ext sigma sigma' hyp (pi1 p) = S.Eq-cong pi1 (substExpr-ext sigma sigma' hyp p)
substExpr-ext sigma sigma' hyp (pi2 p N) =
  Eq-cong2 pi2 (substExpr-ext sigma sigma' hyp p) (substExpr-ext sigma sigma' hyp N)
substExpr-ext sigma sigma' hyp (cast A B p M) =
  Eq-cong4-Expr cast (substExpr-ext sigma sigma' hyp A) (substExpr-ext sigma sigma' hyp B)
                     (substExpr-ext sigma sigma' hyp p) (substExpr-ext sigma sigma' hyp M)
...
substExpr-ren sigma rho (Id A B) =
  Eq-cong2 Id (substExpr-ren sigma rho A) (substExpr-ren sigma rho B)
substExpr-ren sigma rho reflE = S.refl
substExpr-ren sigma rho (sym p) = S.Eq-cong sym (substExpr-ren sigma rho p)
substExpr-ren sigma rho (pi1 p) = S.Eq-cong pi1 (substExpr-ren sigma rho p)
substExpr-ren sigma rho (pi2 p N) =
  Eq-cong2 pi2 (substExpr-ren sigma rho p) (substExpr-ren sigma rho N)
substExpr-ren sigma rho (cast A B p M) =
  Eq-cong4-Expr cast (substExpr-ren sigma rho A) (substExpr-ren sigma rho B)
                     (substExpr-ren sigma rho p) (substExpr-ren sigma rho M)
...
renExpr-ext r1 r2 hyp (Id A B) =
  Eq-cong2 Id (renExpr-ext r1 r2 hyp A) (renExpr-ext r1 r2 hyp B)
renExpr-ext r1 r2 hyp reflE = S.refl
renExpr-ext r1 r2 hyp (sym p) = S.Eq-cong sym (renExpr-ext r1 r2 hyp p)
renExpr-ext r1 r2 hyp (pi1 p) = S.Eq-cong pi1 (renExpr-ext r1 r2 hyp p)
renExpr-ext r1 r2 hyp (pi2 p N) =
  Eq-cong2 pi2 (renExpr-ext r1 r2 hyp p) (renExpr-ext r1 r2 hyp N)
renExpr-ext r1 r2 hyp (cast A B p M) =
  Eq-cong4-Expr cast (renExpr-ext r1 r2 hyp A) (renExpr-ext r1 r2 hyp B)
                     (renExpr-ext r1 r2 hyp p) (renExpr-ext r1 r2 hyp M)
...
renExpr-comp r2 r1 (Id A B) =
  Eq-cong2 Id (renExpr-comp r2 r1 A) (renExpr-comp r2 r1 B)
renExpr-comp r2 r1 reflE = S.refl
renExpr-comp r2 r1 (sym p) = S.Eq-cong sym (renExpr-comp r2 r1 p)
renExpr-comp r2 r1 (pi1 p) = S.Eq-cong pi1 (renExpr-comp r2 r1 p)
renExpr-comp r2 r1 (pi2 p N) =
  Eq-cong2 pi2 (renExpr-comp r2 r1 p) (renExpr-comp r2 r1 N)
renExpr-comp r2 r1 (cast A B p M) =
  Eq-cong4-Expr cast (renExpr-comp r2 r1 A) (renExpr-comp r2 r1 B)
                     (renExpr-comp r2 r1 p) (renExpr-comp r2 r1 M)
...
renExpr-substExpr rho sigma (Id A B) =
  Eq-cong2 Id (renExpr-substExpr rho sigma A) (renExpr-substExpr rho sigma B)
renExpr-substExpr rho sigma reflE = S.refl
renExpr-substExpr rho sigma (sym p) = S.Eq-cong sym (renExpr-substExpr rho sigma p)
renExpr-substExpr rho sigma (pi1 p) = S.Eq-cong pi1 (renExpr-substExpr rho sigma p)
renExpr-substExpr rho sigma (pi2 p N) =
  Eq-cong2 pi2 (renExpr-substExpr rho sigma p) (renExpr-substExpr rho sigma N)
renExpr-substExpr rho sigma (cast A B p M) =
  Eq-cong4-Expr cast (renExpr-substExpr rho sigma A) (renExpr-substExpr rho sigma B)
                     (renExpr-substExpr rho sigma p) (renExpr-substExpr rho sigma M)
...
substExpr-sub tau sigma (Id A B) =
  Eq-cong2 Id (substExpr-sub tau sigma A) (substExpr-sub tau sigma B)
substExpr-sub tau sigma reflE = S.refl
substExpr-sub tau sigma (sym p) = S.Eq-cong sym (substExpr-sub tau sigma p)
substExpr-sub tau sigma (pi1 p) = S.Eq-cong pi1 (substExpr-sub tau sigma p)
substExpr-sub tau sigma (pi2 p N) =
  Eq-cong2 pi2 (substExpr-sub tau sigma p) (substExpr-sub tau sigma N)
substExpr-sub tau sigma (cast A B p M) =
  Eq-cong4-Expr cast (substExpr-sub tau sigma A) (substExpr-sub tau sigma B)
                     (substExpr-sub tau sigma p) (substExpr-sub tau sigma M)
...
EvalRel-bot (Var i)     rho = mkSigma tt (CAST.PaperSemantics.LeCode-Bot (lookupEnv i rho))
EvalRel-bot U           rho = mkSigma tt (CAST.PaperSemantics.LeCode-Bot UCode)
EvalRel-bot (Pi A B)    rho = tt
EvalRel-bot (Lam A M)   rho = tt
EvalRel-bot (App f a)   rho = tt
EvalRel-bot (Id A B)    rho = tt
EvalRel-bot reflE       rho = tt
...
-- Val2-U-to-ValTy2, sup-transport-*, app-transport-* live in CAST.Adequacy.agda
