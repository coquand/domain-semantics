...
module CAST.RawSemantics where
...
EvalRel (App M N) rho (IdCode a' b') =
  Sigma FinEl (\ v -> Pair (EvalRel N rho v)
                           (EvalRel M rho (FunEl (cons (mkSigma v (IdCode a' b')) nil))))
...
EvalRel (Lam A M) rho (IdCode a b)  = Empty
...
EvalRel (Pi A B) rho (IdCode a b)   = Empty

-- Id  (mirrors Pi: IdCode a b approximates Id A B iff a approximates A and
-- b approximates B; coherent.  No function component.)
EvalRel (Id A B) rho Bot = Top
EvalRel (Id A B) rho (IdCode a b) =
  Pair (Coherent (IdCode a b)) (Pair (EvalRel A rho a) (EvalRel B rho b))
EvalRel (Id A B) rho UCode          = Empty
EvalRel (Id A B) rho (FunEl g)      = Empty
EvalRel (Id A B) rho (PiCode a f)   = Empty

-- Proofs (refl, sym, pi1, pi2): proof-irrelevant, only approximant is Bot.
EvalRel refl rho Bot           = Top
EvalRel refl rho UCode         = Empty
EvalRel refl rho (FunEl g)     = Empty
EvalRel refl rho (PiCode a f)  = Empty
EvalRel refl rho (IdCode a b)  = Empty
EvalRel (sym p) rho Bot           = Top
EvalRel (sym p) rho UCode         = Empty
EvalRel (sym p) rho (FunEl g)     = Empty
EvalRel (sym p) rho (PiCode a f)  = Empty
EvalRel (sym p) rho (IdCode a b)  = Empty
EvalRel (pi1 p) rho Bot           = Top
EvalRel (pi1 p) rho UCode         = Empty
EvalRel (pi1 p) rho (FunEl g)     = Empty
EvalRel (pi1 p) rho (PiCode a f)  = Empty
EvalRel (pi1 p) rho (IdCode a b)  = Empty
EvalRel (pi2 p N) rho Bot           = Top
EvalRel (pi2 p N) rho UCode         = Empty
EvalRel (pi2 p N) rho (FunEl g)     = Empty
EvalRel (pi2 p N) rho (PiCode a f)  = Empty
EvalRel (pi2 p N) rho (IdCode a b)  = Empty

-- cast A B p M : B.  A proof p : Id A B does NOT force [[A]] = [[B]], so the
-- coercion is GUARDED: its value keeps the B-typed parts of [[M]].  Concretely
-- u approximates the cast iff u is coherent and  u <= v  for some B-member v
-- (v : b with b <= [[B]]) that is itself a value of M (v <= [[M]]).  Since
-- [[M]] is downward-closed this is exactly "u <= v <= u' with u' <= [[M]]";
-- it is an ideal (coherent, downward- and sup-closed, contains Bot) and makes
-- cast A B p M : B adequate by construction, WITHOUT any [[A]] = [[B]].
EvalRel (cast A B p M) rho u =
  Pair (Coherent u)
    (Sigma FinEl (\ v -> Pair (LeCode u v)
      (Pair (EvalRel M rho v)
        (Sigma FinEl (\ b -> Pair (EvalRel B rho b) (FinMem v b))))))
...
mutual
  ...
  Coherent-val-LeBot-absurd (IdCode a b) cnb ()
...
EvalRel-coh (App M N) rho (IdCode a' b') ev =
  Coherent-singleton-val (fst ev) (IdCode a' b')
    (EvalRel-coh M rho (FunEl (cons (mkSigma (fst ev) (IdCode a' b')) nil)) (snd (snd ev)))
-- Lam
...
EvalRel-coh (Lam A M) rho (IdCode a b) ()
-- Pi
...
EvalRel-coh (Pi A B) rho (IdCode a b) ()
-- Id
EvalRel-coh (Id A B) rho Bot ev = tt
EvalRel-coh (Id A B) rho (IdCode a b) ev = fst ev
EvalRel-coh (Id A B) rho UCode ()
EvalRel-coh (Id A B) rho (FunEl g) ()
EvalRel-coh (Id A B) rho (PiCode a f) ()
-- proofs (refl/sym/pi1/pi2)
EvalRel-coh refl rho Bot ev = tt
EvalRel-coh refl rho UCode ()
EvalRel-coh refl rho (FunEl g) ()
EvalRel-coh refl rho (PiCode a f) ()
EvalRel-coh refl rho (IdCode a b) ()
EvalRel-coh (sym p) rho Bot ev = tt
EvalRel-coh (sym p) rho UCode ()
EvalRel-coh (sym p) rho (FunEl g) ()
EvalRel-coh (sym p) rho (PiCode a f) ()
EvalRel-coh (sym p) rho (IdCode a b) ()
EvalRel-coh (pi1 p) rho Bot ev = tt
EvalRel-coh (pi1 p) rho UCode ()
EvalRel-coh (pi1 p) rho (FunEl g) ()
EvalRel-coh (pi1 p) rho (PiCode a f) ()
EvalRel-coh (pi1 p) rho (IdCode a b) ()
EvalRel-coh (pi2 p N) rho Bot ev = tt
EvalRel-coh (pi2 p N) rho UCode ()
EvalRel-coh (pi2 p N) rho (FunEl g) ()
EvalRel-coh (pi2 p N) rho (PiCode a f) ()
EvalRel-coh (pi2 p N) rho (IdCode a b) ()
-- cast: coherence is the first conjunct
EvalRel-coh (cast A B p M) rho u ev = fst ev
...
EvalRel-Bot (Id A B) rho = tt
EvalRel-Bot refl rho = tt
EvalRel-Bot (sym p) rho = tt
EvalRel-Bot (pi1 p) rho = tt
EvalRel-Bot (pi2 p N) rho = tt
-- cast: Bot is coherent and below the M-value Bot, a member of Bot (≈ B)
EvalRel-Bot (cast A B p M) rho =
  mkSigma tt (mkSigma Bot (mkSigma tt
    (mkSigma (EvalRel-Bot M rho)
      (mkSigma Bot (mkSigma (EvalRel-Bot B rho) tt)))))
...
EvalRel-mon-env (App M N) rho rho' (PiCode a' f') ev envle =
  mkSigma (fst ev) (mkSigma (EvalRel-mon-env N rho rho' (fst ev) (fst (snd ev)) envle)
                             (EvalRel-mon-env M rho rho' _ (snd (snd ev)) envle))
EvalRel-mon-env (App M N) rho rho' (IdCode a' b') ev envle =
  mkSigma (fst ev) (mkSigma (EvalRel-mon-env N rho rho' (fst ev) (fst (snd ev)) envle)
                             (EvalRel-mon-env M rho rho' _ (snd (snd ev)) envle))
-- Lam
...
EvalRel-mon-env (Lam A M) rho rho' (IdCode a b) () envle
-- Pi
...
EvalRel-mon-env (Pi A B) rho rho' (IdCode a f) () envle
-- Id
EvalRel-mon-env (Id A B) rho rho' Bot ev envle = tt
EvalRel-mon-env (Id A B) rho rho' (IdCode a b) ev envle =
  mkSigma (fst ev) (mkSigma (EvalRel-mon-env A rho rho' a (fst (snd ev)) envle)
                             (EvalRel-mon-env B rho rho' b (snd (snd ev)) envle))
EvalRel-mon-env (Id A B) rho rho' UCode () envle
EvalRel-mon-env (Id A B) rho rho' (FunEl g) () envle
EvalRel-mon-env (Id A B) rho rho' (PiCode a f) () envle
-- proofs
EvalRel-mon-env refl rho rho' Bot ev envle = ev
EvalRel-mon-env refl rho rho' UCode () envle
EvalRel-mon-env refl rho rho' (FunEl g) () envle
EvalRel-mon-env refl rho rho' (PiCode a f) () envle
EvalRel-mon-env refl rho rho' (IdCode a b) () envle
EvalRel-mon-env (sym p) rho rho' Bot ev envle = ev
EvalRel-mon-env (sym p) rho rho' UCode () envle
EvalRel-mon-env (sym p) rho rho' (FunEl g) () envle
EvalRel-mon-env (sym p) rho rho' (PiCode a f) () envle
EvalRel-mon-env (sym p) rho rho' (IdCode a b) () envle
EvalRel-mon-env (pi1 p) rho rho' Bot ev envle = ev
EvalRel-mon-env (pi1 p) rho rho' UCode () envle
EvalRel-mon-env (pi1 p) rho rho' (FunEl g) () envle
EvalRel-mon-env (pi1 p) rho rho' (PiCode a f) () envle
EvalRel-mon-env (pi1 p) rho rho' (IdCode a b) () envle
EvalRel-mon-env (pi2 p N) rho rho' Bot ev envle = ev
EvalRel-mon-env (pi2 p N) rho rho' UCode () envle
EvalRel-mon-env (pi2 p N) rho rho' (FunEl g) () envle
EvalRel-mon-env (pi2 p N) rho rho' (PiCode a f) () envle
EvalRel-mon-env (pi2 p N) rho rho' (IdCode a b) () envle
-- cast: transport the M-value and the B-membership through the env step
EvalRel-mon-env (cast A B p M) rho rho' u
  (mkSigma cu (mkSigma v (mkSigma le (mkSigma evM-v (mkSigma b (mkSigma evB-b fm-v-b))))))
  envle =
  mkSigma cu (mkSigma v (mkSigma le
    (mkSigma (EvalRel-mon-env M rho rho' v evM-v envle)
      (mkSigma b (mkSigma (EvalRel-mon-env B rho rho' b evB-b envle) fm-v-b)))))
...
App-decompose M N rho (IdCode a b) nb ev = ev
...
-- App: IdCode row/column
EvalRel-Comp (App M N) rho crho (IdCode a1 b1) Bot ev1 ev2 = comp-Bot-r (IdCode a1 b1)
EvalRel-Comp (App M N) rho crho UCode (IdCode a2 b2) ev1 ev2 =
  App-Comp-helper M N rho (EvalRel-Comp M rho crho) (EvalRel-Comp N rho crho)
    UCode (IdCode a2 b2) (fst ev1) (fst (snd ev1)) (snd (snd ev1)) (fst ev2) (fst (snd ev2)) (snd (snd ev2))
EvalRel-Comp (App M N) rho crho (FunEl g1') (IdCode a2 b2) ev1 ev2 =
  App-Comp-helper M N rho (EvalRel-Comp M rho crho) (EvalRel-Comp N rho crho)
    (FunEl g1') (IdCode a2 b2) (fst ev1) (fst (snd ev1)) (snd (snd ev1)) (fst ev2) (fst (snd ev2)) (snd (snd ev2))
EvalRel-Comp (App M N) rho crho (PiCode a1' f1') (IdCode a2 b2) ev1 ev2 =
  App-Comp-helper M N rho (EvalRel-Comp M rho crho) (EvalRel-Comp N rho crho)
    (PiCode a1' f1') (IdCode a2 b2) (fst ev1) (fst (snd ev1)) (snd (snd ev1)) (fst ev2) (fst (snd ev2)) (snd (snd ev2))
EvalRel-Comp (App M N) rho crho (IdCode a1 b1) UCode ev1 ev2 =
  App-Comp-helper M N rho (EvalRel-Comp M rho crho) (EvalRel-Comp N rho crho)
    (IdCode a1 b1) UCode (fst ev1) (fst (snd ev1)) (snd (snd ev1)) (fst ev2) (fst (snd ev2)) (snd (snd ev2))
EvalRel-Comp (App M N) rho crho (IdCode a1 b1) (FunEl g2') ev1 ev2 =
  App-Comp-helper M N rho (EvalRel-Comp M rho crho) (EvalRel-Comp N rho crho)
    (IdCode a1 b1) (FunEl g2') (fst ev1) (fst (snd ev1)) (snd (snd ev1)) (fst ev2) (fst (snd ev2)) (snd (snd ev2))
EvalRel-Comp (App M N) rho crho (IdCode a1 b1) (PiCode a2' f2') ev1 ev2 =
  App-Comp-helper M N rho (EvalRel-Comp M rho crho) (EvalRel-Comp N rho crho)
    (IdCode a1 b1) (PiCode a2' f2') (fst ev1) (fst (snd ev1)) (snd (snd ev1)) (fst ev2) (fst (snd ev2)) (snd (snd ev2))
EvalRel-Comp (App M N) rho crho (IdCode a1 b1) (IdCode a2 b2) ev1 ev2 =
  App-Comp-helper M N rho (EvalRel-Comp M rho crho) (EvalRel-Comp N rho crho)
    (IdCode a1 b1) (IdCode a2 b2) (fst ev1) (fst (snd ev1)) (snd (snd ev1)) (fst ev2) (fst (snd ev2)) (snd (snd ev2))
...
EvalRel-Comp (Lam A M) rho crho (IdCode a1 b1) v () ev2
EvalRel-Comp (Lam A M) rho crho (FunEl g1) UCode ev1 ()
EvalRel-Comp (Lam A M) rho crho (FunEl g1) (PiCode a2 f2) ev1 ()
EvalRel-Comp (Lam A M) rho crho (FunEl g1) (IdCode a2 b2) ev1 ()
...
EvalRel-Comp (Pi A B) rho crho (IdCode a1 b1) v () ev2
EvalRel-Comp (Pi A B) rho crho (PiCode a1 f1) UCode ev1 ()
EvalRel-Comp (Pi A B) rho crho (PiCode a1 f1) (FunEl g2) ev1 ()
EvalRel-Comp (Pi A B) rho crho (PiCode a1 f1) (IdCode a2 b2) ev1 ()
...
-- Id
EvalRel-Comp (Id A B) rho crho Bot v ev1 ev2 = comp-Bot-l v
EvalRel-Comp (Id A B) rho crho (IdCode a1 b1) Bot ev1 ev2 = comp-Bot-r (IdCode a1 b1)
EvalRel-Comp (Id A B) rho crho UCode v () ev2
EvalRel-Comp (Id A B) rho crho (FunEl g1) v () ev2
EvalRel-Comp (Id A B) rho crho (PiCode a1 f1) v () ev2
EvalRel-Comp (Id A B) rho crho (IdCode a1 b1) UCode ev1 ()
EvalRel-Comp (Id A B) rho crho (IdCode a1 b1) (FunEl g2) ev1 ()
EvalRel-Comp (Id A B) rho crho (IdCode a1 b1) (PiCode a2 f2) ev1 ()
EvalRel-Comp (Id A B) rho crho (IdCode a1 b1) (IdCode a2 b2) ev1 ev2 =
  mkSigma (EvalRel-Comp A rho crho a1 a2 (fst (snd ev1)) (fst (snd ev2)))
          (EvalRel-Comp B rho crho b1 b2 (snd (snd ev1)) (snd (snd ev2)))

-- proofs
EvalRel-Comp refl rho crho Bot v ev1 ev2 = comp-Bot-l v
EvalRel-Comp refl rho crho UCode v () ev2
EvalRel-Comp refl rho crho (FunEl g) v () ev2
EvalRel-Comp refl rho crho (PiCode a f) v () ev2
EvalRel-Comp refl rho crho (IdCode a b) v () ev2
EvalRel-Comp (sym p) rho crho Bot v ev1 ev2 = comp-Bot-l v
EvalRel-Comp (sym p) rho crho UCode v () ev2
EvalRel-Comp (sym p) rho crho (FunEl g) v () ev2
EvalRel-Comp (sym p) rho crho (PiCode a f) v () ev2
EvalRel-Comp (sym p) rho crho (IdCode a b) v () ev2
EvalRel-Comp (pi1 p) rho crho Bot v ev1 ev2 = comp-Bot-l v
EvalRel-Comp (pi1 p) rho crho UCode v () ev2
EvalRel-Comp (pi1 p) rho crho (FunEl g) v () ev2
EvalRel-Comp (pi1 p) rho crho (PiCode a f) v () ev2
EvalRel-Comp (pi1 p) rho crho (IdCode a b) v () ev2
EvalRel-Comp (pi2 p N) rho crho Bot v ev1 ev2 = comp-Bot-l v
EvalRel-Comp (pi2 p N) rho crho UCode v () ev2
EvalRel-Comp (pi2 p N) rho crho (FunEl g) v () ev2
EvalRel-Comp (pi2 p N) rho crho (PiCode a f) v () ev2
EvalRel-Comp (pi2 p N) rho crho (IdCode a b) v () ev2

-- cast: u1 ≤ v1, u2 ≤ v2 with v1,v2 compatible M-values, so u1,u2 compatible
EvalRel-Comp (cast A B p M) rho crho u1 u2
  (mkSigma cu1 (mkSigma v1 (mkSigma le1 (mkSigma evM-v1 _))))
  (mkSigma cu2 (mkSigma v2 (mkSigma le2 (mkSigma evM-v2 _)))) =
  let comp-v = EvalRel-Comp M rho crho v1 v2 evM-v1 evM-v2
      c-u1-v2 = Comp-down u1 v1 v2 le1 comp-v
      c-u2-u1 = Comp-down u2 v2 u1 le2 (Comp-sym u1 v2 c-u1-v2)
  in Comp-sym u2 u1 c-u2-u1
...
EvalRel-down (App M N) rho (PiCode a0 f0) (PiCode a' f') crho cu' ev le =
  let v   = fst ev
      ...
      evM' = EvalRel-down M rho
               ...
               crho c-vu' evM (mkSigma le-u'-ef tt)
  in mkSigma v (mkSigma evN evM')
-- App: IdCode target / source
EvalRel-down (App M N) rho Bot           (IdCode a' b') crho cu' ev ()
EvalRel-down (App M N) rho UCode         (IdCode a' b') crho cu' ev ()
EvalRel-down (App M N) rho (FunEl g0)    (IdCode a' b') crho cu' ev ()
EvalRel-down (App M N) rho (PiCode a0 f0) (IdCode a' b') crho cu' ev ()
EvalRel-down (App M N) rho (IdCode a0 b0) UCode        crho cu' ev ()
EvalRel-down (App M N) rho (IdCode a0 b0) (FunEl g')   crho cu' ev ()
EvalRel-down (App M N) rho (IdCode a0 b0) (PiCode a' f') crho cu' ev ()
EvalRel-down (App M N) rho (IdCode a0 b0) (IdCode a' b') crho cu' ev le =
  let v   = fst ev
      evN = fst (snd ev)
      evM = snd (snd ev)
      c-vu = EvalRel-coh M rho (FunEl (cons (mkSigma v (IdCode a0 b0)) nil)) evM
      cv   = Coherent-singleton-key v (IdCode a0 b0) c-vu
      cu   = Coherent-singleton-val v (IdCode a0 b0) c-vu
      le-refl = fst (LeCode-refl (FunEl (cons (mkSigma v (IdCode a0 b0)) nil)) c-vu)
      c-ef = Coherent-EvalFun (cons (mkSigma v (IdCode a0 b0)) nil) v c-vu cv
      le-u'-ef = LeCode-trans (IdCode a' b') (IdCode a0 b0)
                   (EvalFun (cons (mkSigma v (IdCode a0 b0)) nil) v)
                   cu' cu c-ef le le-refl
      c-vu' = mkCFT cv cu' tt tt tt
      evM' = EvalRel-down M rho
               (FunEl (cons (mkSigma v (IdCode a0 b0)) nil))
               (FunEl (cons (mkSigma v (IdCode a' b')) nil))
               crho c-vu' evM (mkSigma le-u'-ef tt)
  in mkSigma v (mkSigma evN evM')
...
EvalRel-down (Lam A M) rho (IdCode a b) u' crho cu' () le
-- Lam Bot
...
EvalRel-down (Lam A M) rho Bot (IdCode a' b') crho cu' ev ()
...
EvalRel-down (Lam A M) rho (FunEl g) (IdCode a' b') crho cu' ev ()
...
EvalRel-down (Pi A B) rho (IdCode a b) u' crho cu' () le
-- Pi Bot
...
EvalRel-down (Pi A B) rho Bot (IdCode a' b') crho cu' ev ()
...
EvalRel-down (Pi A B) rho (PiCode a f) (IdCode a' b') crho cu' ev ()
...
-- Id: absurd non-IdCode outputs, IdCode->IdCode componentwise
EvalRel-down (Id A B) rho UCode u' crho cu' () le
EvalRel-down (Id A B) rho (FunEl g) u' crho cu' () le
EvalRel-down (Id A B) rho (PiCode a f) u' crho cu' () le
EvalRel-down (Id A B) rho Bot Bot crho cu' ev le = tt
EvalRel-down (Id A B) rho Bot UCode crho cu' ev ()
EvalRel-down (Id A B) rho Bot (FunEl g') crho cu' ev ()
EvalRel-down (Id A B) rho Bot (PiCode a' f') crho cu' ev ()
EvalRel-down (Id A B) rho Bot (IdCode a' b') crho cu' ev ()
EvalRel-down (Id A B) rho (IdCode a b) Bot crho cu' ev le = tt
EvalRel-down (Id A B) rho (IdCode a b) UCode crho cu' ev ()
EvalRel-down (Id A B) rho (IdCode a b) (FunEl g') crho cu' ev ()
EvalRel-down (Id A B) rho (IdCode a b) (PiCode a' f') crho cu' ev ()
EvalRel-down (Id A B) rho (IdCode a b) (IdCode a' b') crho cu' ev le =
  mkSigma cu' (mkSigma (EvalRel-down A rho a a' crho (fst cu') (fst (snd ev)) (fst le))
                       (EvalRel-down B rho b b' crho (snd cu') (snd (snd ev)) (snd le)))

-- proofs: only Bot output
EvalRel-down refl rho UCode u' crho cu' () le
EvalRel-down refl rho (FunEl g) u' crho cu' () le
EvalRel-down refl rho (PiCode a f) u' crho cu' () le
EvalRel-down refl rho (IdCode a b) u' crho cu' () le
EvalRel-down refl rho Bot Bot crho cu' ev le = tt
EvalRel-down refl rho Bot UCode crho cu' ev ()
EvalRel-down refl rho Bot (FunEl g') crho cu' ev ()
EvalRel-down refl rho Bot (PiCode a' f') crho cu' ev ()
EvalRel-down refl rho Bot (IdCode a' b') crho cu' ev ()
EvalRel-down (sym p) rho UCode u' crho cu' () le
EvalRel-down (sym p) rho (FunEl g) u' crho cu' () le
EvalRel-down (sym p) rho (PiCode a f) u' crho cu' () le
EvalRel-down (sym p) rho (IdCode a b) u' crho cu' () le
EvalRel-down (sym p) rho Bot Bot crho cu' ev le = tt
EvalRel-down (sym p) rho Bot UCode crho cu' ev ()
EvalRel-down (sym p) rho Bot (FunEl g') crho cu' ev ()
EvalRel-down (sym p) rho Bot (PiCode a' f') crho cu' ev ()
EvalRel-down (sym p) rho Bot (IdCode a' b') crho cu' ev ()
EvalRel-down (pi1 p) rho UCode u' crho cu' () le
EvalRel-down (pi1 p) rho (FunEl g) u' crho cu' () le
EvalRel-down (pi1 p) rho (PiCode a f) u' crho cu' () le
EvalRel-down (pi1 p) rho (IdCode a b) u' crho cu' () le
EvalRel-down (pi1 p) rho Bot Bot crho cu' ev le = tt
EvalRel-down (pi1 p) rho Bot UCode crho cu' ev ()
EvalRel-down (pi1 p) rho Bot (FunEl g') crho cu' ev ()
EvalRel-down (pi1 p) rho Bot (PiCode a' f') crho cu' ev ()
EvalRel-down (pi1 p) rho Bot (IdCode a' b') crho cu' ev ()
EvalRel-down (pi2 p N) rho UCode u' crho cu' () le
EvalRel-down (pi2 p N) rho (FunEl g) u' crho cu' () le
EvalRel-down (pi2 p N) rho (PiCode a f) u' crho cu' () le
EvalRel-down (pi2 p N) rho (IdCode a b) u' crho cu' () le
EvalRel-down (pi2 p N) rho Bot Bot crho cu' ev le = tt
EvalRel-down (pi2 p N) rho Bot UCode crho cu' ev ()
EvalRel-down (pi2 p N) rho Bot (FunEl g') crho cu' ev ()
EvalRel-down (pi2 p N) rho Bot (PiCode a' f') crho cu' ev ()
EvalRel-down (pi2 p N) rho Bot (IdCode a' b') crho cu' ev ()

-- cast: u' ≤ u ≤ v, so u' ≤ the same M-value v (LeCode-trans); B-witness reused
EvalRel-down (cast A B p M) rho u u' crho cu'
  (mkSigma cu (mkSigma v (mkSigma le-uv (mkSigma evM-v bmem)))) le =
  let cv = EvalRel-coh M rho v evM-v
  in mkSigma cu' (mkSigma v
       (mkSigma (LeCode-trans u' u v cu' cu cv le le-uv)
         (mkSigma evM-v bmem)))
...
-- App: IdCode Bot / cross / IdCode-IdCode
EvalRel-Sup (App M N) rho (IdCode a1' b1') Bot crho cu cv comp eu ev = eu
EvalRel-Sup (App M N) rho UCode (IdCode a2' b2') crho cu cv () eu ev
EvalRel-Sup (App M N) rho (FunEl g1') (IdCode a2' b2') crho cu cv () eu ev
EvalRel-Sup (App M N) rho (PiCode a1' f1') (IdCode a2' b2') crho cu cv () eu ev
EvalRel-Sup (App M N) rho (IdCode a1' b1') UCode crho cu cv () eu ev
EvalRel-Sup (App M N) rho (IdCode a1' b1') (FunEl g2') crho cu cv () eu ev
EvalRel-Sup (App M N) rho (IdCode a1' b1') (PiCode a2' f2') crho cu cv () eu ev
EvalRel-Sup (App M N) rho (IdCode a1' b1') (IdCode a2' b2') crho cu cv comp eu ev =
  let v1   = fst eu
      evN1 = fst (snd eu)
      evM1 = snd (snd eu)
      v2   = fst ev
      ...
      cv1  = EvalRel-coh N rho v1 evN1
      cv2  = EvalRel-coh N rho v2 evN2
      cM1  = EvalRel-coh M rho (FunEl (cons (mkSigma v1 (IdCode a1' b1')) nil)) evM1
      cM2  = EvalRel-coh M rho (FunEl (cons (mkSigma v2 (IdCode a2' b2')) nil)) evM2
      comp-v = EvalRel-Comp N rho crho v1 v2 evN1 evN2
      comp-M = EvalRel-Comp M rho crho
                 (FunEl (cons (mkSigma v1 (IdCode a1' b1')) nil))
                 (FunEl (cons (mkSigma v2 (IdCode a2' b2')) nil))
                 evM1 evM2
      evN-sup = EvalRel-Sup N rho v1 v2 crho cv1 cv2 comp-v evN1 evN2
      evM-2 = EvalRel-Sup M rho
                (FunEl (cons (mkSigma v1 (IdCode a1' b1')) nil))
      ...
      c-2graph = Coherent-Sup
                   ...
                   comp-M cM1 cM2
      c-supv = Coherent-Sup v1 v2 comp-v cv1 cv2
      c-result-val = Coherent-Sup (IdCode a1' b1') (IdCode a2' b2') comp cu cv
      le-down = LeFunCode-Sup-pair v1 (IdCode a1' b1') v2 (IdCode a2' b2') comp-v comp c-2graph c-supv
      c-singleton = mkCFT c-supv c-result-val tt tt tt
      evM-result = EvalRel-down M rho
                     (FunEl (cons (mkSigma v1 (IdCode a1' b1')) (cons (mkSigma v2 (IdCode a2' b2')) nil)))
                     (FunEl (cons (mkSigma (Sup v1 v2) (IdCode (Sup a1' a2') (Sup b1' b2'))) nil))
                     crho c-singleton evM-2 le-down
  in mkSigma (Sup v1 v2) (mkSigma evN-sup evM-result)
...
EvalRel-Sup (Lam A M) rho (IdCode a1 b1) v crho cu cv comp () ev
...
EvalRel-Sup (Lam A M) rho (FunEl g1) (IdCode a2 b2) crho cu cv comp eu ()
...
EvalRel-Sup (Pi A B) rho (IdCode a1 b1) v crho cu cv comp () ev
...
EvalRel-Sup (Pi A B) rho (PiCode a1 f1) (IdCode a2 b2) crho cu cv comp eu ()
...
-- Id: Bot / absurd / IdCode-IdCode componentwise
EvalRel-Sup (Id A B) rho Bot v crho cu cv comp eu ev = ev
EvalRel-Sup (Id A B) rho (IdCode a1 b1) Bot crho cu cv comp eu ev = eu
EvalRel-Sup (Id A B) rho UCode v crho cu cv comp () ev
EvalRel-Sup (Id A B) rho (FunEl g1) v crho cu cv comp () ev
EvalRel-Sup (Id A B) rho (PiCode a1 f1) v crho cu cv comp () ev
EvalRel-Sup (Id A B) rho (IdCode a1 b1) UCode crho cu cv comp eu ()
EvalRel-Sup (Id A B) rho (IdCode a1 b1) (FunEl g2) crho cu cv comp eu ()
EvalRel-Sup (Id A B) rho (IdCode a1 b1) (PiCode a2 f2) crho cu cv comp eu ()
EvalRel-Sup (Id A B) rho (IdCode a1 b1) (IdCode a2 b2) crho cu cv comp eu ev =
  let evA1 = fst (snd eu) ; evB1 = snd (snd eu)
      evA2 = fst (snd ev) ; evB2 = snd (snd ev)
      ca1 = EvalRel-coh A rho a1 evA1 ; ca2 = EvalRel-coh A rho a2 evA2
      cb1 = EvalRel-coh B rho b1 evB1 ; cb2 = EvalRel-coh B rho b2 evB2
  in mkSigma (Coherent-Sup (IdCode a1 b1) (IdCode a2 b2) comp cu cv)
             (mkSigma (EvalRel-Sup A rho a1 a2 crho ca1 ca2 (fst comp) evA1 evA2)
                      (EvalRel-Sup B rho b1 b2 crho cb1 cb2 (snd comp) evB1 evB2))

-- proofs: only Bot output
EvalRel-Sup refl rho Bot v crho cu cv comp eu ev = ev
EvalRel-Sup refl rho UCode v crho cu cv comp () ev
EvalRel-Sup refl rho (FunEl g1) v crho cu cv comp () ev
EvalRel-Sup refl rho (PiCode a1 f1) v crho cu cv comp () ev
EvalRel-Sup refl rho (IdCode a1 b1) v crho cu cv comp () ev
EvalRel-Sup (sym p) rho Bot v crho cu cv comp eu ev = ev
EvalRel-Sup (sym p) rho UCode v crho cu cv comp () ev
EvalRel-Sup (sym p) rho (FunEl g1) v crho cu cv comp () ev
EvalRel-Sup (sym p) rho (PiCode a1 f1) v crho cu cv comp () ev
EvalRel-Sup (sym p) rho (IdCode a1 b1) v crho cu cv comp () ev
EvalRel-Sup (pi1 p) rho Bot v crho cu cv comp eu ev = ev
EvalRel-Sup (pi1 p) rho UCode v crho cu cv comp () ev
EvalRel-Sup (pi1 p) rho (FunEl g1) v crho cu cv comp () ev
EvalRel-Sup (pi1 p) rho (PiCode a1 f1) v crho cu cv comp () ev
EvalRel-Sup (pi1 p) rho (IdCode a1 b1) v crho cu cv comp () ev
EvalRel-Sup (pi2 p N) rho Bot v crho cu cv comp eu ev = ev
EvalRel-Sup (pi2 p N) rho UCode v crho cu cv comp () ev
EvalRel-Sup (pi2 p N) rho (FunEl g1) v crho cu cv comp () ev
EvalRel-Sup (pi2 p N) rho (PiCode a1 f1) v crho cu cv comp () ev
EvalRel-Sup (pi2 p N) rho (IdCode a1 b1) v crho cu cv comp () ev

-- cast: Sup of two B-typed M-values is again a B-typed M-value (finMem-Sup-both),
-- and Sup u1 u2 ≤ Sup v1 v2 (LeCode-Sup-lub of the two transitive bounds).
EvalRel-Sup (cast A B p M) rho u1 u2 crho cu1 cu2 comp
  (mkSigma _ (mkSigma v1 (mkSigma le1 (mkSigma evM-v1 (mkSigma b1 (mkSigma evB-b1 fm1))))))
  (mkSigma _ (mkSigma v2 (mkSigma le2 (mkSigma evM-v2 (mkSigma b2 (mkSigma evB-b2 fm2)))))) =
  let cv1    = EvalRel-coh M rho v1 evM-v1
      cv2    = EvalRel-coh M rho v2 evM-v2
      comp-v = EvalRel-Comp M rho crho v1 v2 evM-v1 evM-v2
      cV     = Coherent-Sup v1 v2 comp-v cv1 cv2
      le-u1-V = LeCode-trans u1 v1 (Sup v1 v2) cu1 cv1 cV le1
                  (LeCode-Sup-left v1 v2 comp-v cv1 cv2)
      le-u2-V = LeCode-trans u2 v2 (Sup v1 v2) cu2 cv2 cV le2
                  (LeCode-Sup-right v1 v2 comp-v cv1 cv2)
      cb1    = EvalRel-coh B rho b1 evB-b1
      cb2    = EvalRel-coh B rho b2 evB-b2
      comp-b = EvalRel-Comp B rho crho b1 b2 evB-b1 evB-b2
  in mkSigma (Coherent-Sup u1 u2 comp cu1 cu2)
       (mkSigma (Sup v1 v2)
         (mkSigma (LeCode-Sup-lub u1 u2 (Sup v1 v2) le-u1-V le-u2-V)
           (mkSigma (EvalRel-Sup M rho v1 v2 crho cv1 cv2 comp-v evM-v1 evM-v2)
             (mkSigma (Sup b1 b2)
               (mkSigma (EvalRel-Sup B rho b1 b2 crho cb1 cb2 comp-b evB-b1 evB-b2)
                 (finMem-Sup-both v1 v2 b1 b2 fm1 fm2 comp-b comp-v))))))
